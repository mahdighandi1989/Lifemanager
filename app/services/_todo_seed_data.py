...
LISTS_DATA: dict[str, list[dict]] = {
    ...
    # ── 2 remaining large lists — content extraction deferred to a
    # subsequent commit because the PDFs are long (موضوعات برای تفکر:
    # 13 pages of philosophical prompts, کارهای اصلی این هفته:
    # 7 pages of weekly checklist items). Empty entries here so the
    # lists still exist; the user can populate them via the UI in the
    # meantime, or a future migration can extend them.
    "موضوعات برای تفکر": [],
