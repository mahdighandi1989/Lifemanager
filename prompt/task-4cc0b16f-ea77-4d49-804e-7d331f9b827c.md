---
task_id: 4cc0b16f-ea77-4d49-804e-7d331f9b827c
title: 'Dead code: Inspector Bridge Script در frontend/index.html'
type: refactor
priority: medium
...
created_at: '2026-06-02T21:12:46.662294+00:00'
updated_at: '2026-06-02T21:47:54.584552+00:00'
target_files:
- frontend/index.html
---

# Dead code: Inspector Bridge Script در frontend/index.html

## Raw Idea

فایل frontend/index.html شامل یک اسکریپت بزرگ (بیش از 400 خط) به نام 'Inspector Bridge Script' است که برای اتصال WebSocket به یک سرویس خارجی (ai-creator-backend-q677.onrender.com) و مانیتورینگ شبکه و رویدادهای کاربر (کلیک، اسکرول، تایپ) طراحی شده است. این اسکریپت با توجه به نام پروژه و آدرس WebSocket سخت‌کد شده، یک ابزار دیباگ/بازرسی خارجی است و بخشی از منطق اصلی برنامه نیست. وجود این کد مرده در فایل entry point برنامه، حجم باندل را افزایش می‌دهد و ممکن است باعث نشت اطلاعات کاربر (مانند URL‌های درخواست‌های شبکه) به سرور خارجی شود.
...
- `frontend/index.html:10-400` — `Inspector Bridge Script` — کل بلوک اسکریپت باید حذف شود.
  ```
...
Vite, React, Frontend Entry Point

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/index.html` (سطر 1) — فایل حاوی اسکریپت مرده
- `frontend/vite.config.js` (سطر 1) — می‌توان از Vite plugin برای تزریق شرطی اسکریپت استفاده کرد

## 🌐 نقشهٔ وابستگی‌ها
این اسکریپت به هیچ فایل دیگری در پروژه وابسته نیست و یک بلوک مستقل است.
...
- [ ] اسکریپت Inspector Bridge از فایل frontend/index.html حذف شده باشد.
- [ ] برنامه بدون خطا بیلد و اجرا شود.
- [ ] هیچ درخواست شبکه‌ای به آدرس ai-creator-backend-q677.onrender.com ارسال نشود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. حذف کامل اسکریپت Inspector Bridge از فایل frontend/index.html. اگر این اسکریپت برای توسعه یا دیباگ ضروری است، باید به صورت شرطی و فقط در محیط توسعه (development) لود شود.
...
## 🧪 دستورات اعتبارسنجی
- `grep -r 'Inspector Bridge' frontend/`
- `grep -r 'ai-creator-backend' frontend/`
...
1. اسکریپت Inspector Bridge از فایل frontend/index.html حذف شده باشد. _(verify: static)_
2. برنامه بدون خطا بیلد و اجرا شود. _(verify: ui_interaction)_
3. هیچ درخواست شبکه‌ای به آدرس ai-creator-backend-q677.onrender.com ارسال نشود. _(verify: ui_interaction)_
