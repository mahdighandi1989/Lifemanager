---
task_id: 7d2cc306-30a7-49c7-9f16-0947bc5920a4
title: Ensure sensitive configuration files are ignored by Git
type: security
priority: high
...
created_at: '2026-06-09T21:30:59.456666+00:00'
updated_at: '2026-06-09T22:32:49.993545+00:00'
target_files:
- .gitignore
---

# Ensure sensitive configuration files are ignored by Git

## Raw Idea

The project uses an `.env.example` file, indicating that `.env` files are used for environment-specific configurations, which often contain sensitive data like database credentials, API keys, and other secrets. It is critical to ensure that actual `.env` files (and other potential sensitive files like `credentials.json`, `*.key`) are explicitly listed in `.gitignore` to prevent accidental commitment to version control. While no live `.env` file was found in the provided file list, its absence in `.gitignore` would be a significant risk.
...
- `.gitignore:N/A` — `N/A` — Append new lines to the end of the `.gitignore` file.
  ```
  N/A
  ```
...
Git version control, environment variable management.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` — Indicates the use of .env files for configuration.

## 🌐 نقشهٔ وابستگی‌ها
This change affects Git's tracking of files and does not impact application runtime dependencies.
...
- [ ] `.gitignore` contains entries for `.env`, `*.key`, `*.pem`, `credentials.json`, and `service-account*.json`.
- [ ] Running `git check-ignore -v .env` returns a match, indicating the file is ignored.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Add `.env`, `*.key`, `*.pem`, `credentials.json`, `service-account*.json` to the `.gitignore` file to prevent sensitive information from being committed. This is a standard security practice.
...
1. `.gitignore` contains entries for `.env`, `*.key`, `*.pem`, `credentials.json`, and `service-account*.json`. _(verify: static)_
2. Running `git check-ignore -v .env` returns a match, indicating the file is ignored. _(verify: backend_test)_
