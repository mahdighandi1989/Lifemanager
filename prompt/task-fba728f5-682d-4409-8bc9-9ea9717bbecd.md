---
task_id: fba728f5-682d-4409-8bc9-9ea9717bbecd
title: ورک‌فلو CI/CD موجود فاقد مراحل تست و linting برای کد برنامه است.
...
created_at: '2026-06-09T21:32:01.023146+00:00'
updated_at: '2026-06-09T22:34:54.999643+00:00'
target_files:
- .github/workflows/ci.yml
---

# ورک‌فلو CI/CD موجود فاقد مراحل تست و linting برای کد برنامه است.

## Raw Idea

فایل `.github/workflows/claude-auto-task.yml` نشان‌دهنده وجود اتوماسیون GitHub Actions است. با این حال، یک پروژه کامل معمولاً شامل ورک‌فلوهای CI/CD برای اجرای تست‌های واحد، تست‌های یکپارچه‌سازی، linting (بررسی سبک کد) و احتمالاً ساخت ایمیج‌های داکر است. ورک‌فلو فعلی این جنبه‌های حیاتی برای تضمین کیفیت و قابلیت استقرار برنامه Lifemanager را پوشش نمی‌دهد.
...
- `.github/workflows/ci.yml:N/A` — `N/A` — فایل جدید برای ورک‌فلو CI/CD
  ```
  N/A
  ```
...
GitHub Actions، CI/CD برای برنامه‌های پایتون.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `requirements.txt` — برای نصب وابستگی‌ها در CI/CD
- `tests/` — برای اجرای تست‌ها

## 🌐 نقشهٔ وابستگی‌ها
اتوماسیون CI/CD برای تضمین کیفیت کد، شناسایی زودهنگام باگ‌ها و تسهیل استقرار ضروری است.
...
- [ ] فایل `.github/workflows/ci.yml` ایجاد شود.
- [ ] ورک‌فلو شامل مراحل نصب وابستگی‌ها و اجرای `pytest` باشد.
- [ ] ورک‌فلو در هر push به `main` و هر Pull Request اجرا شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک ورک‌فلو GitHub Actions جدید (مثلاً `ci.yml`) ایجاد کنید یا ورک‌فلو موجود را گسترش دهید تا شامل مراحل زیر باشد: نصب وابستگی‌ها از `requirements.txt`، اجرای تست‌ها با `pytest` (پس از نوشتن تست‌ها)، و اجرای ابزارهای linting (مانند `flake8` یا `black`) برای بررسی کیفیت کد. این ورک‌فلو باید در هر push به شاخه اصلی و هر Pull Request اجرا شود.
...
## 🧪 دستورات اعتبارسنجی
- `git push origin main (پس از ایجاد فایل)`

## ⚠️ ریسک‌ها و موارد احتیاط
جزئی، احتمال خطای پیکربندی در ورک‌فلو.
...
1. فایل `.github/workflows/ci.yml` ایجاد شود. _(verify: static)_
2. ورک‌فلو شامل مراحل نصب وابستگی‌ها و اجرای `pytest` باشد. _(verify: static)_
3. ورک‌فلو در هر push به `main` و هر Pull Request اجرا شود. _(verify: static)_
