---
task_id: b8cc3ee4-bebd-4871-ae81-895414b85a29
title: نقص در مدیریت خطا و بازگشت ۴۰۴ در endpoint GET /api/lists/{list_id}
type: bug
priority: high
...
created_at: '2026-06-02T21:10:28.552559+00:00'
updated_at: '2026-06-03T18:43:38.378380+00:00'
target_files:
- app/routes/lists.py
---

# نقص در مدیریت خطا و بازگشت ۴۰۴ در endpoint GET /api/lists/{list_id}

## Raw Idea

در endpoint GET /api/lists/{list_id} (فایل app/routes/lists.py)، اگر لیست مورد نظر وجود نداشته باشد، تابع list_service.get_list استثناء NoResultFound پرتاب می‌کند. این استثناء توسط دکوراتور @handle_errors گرفته شده و به ۴۰۴ تبدیل می‌شود. اما در خط ۷۷، تابع get_list پارامتر user_id را دریافت نمی‌کند و بررسی مالکیت انجام نمی‌دهد. این بدان معناست که هر کاربری می‌تواند با دانستن ID یک لیست، محتویات آن را مشاهده کند، حتی اگر به او تعلق نداشته باشد. این یک شکاف امنیتی جدی است.
...
- `app/routes/lists.py:71-105` — `get_list` — تابع get_list پارامتر user_id را دریافت نمی‌کند و بررسی مالکیت انجام نمی‌دهد.
  ```python
  @router.get(
      "/api/lists/{list_id}",
      tags=["todo-lists"],
...
- `app/services/list_service.py` (سطر 1) — شامل تابع get_list که باید بررسی مالکیت را انجام دهد
- `app/dependencies/auth.py` (سطر 1) — شامل get_optional_user_id که باید به عنوان dependency اضافه شود
...
## 🌐 نقشهٔ وابستگی‌ها
این endpoint توسط frontend/src/pages/ListDetail.jsx فراخوانی می‌شود. عدم بررسی مالکیت می‌تواند منجر به دسترسی غیرمجاز به لیست‌های خصوصی کاربران دیگر شود.
...
- [ ] endpoint GET /api/lists/{list_id} برای لیست‌های متعلق به کاربر دیگر ۴۰۴ برمی‌گرداند
- [ ] تابع list_service.get_list پارامتر user_id را دریافت می‌کند
- [ ] تست واحد جدید برای بررسی مالکیت اضافه شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. افزودن پارامتر user_id به تابع get_list در app/routes/lists.py و ارسال آن به list_service.get_list. در list_service، قبل از بازگشت لیست، بررسی شود که user_id لیست با user_id کاربر مطابقت دارد. در غیر این صورت، استثناء NoResultFound یا HTTPException با status 404 پرتاب شود.
...
_بعد:_
```
async def get_list(list_id: int, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_optional_user_id)) -> dict:
    lst = await list_service.get_list(db, list_id, user_id=user_id)
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_lists.py -k test_get_list_ownership`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر امضای تابع get_list ممکن است سایر فراخوانی‌ها را تحت تأثیر قرار دهد. نیاز به بررسی تمام فراخوانی‌های list_service.get_list در پروژه است.
...
1. endpoint GET /api/lists/{list_id} برای لیست‌های متعلق به کاربر دیگر ۴۰۴ برمی‌گرداند _(verify: api_response)_
2. تابع list_service.get_list پارامتر user_id را دریافت می‌کند _(verify: static)_
3. تست واحد جدید برای بررسی مالکیت اضافه شود _(verify: backend_test)_
