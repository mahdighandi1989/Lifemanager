---
task_id: b51caaa1-cb3a-4dfd-b452-59befbd0c9c8
title: فایل `.env.example` ممکن است حاوی مقادیر پیش‌فرض حساس باشد یا فاقد مستندات واضح باشد
type: security
priority: medium
...
created_at: '2026-06-09T21:32:00.442051+00:00'
updated_at: '2026-06-09T22:33:59.463362+00:00'
target_files:
- config/.env.example
---

# فایل `.env.example` ممکن است حاوی مقادیر پیش‌فرض حساس باشد یا فاقد مستندات واضح باشد

## Raw Idea

وجود `config/.env.example` یک روش خوب برای راهنمایی توسعه‌دهندگان در مورد متغیرهای محیطی مورد نیاز است. با این حال، بسیار مهم است که این فایل از بهترین شیوه‌های امنیتی پیروی کند: هرگز نباید حاوی مقادیر حساس واقعی (مانند اعتبارنامه پایگاه داده تولید، کلیدهای API، کلیدهای مخفی) باشد و به جای آن باید از مقادیر جایگزین یا رشته‌های خالی استفاده کند. علاوه بر این، باید هدف هر متغیر و اجباری بودن آن را به وضوح مستند کند. مقادیر پیش‌فرض ناامن یا عدم وضوح می‌تواند منجر به استفاده مستقیم توسعه‌دهندگان یا سیستم‌های استقرار از فایل نمونه شود و به طور ناخواسته اطلاعات حساس را افشا کند یا با پیکربندی‌های ضعیف مستقر شود.
...
- `config/.env.example` — محتوای این فایل محیطی نمونه را بررسی و به‌روز کنید.
...
متغیرهای محیطی، پیکربندی برنامه، پایتون.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.dockerignore` — اطمینان از اینکه فایل‌های `.env` به طور تصادفی commit یا در ایمیج‌های داکر گنجانده نمی‌شوند.
- `(فایل راه‌اندازی برنامه، مثلاً main.py)` — جایی که متغیرهای محیطی بارگذاری و اعتبارسنجی می‌شوند.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر عمدتاً بر تنظیمات پیکربندی پروژه و فرآیند ورود توسعه‌دهندگان تأثیر می‌گذارد.
...
- [ ] `config/.env.example` فقط حاوی مقادیر جایگزین یا خالی برای متغیرهای حساس است.
- [ ] هر متغیر در `.env.example` به وضوح مستند شده است.
- [ ] برنامه دارای بررسی‌های زمان اجرا برای جلوگیری از راه‌اندازی با متغیرهای محیطی حیاتی از دست رفته یا پیش‌فرض‌های ناامن است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. محتوای `config/.env.example` را بررسی کنید. هرگونه مقدار حساس واقعی را با جایگزین‌های عمومی (مانند `YOUR_SECRET_KEY_HERE`، `DB_PASSWORD=changeme`) یا رشته‌های خالی جایگزین کنید. 2. برای هر متغیر محیطی، توضیحات یا کامنت‌هایی اضافه کنید که هدف آن و اجباری بودن آن را توضیح دهد. 3. بررسی‌های سطح برنامه را در طول راه‌اندازی پیاده‌سازی کنید تا اطمینان حاصل شود که متغیرهای محیطی حیاتی تنظیم شده‌اند و حاوی مقادیر پیش‌فرض ناامن شناخته شده نیستند (مثلاً `SECRET_KEY` نباید `super-secret-key` باشد).

## 💡 نمونه‌های قبل/بعد
**نمونه محتوای `.env.example`**

_قبل:_
```
SECRET_KEY=my-insecure-default-key
...
DATABASE_URL=sqlite:///./sql_app.db # Default for local development
```
...
## 🧪 دستورات اعتبارسنجی
- `فایل `config/.env.example` را به صورت دستی بررسی کنید.`
- `تلاش برای راه‌اندازی برنامه با یک فایل `.env` حاوی پیش‌فرض‌های ناامن یا متغیرهای حیاتی از دست رفته برای تأیید بررسی‌های زمان اجرا.`

## ⚠️ ریسک‌ها و موارد احتیاط
بازسازی جزئی در صورت عدم استحکام بارگذاری پیکربندی موجود؛ احتمال مشکلات راه‌اندازی اولیه در صورت عدم وضوح مستندات.
...
1. `config/.env.example` فقط حاوی مقادیر جایگزین یا خالی برای متغیرهای حساس است. _(verify: static)_
2. هر متغیر در `.env.example` به وضوح مستند شده است. _(verify: static)_
3. برنامه دارای بررسی‌های زمان اجرا برای جلوگیری از راه‌اندازی با متغیرهای محیطی حیاتی از دست رفته یا پیش‌فرض‌های ناامن است. _(verify: backend_test)_
