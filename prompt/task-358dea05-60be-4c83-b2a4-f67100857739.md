---
task_id: 358dea05-60be-4c83-b2a4-f67100857739
title: 'endpoint بک‌اند بلااستفاده: POST /generate'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:53.918012+00:00'
updated_at: '2026-06-03T18:44:30.973051+00:00'
target_files:
- app/routes/ai.py
---
...
---
[scan #2 at 2026-06-02T21:12:54.139039+00:00]
## 📋 شرح
endpoint `PATCH /configs/{config_id}` در `app/routes/ai.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
## 
---
[scan #3 at 2026-06-02T21:12:54.249007+00:00]
## 📋 شرح
endpoint `DELETE /configs/{config_id}` در `app/routes/ai.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
#
---
[scan #4 at 2026-06-02T21:12:54.362589+00:00]
## 📋 شرح
endpoint `POST /query` در `app/routes/ai.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
endpoint بدون ca
---
[scan #5 at 2026-06-02T21:12:54.475037+00:00]
## 📋 شرح
endpoint `GET /providers/{provider_id}` در `app/routes/ai.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #6 at 2026-06-02T21:12:54.586335+00:00]
## 📋 شرح
endpoint `PATCH /providers/{provider_id}` در `app/routes/ai.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `POST /generate` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
