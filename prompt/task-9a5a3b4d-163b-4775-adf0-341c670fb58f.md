...
title: 'نقص در مکانیزم احراز هویت: get_optional_user_id در بسیاری از endpointها بدون اعتبارسنجی توکن استفاده می‌شود'
type: security
priority: critical
...
updated_at: '2026-06-05T18:00:51.879334+00:00'
target_files:
- app/dependencies/auth.py
---

# نقص در مکانیزم احراز هویت: get_optional_user_id در بسیاری از endpointها بدون اعتبارسنجی توکن استفاده می‌شود
...
- `app/dependencies/auth.py:1-50` — `get_optional_user_id` — این تابع باید بازنویسی شود تا حداقل امضای توکن را بررسی کند
  ```python
  def get_optional_user_id() -> int:
      return 0  # ⚠️ هیچ اعتبارسنجی انجام نمی‌دهد
  ```
...
FastAPI + JWT + SQLAlchemy async

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `app/routes/ai.py` (سطر 109) — استفاده از get_optional_user_id در خط 109
- `app/routes/assets.py` (سطر 45) — استفاده در خط 45
- `app/routes/context.py` (سطر 38) — استفاده در خط 38
- `app/routes/finance.py` (سطر 93) — استفاده در خط 93
- `app/routes/lists.py` (سطر 48) — استفاده در خط 48
- `app/database.py` — `auth.py` این فایل را import می‌کند
- `app/models/user.py` — `auth.py` این فایل را import می‌کند
- `app/services/auth_service.py` — `auth.py` این فایل را import می‌کند
- `app/models/user_oauth.py` — `auth.py` این فایل را import می‌کند
- `app/routes/auth_google.py` — این فایل `auth.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این تابع در بیش از 20 route مختلف استفاده شده و تغییر آن روی تمام APIهای برنامه تأثیر می‌گذارد
...
1. بازبینی کامل معماری احراز هویت: یا باید get_optional_user_id را با get_current_user جایگزین کرد برای endpointهایی که نیاز به امنیت دارند، یا مکانیزمی طراحی کرد که کاربر 0 دسترسی محدود (read-only) داشته باشد. همچنین باید migrationای نوشت که داده‌های موجود با user_id=0 را به کاربران واقعی منتقل کند.

## 💡 نمونه‌های قبل/بعد
**رفع امنیتی get_optional_user_id**
...
_بعد:_
```
async def get_optional_user_id(token: str = Depends(oauth2_scheme)) -> int:
    if not token:
        return 0
...
## 🧪 دستورات اعتبارسنجی
- `pytest tests/test_auth.py -k unauthorized`
- `curl -X GET http://localhost:8000/api/finance/incomes -H 'Authorization: Bearer invalid_token'`

## ⚠️ ریسک‌ها و موارد احتیاط
شکستن sessionهای فعلی کاربر 0؛ نیاز به migration داده
...
1. get_optional_user_id حداقل امضای JWT را بررسی کند _(verify: api_response)_
2. endpointهای حساس (finance, assets, context) از get_current_user استفاده کنند _(verify: static)_
3. داده‌های کاربر 0 به کاربران واقعی منتقل شوند _(verify: manual_only)_
4. تست‌های امنیتی برای دسترسی غیرمجاز اضافه شود _(verify: backend_test)_

## Task Steps

### Step 1: بررسی و مستندسازی تمام endpointهایی که از get_optional_user_id استفاده می‌کنند
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل اسکن کامل کدبیس برای یافتن تمام مکان‌هایی است که از تابع get_optional_user_id استفاده شده است. باید تمام فایل‌های route (assets, context, finance, ai, lists و هر فایل دیگر) بررسی شوند. خروجی این مرحله یک لیست جامع از تمام endpointها، خطوط کد، و مسیرهای فایل است. این مرحله فقط شناسایی و مستندسازی است و هیچ تغییری در کد ایجاد نمی‌کند. نکته حیاتی: هر مورد باید با مسیر دقیق فایل و شماره خط ثبت شود.
**Excerpt:**
```
در بسیاری از routeها (assets, context, finance, ai, lists, ...) از `get_optional_user_id` استفاده شده که در صورت نبود توکن، user_id=0 برمی‌گرداند. این یعنی هر کاربر ناشناس می‌تواند به داده‌های کاربر 0 (که احتمالاً کاربر پیش‌فرض است) دسترسی کامل داشته باشد. این یک شکاف امنیتی جدی است چون عملاً لایه احراز هویت را برای اکثر endpointها غیرفعال کرده. شواهد: در app/routes/ai.py خط 109 از get_optional_user_id استفاده شده، در app/routes/assets.py خط 45، در app/routes/context.py خط 38، و ده‌ها endpoint دیگر.
```

### Step 2: تحلیل ریسک و دسته‌بندی endpointهای آسیب‌پذیر بر اساس حساسیت داده
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل تحلیل هر endpoint شناسایی‌شده در مرحله قبل و دسته‌بندی آن‌ها بر اساس سطح ریسک (بالا، متوسط، پایین) است. معیارهای ریسک شامل: نوع داده‌های در معرض خطر (مالی، شخصی، تنظیمات)، قابلیت نوشتن/حذف داده، و تعداد کاربران متأثر. خروجی این مرحله یک ماتریس ریسک است که اولویت اصلاح را مشخص می‌کند. نکته حیاتی: endpointهای دارای عملیات نوشتن (POST, PUT, DELETE) باید ریسک بالاتری دریافت کنند.
**Excerpt:**
```
این یعنی هر کاربر ناشناس می‌تواند به داده‌های کاربر 0 (که احتمالاً کاربر پیش‌فرض است) دسترسی کامل داشته باشد. این یک شکاف امنیتی جدی است چون عملاً لایه احراز هویت را برای اکثر endpointها غیرفعال کرده.
```

### Step 3: طراحی راه‌حل: ایجاد تابع get_required_user_id با احراز هویت اجباری
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل طراحی یک تابع جدید به نام get_required_user_id است که برخلاف get_optional_user_id، در صورت نبود توکن معتبر، خطای 401 Unauthorized برمی‌گرداند. این تابع باید از همان مکانیزم استخراج توکن استفاده کند اما به‌جای بازگشت user_id=0، یک HTTPException با status_code=401 پرتاب کند. نکته حیاتی: این تابع باید با middleware احراز هویت موجود سازگار باشد و از همان توابع استخراج توکن استفاده کند. طراحی باید شامل signature تابع، نوع بازگشت، و پیام خطا باشد.
**Excerpt:**
```
از `get_optional_user_id` استفاده شده که در صورت نبود توکن، user_id=0 برمی‌گرداند. این یعنی هر کاربر ناشناس می‌تواند به داده‌های کاربر 0 دسترسی کامل داشته باشد.
```

### Step 4: اصلاح endpointهای بحرانی (ریسک بالا) در app/routes/finance.py برای استفاده از get_required_user_id
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل جایگزینی تمام موارد استفاده از get_optional_user_id در فایل app/routes/finance.py با get_required_user_id است. این فایل شامل endpointهای مالی است که بالاترین ریسک را دارند. تمام endpointهای GET, POST, PUT, DELETE در این فایل باید بررسی و اصلاح شوند. نکته حیاتی: باید اطمینان حاصل شود که هیچ endpoint مالی بدون احراز هویت باقی نمی‌ماند. همچنین باید تست شود که endpointها با توکن معتبر همچنان کار می‌کنند.
— [merged] این مرحله شامل جایگزینی تمام موارد استفاده از get_optional_user_id در فایل app/routes/ai.py با get_required_user_id است. این فایل شامل endpointهای هوش مصنوعی است که ممکن است به داده‌های حساس دسترسی داشته باشند. تمام endpointها باید بررسی شوند. نکته حیاتی: خط 109 که در متن کاربر ذکر شده باید اولین اولویت باشد. همچنین باید بررسی شود که آیا برخی endpointها واقعاً نیاز به دسترسی ناشناس دارند (مثلاً برای دمو) که در آن صورت باید جداگانه مدیریت شوند.
— [merged] این مرحله شامل جایگزینی تمام موارد استفاده از get_optional_user_id در فایل app/routes/assets.py با get_required_user_id است. این فایل شامل endpointهای مدیریت دارایی‌ها است. تمام endpointها باید بررسی شوند. نکته حیاتی: خط 45 که در متن کاربر ذکر شده باید اولین اولویت باشد. دارایی‌ها معمولاً داده‌های حساس کاربر هستند.
— [merged] این مرحله شامل جایگزینی تمام موارد استفاده از get_optional_user_id در فایل app/routes/context.py با get_required_user_id است. این فایل شامل endpointهای مدیریت context کاربر است. تمام endpointها باید بررسی شوند. نکته حیاتی: خط 38 که در متن کاربر ذکر شده باید اولین اولویت باشد. context کاربر معمولاً شامل داده‌های شخصی و تنظیمات است.
— [merged] این مرحله شامل جایگزینی تمام موارد استفاده از get_optional_user_id در فایل app/routes/lists.py با get_required_user_id است. این فایل شامل endpointهای مدیریت لیست‌های کاربر است. تمام endpointها باید بررسی شوند. نکته حیاتی: لیست‌ها ممکن است شامل داده‌های شخصی یا تنظیمات باشند.
**Excerpt:**
```
در بسیاری از routeها (assets, context, finance, ai, lists, ...) از `get_optional_user_id` استفاده شده... شواهد: در app/routes/ai.py خط 109 از get_optional_user_id استفاده شده، در app/routes/assets.py خط 45، در app/routes/context.py خط 38، و ده‌ها endpoint دیگر.
```

### Step 5: اصلاح سایر فایل‌های route شناسایی‌شده در مرحله 1 (ریسک متوسط)
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل جایگزینی تمام موارد استفاده از get_optional_user_id در سایر فایل‌های route که در مرحله 1 شناسایی شده‌اند اما در مراحل قبل پوشش داده نشده‌اند. این شامل فایل‌هایی با ریسک متوسط است. نکته حیاتی: هر فایل باید جداگانه بررسی شود و endpointهایی که واقعاً نیاز به دسترسی ناشناس دارند (مانند صفحه اصلی یا دمو) باید با مکانیزم دیگری مدیریت شوند.
**Excerpt:**
```
و ده‌ها endpoint دیگر
```

### Step 6: مدیریت endpointهای استثنا که نیاز به دسترسی ناشناس دارند
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل شناسایی و مدیریت endpointهایی است که واقعاً نیاز به دسترسی ناشناس دارند (مانند ثبت‌نام، ورود، صفحه اصلی، دمو). برای این endpointها باید یک مکانیزم explicit تعریف شود که نشان دهد دسترسی ناشناس مجاز است. این می‌تواند از طریق یک decorator خاص یا لیست سفید انجام شود. نکته حیاتی: این endpointها باید به صراحت مستند شوند و نباید از get_optional_user_id استفاده کنند.
**Excerpt:**
```
در صورت نبود توکن، user_id=0 برمی‌گرداند. این یعنی هر کاربر ناشناس می‌تواند به داده‌های کاربر 0 دسترسی کامل داشته باشد.
```

### Step 7: نوشتن تست‌های واحد برای get_required_user_id
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد برای تابع جدید get_required_user_id است. تست‌ها باید شامل موارد زیر باشند: 1) با توکن معتبر، user_id صحیح برگردانده شود، 2) بدون توکن، خطای 401 برگردانده شود، 3) با توکن نامعتبر، خطای 401 برگردانده شود، 4) با توکن منقضی، خطای 401 برگردانده شود. نکته حیاتی: تست‌ها باید مستقل از دیتابیس باشند و از mocking استفاده کنند.
**Excerpt:**
```
ایجاد یک تابع جدید به نام get_required_user_id که در صورت نبود توکن معتبر، خطای 401 Unauthorized برمی‌گرداند.
```

### Step 8: نوشتن تست‌های integration برای endpointهای اصلاح‌شده
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های integration برای endpointهای اصلاح‌شده در مراحل 4 تا 9 است. تست‌ها باید شامل موارد زیر باشند: 1) ارسال درخواست با توکن معتبر به هر endpoint و دریافت پاسخ موفق، 2) ارسال درخواست بدون توکن به هر endpoint و دریافت خطای 401، 3) ارسال درخواست با توکن نامعتبر به هر endpoint و دریافت خطای 401. نکته حیاتی: تست‌ها باید با دیتابیس تست کار کنند و داده‌های تست را پس از اتمام پاک کنند.
**Excerpt:**
```
اصلاح endpointهای بحرانی (ریسک بالا) در app/routes/finance.py, ai.py, assets.py, context.py, lists.py برای استفاده از get_required_user_id
```

### Step 9: بررسی و اصلاح endpointهای باقی‌مانده در سایر ماژول‌ها (admin, api, webhook)
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح تمام ماژول‌های route دیگر مانند admin, api, webhook و هر ماژول دیگری است که ممکن است از get_optional_user_id استفاده کند. این ماژول‌ها ممکن است در مسیرهای مختلفی مانند app/routes/admin/, app/routes/api/, app/routes/webhook/ قرار داشته باشند. نکته حیاتی: webhookها ممکن است نیاز به دسترسی ناشناس داشته باشند و باید جداگانه مدیریت شوند.
**Excerpt:**
```
و ده‌ها endpoint دیگر
```

### Step 10: به‌روزرسانی مستندات API برای نشان دادن نیاز به احراز هویت
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل به‌روزرسانی مستندات API (مانند Swagger/OpenAPI) برای نشان دادن اینکه کدام endpointها نیاز به احراز هویت دارند. باید برای هر endpoint اصلاح‌شده، پارامتر Authorization به عنوان required مشخص شود. نکته حیاتی: endpointهای عمومی که نیاز به احراز هویت ندارند باید به وضوح مشخص شوند.
**Excerpt:**
```
اصلاح endpointهای بحرانی برای استفاده از get_required_user_id
```

### Step 11: انجام audit امنیتی نهایی و تهیه گزارش
**Status:** `partial` (50%)
**Scope:** این مرحله شامل یک audit امنیتی نهایی برای اطمینان از عدم وجود موارد باقی‌مانده از get_optional_user_id در endpointهای حساس است. باید کل کدبیس دوباره اسکن شود و گزارش نهایی تهیه شود. نکته حیاتی: گزارش باید شامل لیست endpointهای اصلاح‌شده، endpointهای عمومی باقی‌مانده، و هرگونه استثنا باشد.
**Excerpt:**
```
این یک شکاف امنیتی جدی است چون عملاً لایه احراز هویت را برای اکثر endpointها غیرفعال کرده.
```
