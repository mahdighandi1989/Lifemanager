---
task_id: 80f72208-5a74-4c7d-9187-9c2657a39afb
title: '[منطق] Ambiguous Handling of Empty/Null `extracted_text` Input'
type: logic_audit
priority: high
...
created_at: '2026-06-02T21:12:34.655791+00:00'
updated_at: '2026-06-03T18:44:08.115345+00:00'
---

# [منطق] Ambiguous Handling of Empty/Null `extracted_text` Input
...
Assuming `DriveFile.extracted_text` is intended to be used as the `prompt` for `nlp_service`, the behavior when `extracted_text` is `None` or an empty string is not explicitly defined. The `drive_file` model's description does not specify if `extracted_text` is nullable or guaranteed to be non-empty. While `nlp_service` mentions 'falling back to a placeholder' for text generation, it's not explicitly stated if this fallback mechanism applies to empty or null input prompts, nor what the `generate

## 💥 پیامد (impact)
If `nlp_service` receives an empty or null prompt, it could lead to unexpected errors (e.g., from upstream AI providers), generate meaningless output, or consume resources unnecessarily without producing valuable results. This can break the pipeline's execution, lead to poor user experience, generat

## 🛠 پیشنهاد رفع اولیه
1. Clarify in `app/models/drive_file.py` whether `extracted_text` can be null or empty, and if so, what its default value is. 2. Explicitly define `nlp_service`'s behavior for empty/null `prompt` inputs. Specify what the `generated text` output dictionary will contain in such cases (e.g., `{"generated_text": "", "model_used": "fallback", "tokens_used": 0}` or a specific error structure). 3. Consid
...
### Step 1: تعیین وضعیت nullable و مقدار پیش‌فرض `extracted_text` در مدل `DriveFile`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح مدل `DriveFile` در فایل `app/models/drive_file.py` است. باید مشخص شود که آیا فیلد `extracted_text` می‌تواند `None` یا رشته خالی باشد. اگر nullable است، مقدار پیش‌فرض آن (مثلاً `None` یا `""`) باید به‌صراحت در مدل تعریف شود. همچنین، داک‌استرینگ یا کامنت مدل باید به‌روزرسانی شود تا این رفتار را مستند کند. خارج از این مرحله است: تغییر در سرویس `nlp_service`، نوشتن تست‌ها، یا تغییر در pipelineهای دیگر. نکته حیاتی: این تغییر باید با schema دیتابیس موجود سازگار باشد و migration جدیدی ایجاد نکند مگر اینکه ضروری باشد.
**Excerpt:**
```
1. Clarify in `app/models/drive_file.py` whether `extracted_text` can be null or empty, and if so, what its default value is.
```

### Step 2: تعریف رفتار `nlp_service` برای ورودی `prompt` خالی یا null
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح سرویس `nlp_service` است. باید به‌صراحت مشخص شود که اگر `prompt` (که از `extracted_text` می‌آید) برابر با `None` یا رشته خالی باشد، سرویس چه خروجی‌ای تولید کند. خروجی پیشنهادی: یک دیکشنری با کلیدهای `generated_text` (مقدار `""`)، `model_used` (مقدار `"fallback"`)، و `tokens_used` (مقدار `0`). همچنین، ممکن است یک ساختار خطای خاص تعریف شود. خارج از این مرحله است: تغییر در مدل `DriveFile`، نوشتن تست‌ها، یا تغییر در pipelineهای upstream.
**Excerpt:**
```
2. Explicitly define `nlp_service`'s behavior for empty/null `prompt` inputs. Specify what the `generated text` output dictionary will contain in such cases (e.g., `{"generated_text": "", "model_used": "fallback", "tokens_used": 0}` or a specific error structure).
```

### Step 3: اضافه کردن منطق fallback در pipeline `data` برای مدیریت `extracted_text` خالی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح pipeline `data` است. باید منطقی اضافه شود که قبل از ارسال `extracted_text` به `nlp_service`، بررسی کند که آیا `extracted_text` خالی یا null است. اگر چنین بود، یا باید از fallback تعریف‌شده در مرحله ۲ استفاده کند، یا مستقیماً یک مرحله skip با خروجی پیش‌فرض ایجاد کند. خارج از این مرحله است: تغییر در مدل `DriveFile` یا خود سرویس `nlp_service`. نکته حیاتی: این منطق باید با رفتار تعریف‌شده در مرحله ۲ هماهنگ باشد.
**Excerpt:**
```
3. Consider adding a check in the pipeline to handle empty/null `extracted_text` before calling `nlp_service`, potentially skipping the NLP step or using a default output.
```
