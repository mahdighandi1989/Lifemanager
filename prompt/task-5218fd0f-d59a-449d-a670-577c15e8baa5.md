---
task_id: 5218fd0f-d59a-449d-a670-577c15e8baa5
title: کامپوننت LocationTracker.jsx به endpointهای context متصل نیست و به‌عنوان dead component عمل می‌کند
type: bug
priority: high
...
created_at: '2026-06-02T21:10:28.363097+00:00'
updated_at: '2026-06-02T21:44:09.891361+00:00'
target_files:
- frontend/src/lib/api.js
- frontend/src/components/LocationTracker.jsx
---

# کامپوننت LocationTracker.jsx به endpointهای context متصل نیست و به‌عنوان dead component عمل می‌کند

## Raw Idea

کامپوننت LocationTracker.jsx در frontend/src/components/LocationTracker.jsx وجود دارد و در Layout.jsx استفاده می‌شود، اما هیچ endpoint API واقعی برای ذخیره موقعیت مکانی در frontend/lib/api.js تعریف نشده است. endpointهای backend در app/routes/context.py (POST /api/context/location و POST /api/context/physiological) وجود دارند، اما فرانت‌اند هیچ کدی برای فراخوانی آن‌ها ندارد. این باعث می‌شود کامپوننت LocationTracker عملاً هیچ داده‌ای ارسال نکند و بی‌اثر باشد.
...
- `frontend/src/lib/api.js:1-50` — `api module` — نیاز به اضافه کردن تابع جدید
  ```jsx
  // فاقد تابع recordLocation است
  ```
- `frontend/src/components/LocationTracker.jsx:1-100` — `LocationTracker` — باید به api.js متصل شود
  ```jsx
  // از navigator.geolocation استفاده می‌کند اما داده را ذخیره نمی‌کند
  ```
...
- `app/routes/context.py` (سطر 58) — endpoint POST /api/context/location که باید فراخوانی شود
- `frontend/src/components/Layout.jsx` (سطر 1) — جایی که LocationTracker رندر می‌شود
- `frontend/src/pages/AssetsPage.jsx` — این فایل `api.js` را import می‌کند (caller)
- `frontend/src/pages/BudgetPage.jsx` — این فایل `api.js` را import می‌کند (caller)
- `frontend/src/pages/DriveFiles.jsx` — این فایل `api.js` را import می‌کند (caller)
- `frontend/src/pages/ExternalProjects.jsx` — این فایل `api.js` را import می‌کند (caller)
- `frontend/src/lib/api.js` — `LocationTracker.jsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
LocationTracker در Layout.jsx استفاده می‌شود که در App.jsx رندر می‌شود. api.js توسط تمام صفحات فرانت‌اند import می‌شود.
...
- [ ] تابع recordLocation در api.js وجود داشته باشد
- [ ] LocationTracker.jsx بعد از دریافت موقعیت، recordLocation را فراخوانی کند
- [ ] داده در دیتابیس از طریق endpoint ذخیره شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. اضافه کردن تابع recordLocation به frontend/src/lib/api.js که POST /api/context/location را با latitude, longitude, accuracy_m فراخوانی کند. سپس LocationTracker.jsx را اصلاح کنید تا از این تابع استفاده کند.

## 💡 نمونه‌های قبل/بعد
**اضافه کردن recordLocation به api.js**

_قبل:_
```
// api.js فاقد این تابع
```

_بعد:_
```
export const recordLocation = (lat, lng, accuracy) =>
  api.post('/api/context/location', { lat, lng, accuracy_m: accuracy });
```
...
## 🧪 دستورات اعتبارسنجی
- `grep -r 'recordLocation' frontend/src/`
- `curl -X POST http://localhost:8000/api/context/location -H 'Content-Type: application/json' -d '{"lat":35.6892,"lng":51.3890}'`

## ⚠️ ریسک‌ها و موارد احتیاط
ندارد — اضافه کردن تابع جدید به api.js و فراخوانی آن در LocationTracker.jsx
...
1. تابع recordLocation در api.js وجود داشته باشد _(verify: static)_
2. LocationTracker.jsx بعد از دریافت موقعیت، recordLocation را فراخوانی کند _(verify: ui_interaction)_
3. داده در دیتابیس از طریق endpoint ذخیره شود _(verify: backend_test)_
