---
task_id: 16db2505-8362-49df-8682-306abcde9da6
title: 'endpoint بک‌اند بلااستفاده: WEBSOCKET /ws/ai-stream'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:57.441075+00:00'
updated_at: '2026-06-02T21:51:22.788487+00:00'
target_files:
- app/routes/ai_stream.py
---

# endpoint بک‌اند بلااستفاده: WEBSOCKET /ws/ai-stream

## Raw Idea

## 📋 شرح
endpoint `WEBSOCKET /ws/ai-stream` در `app/routes/ai_stream.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `WEBSOCKET`
- path: `/ws/ai-stream`
- فایل: `app/routes/ai_stream.py`
...
- `app/database.py` — `ai_stream.py` این فایل را import می‌کند
- `app/services/ai/task_feedback.py` — `ai_stream.py` این فایل را import می‌کند
- `app/services/ai_service.py` — `ai_stream.py` این فایل را import می‌کند
- `app/services/task_analysis.py` — `ai_stream.py` این فایل را import می‌کند
...
- [ ] مشخص شد endpoint `WEBSOCKET /ws/ai-stream` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/ws/ai-stream` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/ai_stream.py`
- `ruff check app/routes/ai_stream.py`
- `pytest -x`
...
1. مشخص شد endpoint `WEBSOCKET /ws/ai-stream` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
