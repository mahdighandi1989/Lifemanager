---
task_id: 433fe21f-9601-473e-9c6b-f5337162fe8d
title: سیستم مهاجرت پایگاه داده Alembic راه‌اندازی نشده است.
...
created_at: '2026-06-09T21:32:00.906711+00:00'
updated_at: '2026-06-09T22:34:49.001412+00:00'
target_files:
- alembic.ini
- alembic/env.py
---

# سیستم مهاجرت پایگاه داده Alembic راه‌اندازی نشده است.

## Raw Idea

وجود `alembic` در محیط مجازی نشان‌دهنده قصد استفاده از آن برای مدیریت مهاجرت‌های پایگاه داده با SQLAlchemy است. با این حال، فایل‌های پیکربندی `alembic.ini` و دایرکتوری `alembic/versions` که پس از `alembic init` ایجاد می‌شوند، در ریشه پروژه وجود ندارند. این بدان معناست که سیستم مهاجرت پایگاه داده هنوز برای مدیریت شمای پایگاه داده برنامه Lifemanager راه‌اندازی نشده است.
...
- `alembic.ini:N/A` — `N/A` — فایل پیکربندی Alembic
  ```
  N/A
  ```
- `alembic/env.py:N/A` — `N/A` — فایل محیط Alembic
  ```python
  N/A
  ```
...
SQLAlchemy, Alembic برای مهاجرت‌های پایگاه داده.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `app/database.py` — Alembic برای اتصال به پایگاه داده از این فایل استفاده خواهد کرد
- `app/models.py` — Alembic برای تشخیص تغییرات شمای پایگاه داده به مدل‌ها نیاز دارد

## 🌐 نقشهٔ وابستگی‌ها
Alembic برای مدیریت تغییرات شمای پایگاه داده در پروژه‌های SQLAlchemy ضروری است.
...
- [ ] فایل `alembic.ini` در ریشه پروژه ایجاد شود.
- [ ] دایرکتوری `alembic/` با فایل‌های `env.py` و `script.py.mako` ایجاد شود.
- [ ] دایرکتوری `alembic/versions/` ایجاد شود.
- [ ] فایل `alembic/env.py` برای import کردن مدل‌های SQLAlchemy برنامه پیکربندی شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Alembic را در ریشه پروژه راه‌اندازی کنید. این کار فایل پیکربندی `alembic.ini` و دایرکتوری `alembic/` (شامل `env.py` و `script.py.mako`) را ایجاد می‌کند. سپس `env.py` را برای اتصال به مدل‌های SQLAlchemy برنامه پیکربندی کنید.
...
هیچ، زیرا یک مرحله راه‌اندازی است.
...
1. فایل `alembic.ini` در ریشه پروژه ایجاد شود. _(verify: static)_
2. دایرکتوری `alembic/` با فایل‌های `env.py` و `script.py.mako` ایجاد شود. _(verify: static)_
3. دایرکتوری `alembic/versions/` ایجاد شود. _(verify: static)_
4. فایل `alembic/env.py` برای import کردن مدل‌های SQLAlchemy برنامه پیکربندی شود. _(verify: static)_
