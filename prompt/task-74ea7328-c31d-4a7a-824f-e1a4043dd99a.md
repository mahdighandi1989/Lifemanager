---
task_id: 74ea7328-c31d-4a7a-824f-e1a4043dd99a
title: Alembic environment is not initialized for database migrations
type: refactor
priority: medium
...
created_at: '2026-06-09T21:32:01.532372+00:00'
updated_at: '2026-06-09T22:35:27.817682+00:00'
---

# Alembic environment is not initialized for database migrations

## Raw Idea

Alembic is installed in the virtual environment, but the project lacks the necessary `alembic.ini` configuration file and `versions/` directory structure. This means database schema changes cannot be managed, tracked, or applied programmatically, which is crucial for collaborative development, deployment, and maintaining database consistency over time.
...
Alembic, SQLAlchemy 2.0, asynchronous database operations.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.venv_t/bin/alembic` — The Alembic command-line tool.
- `main.py` — Contains the SQLAlchemy `Base` and database engine configuration.
- `app/models/` — Alembic needs to discover these models for autogenerate functionality.
- `.venv_t/lib/python3.12/site-packages/alembic/templates/async/` — The recommended template for async database operations.

## 🌐 نقشهٔ وابستگی‌ها
Alembic is critical for managing database schema evolution. It depends on the SQLAlchemy models and the database connection setup. Without it, manual schema changes would be required, leading to potential inconsistencies.
...
- [ ] An `alembic.ini` file is created in the project root.
- [ ] A `migrations/` directory is created containing `env.py` and `script.py.mako`.
- [ ] `migrations/env.py` is updated to import the `Base` object and all SQLAlchemy models from the application, and configured for asynchronous operations.
- [ ] The first migration can be successfully generated using `alembic revision --autogenerate -m 'Initial migration'`.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Initialize Alembic in the project root using `alembic init -t async migrations`. This will create the `alembic.ini` file and the `migrations/` directory. Then, configure `alembic.ini` and `migrations/env.py` to correctly point to the SQLAlchemy models (from `app/models/`) and the database URL defined in `main.py` (or a dedicated database configuration file). The `async` template is chosen due to the detected `aiosqlite` and `anyio` dependencies.

## 💡 نمونه‌های قبل/بعد
**Alembic initialization command**
...
**Snippet for migrations/env.py (target_metadata and run_migrations_online)**
...
_بعد:_
```
from app.main import Base, engine # Import Base and engine from your main app file
from app.models import user, task, event # Import all models to be discovered
target_metadata = Base.metadata

# ... (other parts of env.py)
    ...
    async with connectable.connect() as connection:
        await connection.run_sync(
            lambda sync_connection: context.configure(
                connection=sync_connection, target_metadata=target_metadata, literal_binds=True
            )
        )
...
Incorrect Alembic configuration can lead to database corruption or failed migrations, especially during `autogenerate` or `upgrade` operations.
...
1. An `alembic.ini` file is created in the project root. _(verify: static)_
2. A `migrations/` directory is created containing `env.py` and `script.py.mako`. _(verify: static)_
3. `migrations/env.py` is updated to import the `Base` object and all SQLAlchemy models from the application, and configured for asynchronous operations. _(verify: static)_
4. The first migration can be successfully generated using `alembic revision --autogenerate -m 'Initial migration'`. _(verify: backend_test)_
