---
task_id: 36a7b654-a906-47cf-81f9-02c7346c0e62
title: Ensure `.env.example` includes all referenced environment variables
type: docs
priority: medium
...
created_at: '2026-06-09T21:32:00.794573+00:00'
updated_at: '2026-06-09T22:34:41.812253+00:00'
target_files:
- .env.example
---

# Ensure `.env.example` includes all referenced environment variables

## Raw Idea

The presence of `.env.example` suggests that the project relies on environment variables for configuration (e.g., database credentials, API keys, application settings). However, without access to the application's source code, it's impossible to verify if `.env.example` is comprehensive and up-to-date with all variables actually used by the application. An incomplete or outdated `.env.example` can lead to setup difficulties for new developers and deployment issues due to missing or misconfigured environment variables at runtime.
...
- `.env.example:N/A` — `N/A` — This file needs to be updated with all required environment variables.
  ```
...
Python application using environment variables for configuration.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `N/A` (سطر N/A) — All application files that read environment variables (e.g., `config.py`, `main.py`, database connection modules). Specific paths cannot be provided without source code.

## 🌐 نقشهٔ وابستگی‌ها
This affects the configuration of the entire application, including database connections, external service integrations, and general application settings, which are critical for runtime operation.
...
- [ ] Every environment variable referenced in the application code has an entry in `.env.example`.
- [ ] Each entry in `.env.example` includes a brief description of its purpose.
- [ ] The project can be successfully run by configuring variables based solely on the updated `.env.example`.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Conduct a thorough review of the entire codebase (once available) to identify all environment variables accessed (e.g., via `os.getenv()`, `pydantic.Settings`). For each identified variable, add it to `.env.example` with a descriptive comment explaining its purpose and a placeholder value. Ensure sensitive information is not committed directly.

## 💡 نمونه‌های قبل/بعد
**Before (potentially incomplete `.env.example`)**
...
**After (example of adding a missing variable with comment)**
...
Overlooking some variables could still lead to runtime errors or misconfigurations, requiring further debugging.
...
1. Every environment variable referenced in the application code has an entry in `.env.example`. _(verify: static)_
2. Each entry in `.env.example` includes a brief description of its purpose. _(verify: manual_only)_
3. The project can be successfully run by configuring variables based solely on the updated `.env.example`. _(verify: backend_test)_
