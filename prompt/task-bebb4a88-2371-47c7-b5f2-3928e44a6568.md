---
task_id: bebb4a88-2371-47c7-b5f2-3928e44a6568
title: 'endpoint بک‌اند بلااستفاده: POST /api/drive/upload'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:58.621476+00:00'
updated_at: '2026-06-03T18:44:43.130923+00:00'
target_files:
- app/routes/drive.py
---

# endpoint بک‌اند بلااستفاده: POST /api/drive/upload

## Raw Idea

## 📋 شرح
endpoint `POST /api/drive/upload` در `app/routes/drive.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/api/drive/upload`
- فایل: `app/routes/drive.py`
...
---
[scan #2 at 2026-06-02T21:12:58.862082+00:00]
## 📋 شرح
endpoint `GET /api/drive/folders` در `app/routes/drive.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `app/database.py` — `drive.py` این فایل را import می‌کند
- `app/dependencies/auth.py` — `drive.py` این فایل را import می‌کند
- `app/middleware.py` — `drive.py` این فایل را import می‌کند
- `app/models/drive_file.py` — `drive.py` این فایل را import می‌کند
...
- [ ] مشخص شد endpoint `POST /api/drive/upload` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/api/drive/upload` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/drive.py`
- `ruff check app/routes/drive.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /api/drive/upload` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
