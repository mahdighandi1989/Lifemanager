---
task_id: 203bf286-cea3-409a-b862-f42412d97f8d
...
created_at: '2026-06-09T21:30:59.030204+00:00'
updated_at: '2026-06-09T22:32:22.511106+00:00'
target_files:
- (کد برنامه ارائه نشده است)
---
...
پروژه از SQLAlchemy برای ORM و Alembic برای مهاجرت‌های پایگاه داده استفاده می‌کند، که با حضور آن‌ها در محیط مجازی مشخص است. یک مشکل رایج در این تنظیمات، عدم تطابق بین مدل‌های پایتون (که طرح‌واره مورد نظر را نشان می‌دهند) و طرح‌واره واقعی پایگاه داده تعریف شده توسط اسکریپت‌های مهاجرت Alembic است. این ناهماهنگی می‌تواند منجر به خطاهای زمان اجرا، ناسازگاری داده‌ها یا رفتار غیرمنتظره برنامه شود. بدون دسترسی به تعاریف مدل برنامه و اسکریپت‌های مهاجرت Alembic، تأیید همگام‌سازی فعلی آن‌ها غیرممکن است.
...
- `(کد برنامه ارائه نشده است):N/A` — `N/A` — فایل‌های مدل برنامه خاص (مانند `app/models.py`) و پیکربندی Alembic (`alembic.ini`, `alembic/env.py`) در لیست فایل‌های ارائه شده موجود نیستند. این یافته بر اساس حضور `SQLAlchemy` و `alembic` در محیط مجازی است.
  ```
  N/A
  ```
...
پایتون، SQLAlchemy، Alembic، پایگاه داده رابطه‌ای (احتمالاً SQLite با توجه به `aiosqlite` یا PostgreSQL/MySQL).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.venv_t/bin/alembic` — فایل اجرایی برای دستورات Alembic، نشان‌دهنده استفاده از آن.
- `.venv_t/lib/python3.12/site-packages/SQLAlchemy-2.0.36.dist-info/METADATA` — شواهدی از استفاده از ORM SQLAlchemy.
- `.venv_t/lib/python3.12/site-packages/alembic-1.14.0.dist-info/METADATA` — شواهدی از استفاده از ابزار مهاجرت Alembic.

## 🌐 نقشهٔ وابستگی‌ها
این یک نگرانی اصلی برای یکپارچگی پایگاه داده است که بر تمام بخش‌های برنامه که با پایگاه داده تعامل دارند تأثیر می‌گذارد. هرگونه تغییر در مدل داده نیاز به به‌روزرسانی مهاجرت‌های مربوطه دارد.
...
- [ ] یک کار CI/CD، `alembic check` (یا معادل آن) را اجرا می‌کند و در صورت عدم همگام‌سازی مدل‌ها و مهاجرت‌ها، با شکست مواجه می‌شود.
- [ ] مستنداتی برای توسعه‌دهندگان در مورد نحوه تولید و اعمال صحیح مهاجرت‌ها وجود دارد.
- [ ] هیچ تغییر دستی در طرح‌واره پایگاه داده بدون مهاجرت‌های Alembic مربوطه انجام نمی‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک بررسی CI/CD قوی یا یک مرحله گردش کار توسعه منظم را برای شناسایی خودکار مغایرت‌ها بین مدل‌های SQLAlchemy و مهاجرت‌های Alembic پیاده‌سازی کنید. این کار معمولاً شامل اجرای `alembic check` یا `alembic autogenerate --diff-check` در برابر مدل‌های فعلی است تا اطمینان حاصل شود که هیچ مهاجرت معلقی بر اساس مدل‌های فعلی مورد نیاز نیست، یا اینکه مهاجرت‌های موجود به درستی مدل‌ها را منعکس می‌کنند. این بررسی باید به عنوان بخشی از هوک pre-commit یا پایپ‌لاین CI انجام شود.

## 💡 نمونه‌های قبل/بعد
**مرحله مفهومی CI/CD برای بررسی مهاجرت**

_قبل:_
```
# هیچ بررسی مهاجرت صریحی در CI وجود ندارد
```
  ...
  build:
    ...
    steps:
      ...
      - name: Install dependencies
        run: pip install -r requirements.txt # فرض بر وجود requirements.txt
      - name: Check Alembic migrations sync with models
        run: alembic check # نیاز به وجود alembic.ini و فایل‌های مدل
```
...
## 🧪 دستورات اعتبارسنجی
- `alembic check`
- `alembic revision --autogenerate --check`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به درک ساختار مدل و مهاجرت موجود دارد؛ احتمال مثبت کاذب در صورت پیکربندی نادرست `alembic check` یا عدم همگام‌سازی عمدی مدل‌ها به دلایل خاص (مانند جداول قدیمی).
...
1. یک کار CI/CD، `alembic check` (یا معادل آن) را اجرا می‌کند و در صورت عدم همگام‌سازی مدل‌ها و مهاجرت‌ها، با شکست مواجه می‌شود. _(verify: backend_test)_
2. مستنداتی برای توسعه‌دهندگان در مورد نحوه تولید و اعمال صحیح مهاجرت‌ها وجود دارد. _(verify: manual_only)_
3. هیچ تغییر دستی در طرح‌واره پایگاه داده بدون مهاجرت‌های Alembic مربوطه انجام نمی‌شود. _(verify: manual_only)_
