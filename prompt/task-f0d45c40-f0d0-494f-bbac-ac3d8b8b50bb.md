---
task_id: f0d45c40-f0d0-494f-bbac-ac3d8b8b50bb
title: شرایط رقابتی (Race Conditions) احتمالی در عملیات‌های ناهمزمان یا وضعیت مشترک
type: bug
priority: medium
...
# شرایط رقابتی (Race Conditions) احتمالی در عملیات‌های ناهمزمان یا وضعیت مشترک

## Raw Idea

با توجه به استفاده از FastAPI (که ذاتاً ناهمزمان است) و Celery (برای پردازش وظایف پس‌زمینه)، احتمال بروز شرایط رقابتی در صورت دسترسی همزمان به وضعیت مشترک (مانند رکوردهای پایگاه داده، کش‌های درون حافظه، یا فایل‌ها) بدون مکانیزم‌های همگام‌سازی مناسب (مانند قفل‌ها، تراکنش‌های پایگاه داده با سطح ایزوله‌سازی مناسب، یا عملیات‌های اتمی) وجود دارد. این می‌تواند منجر به داده‌های ناسازگار یا رفتار غیرقابل پیش‌بینی برنامه شود. نیاز به تأیید با بررسی کد.
...
- `app/services/inventory.py` — `update_stock_level` — مثالی از یک سرویس که ممکن است وضعیت مشترک (مانند موجودی کالا) را اصلاح کند. مسیر و نام تابع فرضی هستند.
- `app/tasks/order_processing.py` — `process_payment` — مثالی از یک وظیفه Celery که ممکن است وضعیت مشترک (مانند وضعیت سفارش) را اصلاح کند. مسیر و نام تابع فرضی هستند.
...
FastAPI (async) + Celery (concurrent tasks) + SQLAlchemy

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `app/models/item.py` — مدل‌های پایگاه داده که ممکن است وضعیت مشترک را نشان دهند.
- `app/main.py` — فایل اصلی که ممکن است سرویس‌ها را فراخوانی کند.

## 🌐 نقشهٔ وابستگی‌ها
این یافته بر یکپارچگی داده‌ها و قابلیت اطمینان سیستم در محیط‌های با بار بالا تأثیر می‌گذارد. هر عملیاتی که وضعیت مشترک را اصلاح می‌کند، پتانسیل ایجاد شرایط رقابتی را دارد.
...
- [ ] تمام عملیات‌هایی که وضعیت مشترک را اصلاح می‌کنند، از مکانیزم‌های همگام‌سازی مناسب استفاده می‌کنند.
- [ ] تست‌های همزمانی (concurrency tests) برای سناریوهای حیاتی اضافه می‌شوند.
- [ ] یکپارچگی داده‌ها در شرایط بار بالا حفظ می‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. شناسایی تمام نقاطی که وضعیت مشترک توسط چندین درخواست FastAPI یا وظایف Celery اصلاح می‌شود. پیاده‌سازی مکانیزم‌های همگام‌سازی مناسب: استفاده از قفل‌های پایگاه داده (مانند `SELECT FOR UPDATE`)، قفل‌های توزیع‌شده (برای Celery)، یا طراحی عملیات‌ها به صورت اتمی و idempotent.

## 💡 نمونه‌های قبل/بعد
**نمونه به‌روزرسانی موجودی با قفل پایگاه داده**
...
with db.begin(): # Ensure transaction
    item = db.execute(select(Item).filter_by(id=item_id).with_for_update()).scalar_one_or_none()
    if item and item.stock >= quantity:
        item.stock -= quantity
        db.add(item)
    else:
        raise ValueError('موجودی کافی نیست یا آیتم یافت نشد.')
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest app/tests/services/test_inventory.py --concurrent`
- `celery -A app.tasks worker --loglevel=info`

## ⚠️ ریسک‌ها و موارد احتیاط
پیچیدگی در پیاده‌سازی و اشکال‌زدایی؛ احتمال کاهش عملکرد به دلیل قفل‌گذاری؛ نیاز به درک عمیق از رفتار همزمان سیستم.
