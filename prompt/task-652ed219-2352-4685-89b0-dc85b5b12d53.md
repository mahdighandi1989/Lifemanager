---
task_id: 652ed219-2352-4685-89b0-dc85b5b12d53
title: '[منطق] Lack of AI Response Validation and Parsing'
type: logic_audit
priority: critical
...
created_at: '2026-06-02T20:54:11.026022+00:00'
updated_at: '2026-06-02T20:54:11.026025+00:00'
---

# [منطق] Lack of AI Response Validation and Parsing
...
The `nlp_service.py` returns raw generated text as a string within a dictionary. While `app/schemas/ai_schema.py` mentions 'response models', there is no explicit indication that these schemas are used to validate or parse the actual output from the AI model after generation. The `model_service.py` receives this raw output and passes it along, potentially without structural or content validation.

## 💥 پیامد (impact)
Downstream components might receive malformed, incomplete, or unexpected AI responses, leading to runtime errors, incorrect data processing, or displaying unreliable information to users. This makes the pipeline brittle and difficult to debug, especially if structured output is expected.

## 🛠 پیشنهاد رفع اولیه
Implement explicit post-generation validation and parsing of the AI's output. Define specific Pydantic schemas in `app/schemas/ai_schema.py` for expected AI responses (e.g., `AIAnalysisResultSchema`). In `nlp_service.py` or `model_service.py`, attempt to parse the raw generated text into these schemas and handle validation errors (e.g., retry, flag for human review).
...
### Step 1: تعریف Pydantic schema برای خروجی AI در app/schemas/ai_schema.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یا اصلاح کلاس Pydantic `AIAnalysisResultSchema` در فایل `app/schemas/ai_schema.py` است. این schema باید ساختار مورد انتظار خروجی AI را تعریف کند (مثلاً فیلدهای `summary`, `sentiment`, `confidence_score` و غیره). خارج از این مرحله: پیاده‌سازی منطق validation در nlp_service یا model_service، تغییر در pipeline اصلی، یا نوشتن تست. نکته حیاتی: schema باید طوری طراحی شود که بتواند raw text تولیدی توسط LLM را parse کند و validation error های معنادار تولید کند.
**Excerpt:**
```
Define specific Pydantic schemas in `app/schemas/ai_schema.py` for expected AI responses (e.g., `AIAnalysisResultSchema`). In `nlp_service.py` or `model_service.py`, attempt to parse the raw generated text into these schemas and handle validation errors (e.g., retry, flag for human review).
```

### Step 2: پیاده‌سازی post-generation validation در nlp_service.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `nlp_service.py` است تا پس از دریافت raw text از AI model، آن را با استفاده از `AIAnalysisResultSchema` (تعریف‌شده در مرحله قبل) parse و validate کند. باید خطاهای validation (مثلاً `ValidationError` از Pydantic) را مدیریت کند — مثلاً با retry یا flagging برای بررسی انسانی. خارج از این مرحله: تغییر در model_service.py، نوشتن تست، یا تغییر در schema. نکته حیاتی: این مرحله فرض می‌کند که `nlp_service.py` تابعی دارد که raw output را از AI دریافت می‌کند.
**Excerpt:**
```
In `nlp_service.py` or `model_service.py`, attempt to parse the raw generated text into these schemas and handle validation errors (e.g., retry, flag for human review).
```

### Step 3: یکپارچه‌سازی validation در model_service.py (در صورت نیاز)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح فایل `model_service.py` است. اگر `model_service.py` مستقیماً raw output را از `nlp_service` دریافت می‌کند و آن را به downstream می‌فرستد، باید validation با `AIAnalysisResultSchema` در اینجا نیز انجام شود (یا حداقل بررسی شود که آیا validation قبلاً در `nlp_service` انجام شده است). خارج از این مرحله: تغییر در nlp_service.py، نوشتن تست. نکته حیاتی: اگر `model_service.py` فقط یک پاس‌دهنده است و validation در `nlp_service` انجام شده، این مرحله ممکن است فقط شامل یک بررسی و مستندسازی باشد.
**Excerpt:**
```
In `nlp_service.py` or `model_service.py`, attempt to parse the raw generated text into these schemas and handle validation errors (e.g., retry, flag for human review).
```

### Step 4: نوشتن unit tests برای validation logic
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن unit tests برای validation logic پیاده‌سازی‌شده در مراحل 2 و 3 است. تست‌ها باید شامل موارد زیر باشند: (1) یک raw text معتبر که به درستی parse شود، (2) یک raw text نامعتبر که باعث `ValidationError` شود و خطا به درستی مدیریت شود، (3) edge cases مانند empty string یا JSON نامعتبر. خارج از این مرحله: integration tests، تغییر در schema یا pipeline. نکته حیاتی: تست‌ها باید در فایل تست مربوطه (مثلاً `tests/test_nlp_service.py` یا `tests/test_model_service.py`) نوشته شوند.
**Excerpt:**
```
Implement explicit post-generation validation and parsing of the AI's output. Define specific Pydantic schemas in `app/schemas/ai_schema.py` for expected AI responses (e.g., `AIAnalysisResultSchema`). In `nlp_service.py` or `model_service.py`, attempt to parse the raw generated text into these schemas and handle validation errors (e.g., retry, flag for human review).
```
