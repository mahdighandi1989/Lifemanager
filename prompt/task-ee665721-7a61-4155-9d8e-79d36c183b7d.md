---
task_id: ee665721-7a61-4155-9d8e-79d36c183b7d
title: 'endpoint بک‌اند بلااستفاده: POST /sentiment/analyze'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:56.968341+00:00'
updated_at: '2026-06-03T18:44:35.045546+00:00'
target_files:
- app/routes/ai_profile.py
---

# endpoint بک‌اند بلااستفاده: POST /sentiment/analyze

## Raw Idea

## 📋 شرح
endpoint `POST /sentiment/analyze` در `app/routes/ai_profile.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/sentiment/analyze`
- فایل: `app/routes/ai_profile.py`
...
---
[scan #2 at 2026-06-02T21:12:57.205852+00:00]
## 📋 شرح
endpoint `GET /sentiment/profile` در `app/routes/ai_profile.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/sentiment/profile`
- فایل: `app/routes/ai_profile.py`
- علت: no frontend fetch + no recent call in l
---
[scan #3 at 2026-06-02T21:12:57.325742+00:00]
## 📋 شرح
endpoint `POST /assessments/holistic_profile` در `app/routes/ai_profile.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/assessments/holistic_profile`
- فایل: `app/routes/ai_profile.py`
- علت: no frontend fet
---
[scan #4 at 2026-06-02T21:12:57.440780+00:00]
## 📋 شرح
endpoint `GET /assessments/holistic_profile/{profile_user_id}` در `app/routes/ai_profile.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/assessments/holistic_profile/{profile_user_id}`
- فایل: `app/routes/ai_
...
- `app/config.py` — `ai_profile.py` این فایل را import می‌کند
- `app/database.py` — `ai_profile.py` این فایل را import می‌کند
- `app/dependencies/auth.py` — `ai_profile.py` این فایل را import می‌کند
- `app/middleware.py` — `ai_profile.py` این فایل را import می‌کند
- `tests/test_profile_engine_14e65214.py` — این فایل `ai_profile.py` را import می‌کند (caller)
...
- [ ] مشخص شد endpoint `POST /sentiment/analyze` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/sentiment/analyze` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/ai_profile.py`
- `ruff check app/routes/ai_profile.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /sentiment/analyze` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
