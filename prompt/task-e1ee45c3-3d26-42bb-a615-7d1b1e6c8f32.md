---
task_id: e1ee45c3-3d26-42bb-a615-7d1b1e6c8f32
title: "دکمه‌ی UI بدون handler: dismiss(idx)}\n                  className=\"bg-green-600 text-white text-xs round"
type: bug_fix
priority: medium
...
created_at: '2026-06-02T21:12:49.572754+00:00'
updated_at: '2026-06-02T21:49:12.852798+00:00'
target_files:
- frontend/src/components/RecommendationPanel.jsx
---

# دکمه‌ی UI بدون handler: dismiss(idx)}
                  className="bg-green-600 text-white text-xs round

## Raw Idea

## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/RecommendationPanel.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `dismiss(idx)}
                  className="bg-green-600 text-white text-xs round`
- فایل: `frontend/src/components/RecommendationPanel.jsx`
...
---
[scan #2 at 2026-06-02T21:12:49.815471+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/RecommendationPanel.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
---
[scan #3 at 2026-06-02T21:12:49.949396+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/deduplication/DeduplicationPanel.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `merge(group, it.id)}
                      cla
...
- `frontend/src/lib/api.js` — `RecommendationPanel.jsx` این فایل را import می‌کند
- `frontend/src/pages/Recommendations.jsx` — این فایل `RecommendationPanel.jsx` را import می‌کند (caller)
- `frontend/src/components/__tests__/Recommendations.test.jsx` — این فایل `RecommendationPanel.jsx` را import می‌کند (caller)
- `frontend/src/components/__tests__/profile_engine.test.jsx` — این فایل `RecommendationPanel.jsx` را import می‌کند (caller)
...
- [ ] git blame مشخص می‌کند چرا این دکمه `dismiss(idx)}
                  className="bg-green-600 text-white text-xs round` فاقد handler است
...
1. git blame مشخص می‌کند چرا این دکمه `dismiss(idx)}
                  className="bg-green-600 text-white text-xs round` فاقد handler است _(verify: manual_only)_
