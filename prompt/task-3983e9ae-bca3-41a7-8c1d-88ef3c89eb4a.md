---
task_id: 3983e9ae-bca3-41a7-8c1d-88ef3c89eb4a
title: Core FastAPI application structure and entry point are missing
type: refactor
priority: critical
...
The project's virtual environment contains FastAPI, Uvicorn, and related libraries, but there are no visible application files (e.g., `main.py`, `app/` directory) that define the FastAPI application instance, routes, or database connection. The project cannot run as a web service in its current state, making it an empty shell despite dependency installations. This is the most critical missing piece for a functional application.
...
Python 3.12, FastAPI, Uvicorn, SQLAlchemy 2.0 (async with `aiosqlite`).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.venv_t/bin/fastapi` — Indicates FastAPI is installed and intended for use.
- `.venv_t/bin/uvicorn` — Indicates Uvicorn is installed for serving the FastAPI app.
- `config/.env.example` — Will need to be updated with database connection details.
- `.venv_t/lib/python3.12/site-packages/aiosqlite-0.20.0.dist-info/` — Suggests using aiosqlite for async SQLite connections.

## 🌐 نقشهٔ وابستگی‌ها
This is the foundational step for the entire application. All other components (models, routes, services) will depend on this core structure and the database session setup.
...
- [ ] A `main.py` file is created in the project root.
- [ ] The `main.py` file initializes a `FastAPI` instance.
- [ ] A simple GET endpoint `/` is defined and returns a JSON response.
- [ ] Basic SQLAlchemy async engine and sessionmaker are configured using `DATABASE_URL` from environment variables.
- [ ] The application can be started successfully using `uvicorn main:app --reload` and the root endpoint is accessible.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Create a `main.py` file in the project root (or `app/main.py` for better organization) to initialize a FastAPI application. This file should include the `FastAPI` instance, a simple root endpoint (e.g., `/`), and basic setup for an asynchronous database connection using SQLAlchemy and `aiosqlite` (as detected in dependencies).
...
@app.get('/')
async def read_root():
    return {'message': 'Welcome to Lifemanager API!'}

@app.on_event('startup')
async def startup_event():
    # Create tables if they don't exist (for initial setup, remove for Alembic in production)
