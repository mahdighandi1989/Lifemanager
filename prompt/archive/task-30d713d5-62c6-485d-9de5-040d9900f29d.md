---
task_id: 30d713d5-62c6-485d-9de5-040d9900f29d
title: رسیدگی به endpoint بلااستفاده GET /api/projects/{project_id}
type: audit
priority: medium
...
created_at: '2026-05-26T20:27:20.970979+00:00'
updated_at: '2026-05-29T20:35:03.857328+00:00'
archived: true
...
# رسیدگی به endpoint بلااستفاده GET /api/projects/{project_id}
...
---
[scan #2 at 2026-05-26T20:27:21.571774+00:00]
## 📋 شرح
endpoint `POST /api/projects` در `app/routes/projects.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
## 🤔 چر
---
[scan #3 at 2026-05-26T20:27:21.882418+00:00]
## 📋 شرح
endpoint `PUT /api/projects/{project_id}` در `app/routes/projects.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #4 at 2026-05-26T20:27:22.215758+00:00]
## 📋 شرح
endpoint `DELETE /api/projects/{project_id}` در `app/routes/projects.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `GET /api/projects/{project_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
