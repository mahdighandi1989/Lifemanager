...
updated_at: '2026-06-05T04:43:38.642903+00:00'
archived: true
archived_at: '2026-06-05T04:43:32.441768+00:00'
target_files:
- .env.example
...
- [x] **مرحله 1: اضافه کردن متغیر GOOGLE_REDIRECT_URI به فایل .env.example** — این مرحله شامل افزودن متغیر محیطی GOOGLE_REDIRECT_URI به فایل .env.example است. این متغیر باید با یک مقدار placeholder (مثلاً http://localhost:8000/auth/google/callback) تعریف شود. خارج از این مرحله: تغییر در فایل app/routes/auth_google.py، تغییر در منطق OAuth، یا تغییر در سایر متغیرهای محیطی. نکته 

---

# 🔹 مرحله 1: اضافه کردن متغیر GOOGLE_REDIRECT_URI به فایل .env.example

**Scope:** این مرحله شامل افزودن متغیر محیطی GOOGLE_REDIRECT_URI به فایل .env.example است. این متغیر باید با یک مقدار placeholder (مثلاً http://localhost:8000/auth/google/callback) تعریف شود. خارج از این مرحله: تغییر در فایل app/routes/auth_google.py، تغییر در منطق OAuth، یا تغییر در سایر متغیرهای محیطی. نکته حیاتی: مقدار placeholder باید با redirect_uri واقعی که در کد استفاده می‌شود (خط 30 فایل auth_google.py) مطابقت داشته باشد.
**Key terms:** app/routes/auth_google.py, GOOGLE_REDIRECT_URI, .env.example, redirect_uri, OAuth
...
- `.env.example:حدود خط 70-75 (بعد از GOOGLE_CLIENT_SECRET)` — `GOOGLE_REDIRECT_URI` — این فایل deep-read نشده — مجری باید مسیر را خود تأیید کند. اما path از ساختار کامل پروژه انتخاب شده است.
  ```
  # GOOGLE_REDIRECT_URI must match the value registered in the
  ...
  ```
- `app/routes/auth_google.py:38-39` — `redirect_uri` — این خط نشان می‌دهد که مقدار پیش‌فرض redirect_uri چیست و placeholder باید با آن مطابقت داشته باشد.
  ```python
...
- `app/routes/auth_google.py` (سطر 38) — این فایل از settings.GOOGLE_REDIRECT_URI استفاده می‌کند (خط 38). تغییر در .env.example روی این فایل تأثیر می‌گذارد.
- `app/core/config.py` — این فایل احتمالاً settings.GOOGLE_REDIRECT_URI را از env می‌خواند. تغییر در .env.example روی این فایل تأثیر می‌گذارد.

## 🔍 Context و وضعیت فعلی
کاربر درخواست افزودن متغیر محیطی GOOGLE_REDIRECT_URI به فایل .env.example را دارد. این متغیر در فایل app/routes/auth_google.py در خط 30 برای تعیین redirect_uri در OAuth استفاده می‌شود. مقدار placeholder باید با redirect_uri واقعی که در کد استفاده می‌شود (http://localhost:8000/auth/google/callback) مطابقت داشته باشد. کاربر تأکید کرده که خارج از این مرحله: تغییری در فایل app/routes/auth_google.py، تغییر در منطق OAuth، یا تغییر در سایر متغیرهای محیطی انجام نشود. در فایل .env.example فعلی (که در deep context موجود است)، متغیر GOOGLE_REDIRECT_URI تعریف نشده است. این متغیر در خط 38 فایل app/routes/auth_google.py به صورت زیر استفاده می‌شود: redirect_uri = settings.GOOGLE_REDIRECT_URI or "http://localhost:8000/auth/google/callback". بنابراین مقدار placeholder باید دقیقاً http://localhost:8000/auth/google/callback باشد تا با fallback در کد هماهنگ باشد. کلیدواژه‌های اصلی: app/routes/auth_google.py, GOOGLE_REDIRECT_URI, .env.example, redirect_uri, OAuth.
...
1. 1. فایل .env.example را باز کنید. 2. بعد از خط GOOGLE_CLIENT_SECRET=<YOUR_GOOGLE_CLIENT_SECRET> (که در deep context موجود است)، یک خط جدید اضافه کنید با محتوای: GOOGLE_REDIRECT_URI=http://localhost:8000/auth/google/callback. 3. مطمئن شوید که مقدار placeholder (http://localhost:8000/auth/google/callback) دقیقاً با redirect_uri پیش‌فرض در خط 38 فایل app/routes/auth_google.py مطابقت دارد. 4. هیچ تغییری در فایل app/routes/auth_google.py، منطق OAuth، یا سایر متغیرهای محیطی ایجاد نکنید. 5. فایل را ذخیره کنید.
