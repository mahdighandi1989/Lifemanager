---
task_id: 32145cd6-a920-4c87-8467-21026dca420d
title: '[منطق] Missing Hallucination Detection and Mitigation'
type: logic_audit
priority: critical
...
created_at: '2026-06-02T20:54:44.394545+00:00'
updated_at: '2026-06-05T18:01:43.774515+00:00'
archived: true
archived_at: '2026-06-05T18:01:37.884404+00:00'
---

# [منطق] Missing Hallucination Detection and Mitigation
...
None of the described components include explicit mechanisms or strategies to detect, flag, or mitigate AI hallucinations. The pipeline generates text and passes it through without any explicit checks for factual accuracy, coherence, or consistency with known data.

## 💥 پیامد (impact)
The system could generate and present factually incorrect, nonsensical, or misleading information to users, severely damaging user trust and potentially leading to incorrect decisions or actions based on false data. This is a fundamental risk for LLM applications.

## 🛠 پیشنهاد رفع اولیه
Integrate hallucination detection techniques. This could involve: fact-checking against a knowledge base (potentially leveraging `app/services/ai/content_analysis_service`), confidence scoring (if available from the LLM provider), output consistency checks, or flagging uncertain outputs for human review. Prompt engineering should also aim to reduce hallucination.
...
### Step 1: تحلیل و شناسایی نقاط ورودی و خروجی pipeline ai_llm برای تزریق تشخیص توهم
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی کامل pipeline `ai_llm` برای شناسایی تمام نقاط ورودی (prompt, context, user input) و خروجی‌های (generated text, confidence scores) است. هدف، نقشه‌برداری از جریان داده و شناسایی مکان‌های مناسب برای تزریق مکانیزم‌های تشخیص توهم است. خارج از این مرحله، پیاده‌سازی خود مکانیزم‌ها یا تغییر در pipeline است. نکته حیاتی: باید فایل‌های مرتبط با `ai_llm` و `content_analysis_service` در `app/services/ai/` یافت و تحلیل شوند.
**Excerpt:**
```
در pipeline `ai_llm` یک ناسازگاری منطقی پیدا شد: None of the described components include explicit mechanisms or strategies to detect, flag, or mitigate AI hallucinations. The pipeline generates text and passes it through without any explicit checks for factual accuracy, coherence, or consistency with known data.
```

### Step 2: طراحی و پیاده‌سازی مکانیزم fact-checking با استفاده از content_analysis_service
**Status:** `done` (100%)
**Scope:** این مرحله شامل طراحی و پیاده‌سازی یک ماژول fact-checking است که از `app/services/ai/content_analysis_service` برای اعتبارسنجی حقایق در خروجی pipeline `ai_llm` استفاده می‌کند. این ماژول باید بتواند ادعاهای موجود در متن تولید شده را استخراج کرده و آن‌ها را با یک پایگاه دانش (که می‌تواند یک دانش‌نامه داخلی یا یک API خارجی باشد) تطبیق دهد. خارج از این مرحله: پیاده‌سازی confidence scoring یا consistency checks. نکته حیاتی: باید از سرویس موجود `content_analysis_service` استفاده شود، نه اینکه یک سرویس جدید از صفر ساخته شود.
**Excerpt:**
```
Integrate hallucination detection techniques. This could involve: fact-checking against a knowledge base (potentially leveraging `app/services/ai/content_analysis_service`), confidence scoring (if available from the LLM provider), output consistency checks, or flagging uncertain outputs for human review.
```

### Step 3: طراحی و پیاده‌سازی مکانیزم confidence scoring برای خروجی LLM
**Status:** `done` (100%)
**Scope:** این مرحله شامل پیاده‌سازی مکانیزمی برای دریافت و تفسیر امتیاز اطمینان (confidence score) از provider LLM است. اگر provider امتیاز اطمینان ارائه می‌دهد، این امتیاز باید گرفته، نرمالایز شده و به خروجی pipeline اضافه شود. اگر provider این قابلیت را ندارد، یک امتیاز اطمینان مصنوعی (heuristic-based) بر اساس طول پاسخ، تکرارها، یا سایر معیارها محاسبه شود. خارج از این مرحله: پیاده‌سازی fact-checking یا consistency checks. نکته حیاتی: باید مستند شود که آیا provider فعلی از confidence scoring پشتیبانی می‌کند یا خیر.
**Excerpt:**
```
confidence scoring (if available from the LLM provider), output consistency checks, or flagging uncertain outputs for human review.
```

### Step 4: طراحی و پیاده‌سازی مکانیزم output consistency checks (بررسی سازگاری خروجی)
**Status:** `done` (100%)
**Scope:** این مرحله شامل پیاده‌سازی یک ماژول است که سازگاری داخلی خروجی تولید شده را بررسی می‌کند. این شامل بررسی تناقضات منطقی در متن (مثلاً یک جمله می‌گوید 'آسمان آبی است' و جمله دیگر می‌گوید 'آسمان قرمز است')، بررسی انسجام (coherence) و اطمینان از اینکه خروجی با context ورودی سازگار است. خارج از این مرحله: fact-checking یا confidence scoring. نکته حیاتی: این ماژول باید بتواند جملات را به صورت معنایی مقایسه کند، نه فقط لغوی.
**Excerpt:**
```
output consistency checks, or flagging uncertain outputs for human review.
```

### Step 5: طراحی و پیاده‌سازی مکانیزم flagging uncertain outputs برای بازبینی انسانی
**Status:** `done` (100%)
**Scope:** این مرحله شامل پیاده‌سازی یک سیستم flagging است که خروجی‌های با اطمینان پایین (بر اساس confidence score و consistency score) را علامت‌گذاری کرده و برای بازبینی انسانی (human review) در صف قرار می‌دهد. این شامل ایجاد یک صف (queue) یا دیتابیس برای ذخیره خروجی‌های flagged، و یک API یا رابط کاربری ساده برای نمایش آن‌ها به بازبین انسانی است. خارج از این مرحله: پیاده‌سازی خودکار اصلاح خروجی‌های flagged. نکته حیاتی: باید یک آستانه (threshold) پیکربندی‌پذیر برای flagging تعریف شود.
**Excerpt:**
```
flagging uncertain outputs for human review.
```

### Step 6: بهبود prompt engineering برای کاهش توهم (hallucination reduction)
**Status:** `done` (100%)
**Scope:** این مرحله شامل بازبینی و بهبود promptهای استفاده شده در pipeline `ai_llm` برای کاهش احتمال تولید توهم است. این شامل اضافه کردن دستورالعمل‌های صریح مانند 'اگر از پاسخ خود مطمئن نیستی، بگو نمی‌دانم'، 'فقط بر اساس اطلاعات موجود در context پاسخ بده'، و 'از حدس زدن خودداری کن' است. خارج از این مرحله: تغییر در معماری pipeline یا اضافه کردن ماژول‌های جدید. نکته حیاتی: promptهای جدید باید با دقت تست شوند تا اطمینان حاصل شود که کیفیت کلی پاسخ‌ها کاهش نمی‌یابد.
**Excerpt:**
```
Prompt engineering should also aim to reduce hallucination.
```

### Step 7: یکپارچه‌سازی (integration) همه مکانیزم‌های تشخیص توهم در pipeline ai_llm
**Status:** `done` (100%)
**Scope:** این مرحله شامل یکپارچه‌سازی تمام مکانیزم‌های توسعه داده شده در مراحل قبل (fact-checking, confidence scoring, consistency checks, flagging) در pipeline اصلی `ai_llm` است. این به معنای اضافه کردن فراخوانی‌های توابع جدید به ترتیب صحیح در جریان pipeline است. خارج از این مرحله: تست یا اشکال‌زدایی (debugging) یکپارچه‌سازی. نکته حیاتی: ترتیب اجرا باید به گونه‌ای باشد که ابتدا fact-checking و consistency checks انجام شود، سپس confidence scoring محاسبه شود، و در نهایت flagging بر اساس نتایج قبلی تصمیم بگیرد.
**Excerpt:**
```
Integrate hallucination detection techniques. This could involve: fact-checking against a knowledge base (potentially leveraging `app/services/ai/content_analysis_service`), confidence scoring (if available from the LLM provider), output consistency checks, or flagging uncertain outputs for human review.
```

### Step 8: نوشتن تست‌های واحد (unit tests) برای هر مکانیزم تشخیص توهم
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد برای هر یک از مکانیزم‌های جدید است: fact-checking, confidence scoring, consistency checks, و flagging. هر تست باید یک سناریوی خاص را پوشش دهد (مثلاً تشخیص یک ادعای نادرست، تشخیص یک تناقض، محاسبه امتیاز اطمینان پایین). خارج از این مرحله: تست‌های یکپارچه‌سازی (integration tests) یا تست‌های end-to-end. نکته حیاتی: تست‌ها باید مستقل از یکدیگر و از pipeline اصلی باشند.
**Excerpt:**
```
این یک coherence issue است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 9: نوشتن تست‌های یکپارچه‌سازی (integration tests) برای pipeline اصلاح‌شده
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست‌های یکپارچه‌سازی است که کل pipeline `ai_llm` را با تمام مکانیزم‌های جدید تست می‌کند. این تست‌ها باید سناریوهای واقعی را شبیه‌سازی کنند و اطمینان حاصل کنند که همه اجزا به درستی با هم کار می‌کنند. خارج از این مرحله: تست‌های واحد (unit tests) یا تست‌های end-to-end. نکته حیاتی: این تست‌ها باید از یک LLM provider ساختگی (mock) استفاده کنند تا وابستگی به سرویس خارجی نداشته باشند.
**Excerpt:**
```
این یک coherence issue است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```
...
**Status:** `done` (100%)
**Scope:** این مرحله شامل یک audit نهایی از تمام تغییرات ایجاد شده برای اطمینان از پوشش کامل نیازمندی‌ها و عدم وجود ناسازگاری جدید است. همچنین شامل به‌روزرسانی مستندات پروژه (مثلاً README, docs/) برای منعکس کردن قابلیت‌های جدید تشخیص توهم است. خارج از این مرحله: ایجاد هرگونه تغییر جدید در کد. نکته حیاتی: این مرحله باید شامل بررسی مجدد وابستگی‌های upstream و downstream باشد تا از عدم شکستن چیزی اطمینان حاصل شود.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```
