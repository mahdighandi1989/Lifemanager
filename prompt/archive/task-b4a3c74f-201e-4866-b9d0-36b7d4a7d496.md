---
task_id: b4a3c74f-201e-4866-b9d0-36b7d4a7d496
title: پیاده‌سازی تست مسیرهای بحرانی احراز هویت (auth_google.py)
type: security
priority: critical
...
created_at: '2026-06-02T20:53:25.906373+00:00'
updated_at: '2026-06-05T18:46:00.524118+00:00'
archived: true
archived_at: '2026-06-05T18:45:54.596878+00:00'
target_files:
- app/routes/auth_google.py
- app/routes/auth_google.py
---

# پیاده‌سازی تست مسیرهای بحرانی احراز هویت (auth_google.py)

## Raw Idea

فایل app/routes/auth_google.py شامل مسیرهای حیاتی OAuth مانند /auth/google/callback، /admin/approve-user/{user_id} و /admin/pending-users است. این مسیرها مستقیماً با مدیریت کاربران و صدور توکن JWT سروکار دارند. هیچ فایل تست متناظری برای این ماژول در ساختار پروژه یافت نشد. با توجه به اینکه این مسیرها دسترسی ادمین را کنترل می‌کنند و امکان تأیید کاربران را فراهم می‌کنند، عدم وجود تست می‌تواند منجر به آسیب‌پذیری‌های امنیتی جدی شود.
...
فقدان تست برای مسیرهای بحرانی احراز هویت (auth_google.py)

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `app/routes/auth_google.py:51-105` — `google_callback` — مسیر بحرانی OAuth callback که توکن JWT صادر می‌کند
  ```python
  ...
  async def google_callback(code: str, db: AsyncSession = Depends(get_db)):
      ...
      if not token_data:
          raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to exchange authorization code for tokens.")
      id_token = token_data.get("id_token")
      if not id_token:
          raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No ID token received from Google.")
      ...
      if not email:
          raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No email received from Google.")
      user = await get_or_create_user(db, email, name)
      jwt_token = create_jwt_token(user)
      if user.status == "pending":
          return RedirectResponse(url="/auth/pending", status_code=302)
      response = RedirectResponse(url="/dashboard", status_code=302)
      response.set_cookie(key="access_token", value=f"Bearer {jwt_token}", httponly=True, max_age=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60, samesite="lax")
      return response
  ```
- `app/routes/auth_google.py:172-205` — `list_pending_users / approve_pending_user`
  ```python
  @router.get("/admin/pending-users", response_model=list[OAuthUserResponse])
  async def list_pending_users(db: AsyncSession = Depends(get_db), current_user: OAuthUser = Depends(get_current_user)):
  ...
  async def approve_pending_user(user_id: int, permissions: str = "read-only", db: AsyncSession = Depends(get_db), current_user: OAuthUser = Depends(get_current_user)):
      if current_user.email != "mohamad.mahdi1988@gmail.com":
          raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admin can approve users.")
      user = await
  ```
...
1. ایجاد فایل تست tests/test_auth_google.py با پوشش کامل مسیرهای OAuth، تأیید کاربران و مدیریت دسترسی ادمین. تست‌ها باید شامل سناریوهای موفق، خطاهای اعتبارسنجی و موارد مرزی مانند تلاش کاربر غیرادمین برای دسترسی به پنل مدیریت باشند.
