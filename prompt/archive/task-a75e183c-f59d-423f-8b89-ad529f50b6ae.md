---
task_id: a75e183c-f59d-423f-8b89-ad529f50b6ae
title: Enforce Default Role for Local User Registration
type: logic_audit
priority: critical
...
created_at: '2026-06-02T21:08:10.859794+00:00'
updated_at: '2026-06-05T18:27:40.969107+00:00'
archived: true
archived_at: '2026-06-05T18:27:34.789824+00:00'
---

# Enforce Default Role for Local User Registration
...
The `app/services/auth_service.py` component handles user registration via a `UserCreate` schema. The provided description does not specify any explicit authorization checks or secure default role/permission assignment logic during this mutation path. If the `UserCreate` schema allows users to specify their own role, or if a sensitive default role is assigned without proper validation, it creates a significant security vulnerability.

## 💥 پیامد (impact)
Unauthorized users could register accounts with elevated privileges (e.g., admin role), leading to full system compromise. Even if roles are not user-definable, the lack of explicit control over default role assignment is a critical risk.

## 🛠 پیشنهاد رفع اولیه
Implement explicit logic within `app/services/auth_service.py` (or the API endpoint calling it) to securely assign roles and permissions during local user registration. This should involve either hardcoding a least-privilege default role, using an invite system, or requiring admin approval, ensuring users cannot self-assign sensitive roles.
...
---
[scan #2 at 2026-06-02T21:08:50.279421+00:00]
## 📋 شرح ناسازگاری
در pipeline `auth` یک ناسازگاری منطقی پیدا شد:

The `app/models/user_oauth.py` defines `OAuthUser` and its associated `UserRole`/`UserPermission` enums, interacting with `app/routes/auth_google.py` (implied) for creation. Similar to local registration, the process for assigning in
...
[منطق] Unsecured Role/Permission Assignment During Local User Registration
...
### Step 1: بررسی و تحلیل منطق فعلی ثبت‌نام در auth_service.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل خواندن کامل فایل app/services/auth_service.py و شناسایی تابع/متد مسئول ثبت‌نام کاربر (UserCreate) است. باید مشخص شود که آیا schema UserCreate شامل فیلد role است یا خیر، و چه مقدار پیش‌فرضی برای role در زمان ثبت‌نام اختصاص داده می‌شود. همچنین باید بررسی شود که آیا endpoint API مربوطه (که auth_service را فراخوانی می‌کند) هرگونه validation یا authorization چک قبل از ثبت‌نام انجام می‌دهد. خروجی این مرحله یک گزارش تحلیلی از وضعیت فعلی است، نه تغییر در کد.
**Excerpt:**
```
The `app/services/auth_service.py` component handles user registration via a `UserCreate` schema. The provided description does not specify any explicit authorization checks or secure default role/permission assignment logic during this mutation path. If the `UserCreate` schema allows users to specify their own role, or if a sensitive default role is assigned without proper validation, it creates a significant security vulnerability.
```

### Step 2: ایمپورت وابستگی‌های مورد نیاز برای مدیریت نقش در auth_service.py
**Status:** `done` (100%)
**Scope:** در این مرحله، ماژول‌ها و کلاس‌های مورد نیاز برای مدیریت نقش و مجوزها به فایل app/services/auth_service.py اضافه می‌شوند. این شامل ایمپورت کردن enum نقش‌ها (در صورت وجود)، مدل‌های دیتابیس مرتبط با نقش، و هرگونه ابزار validation است. خارج از scope این مرحله، تغییر منطق اصلی ثبت‌نام است.
**Excerpt:**
```
Implement explicit logic within `app/services/auth_service.py` (or the API endpoint calling it) to securely assign roles and permissions during local user registration.
```

### Step 3: تعریف یا اصلاح enum نقش‌های مجاز (Roles) در صورت نیاز
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی وجود یک enum برای نقش‌های کاربر (مثلاً RoleEnum) در پروژه است. اگر وجود ندارد، یک enum جدید با نقش‌های مجاز (مثلاً user, admin, moderator) تعریف می‌شود. اگر وجود دارد، بررسی می‌شود که آیا نقش‌های حساس (مثل admin) به درستی تعریف شده‌اند. خارج از scope این مرحله، تغییر منطق ثبت‌نام است.
**Excerpt:**
```
This should involve either hardcoding a least-privilege default role, using an invite system, or requiring admin approval, ensuring users cannot self-assign sensitive roles.
```

### Step 4: اصلاح schema UserCreate برای حذف یا غیرفعال کردن فیلد role (در صورت وجود)
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی schema UserCreate (که احتمالاً در فایل app/schemas/user.py یا مشابه آن تعریف شده) است. اگر این schema شامل یک فیلد role است که کاربر می‌تواند مقداردهی کند، این فیلد باید حذف شود یا به‌صورت read-only (با validator) درآید تا کاربر نتواند آن را تنظیم کند. خارج از scope این مرحله، تغییر منطق سرویس ثبت‌نام است.
**Excerpt:**
```
If the `UserCreate` schema allows users to specify their own role, or if a sensitive default role is assigned without proper validation, it creates a significant security vulnerability.
```

### Step 5: پیاده‌سازی منطق تخصیص نقش پیش‌فرض (least-privilege) در تابع ثبت‌نام auth_service
**Status:** `done` (100%)
**Scope:** در این مرحله، در تابع/متد مسئول ثبت‌نام در app/services/auth_service.py، منطقی اضافه می‌شود که پس از ایجاد کاربر، یک نقش پیش‌فرض با کمترین دسترسی (مثلاً 'user' یا 'viewer') به او تخصیص دهد. این نقش باید از enum تعریف‌شده در مرحله ۳ استفاده کند. خارج از scope این مرحله، پیاده‌سازی invite system یا تایید ادمین است.
**Excerpt:**
```
Implement explicit logic within `app/services/auth_service.py` (or the API endpoint calling it) to securely assign roles and permissions during local user registration. This should involve either hardcoding a least-privilege default role
```

### Step 6: اضافه کردن validation برای جلوگیری از تخصیص نقش‌های حساس در صورت عبور از schema
**Status:** `done` (100%)
**Scope:** به عنوان یک لایه امنیتی اضافی، در تابع ثبت‌نام auth_service، یک validation صریح اضافه می‌شود که اگر به هر دلیلی (مثلاً باگ در schema یا فراخوانی مستقیم) درخواستی با نقش حساس (admin, superuser) به تابع برسد، آن را رد کرده و خطا برگرداند. خارج از scope این مرحله، تغییر schema است.
**Excerpt:**
```
ensuring users cannot self-assign sensitive roles.
```

### Step 7: بررسی و اصلاح endpoint API فراخواننده auth_service (در صورت نیاز)
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی endpoint API (مثلاً در فایل app/api/v1/auth.py یا مشابه) است که تابع ثبت‌نام auth_service را فراخوانی می‌کند. باید اطمینان حاصل شود که این endpoint هیچ roleای از درخواست کاربر به سرویس پاس نمی‌دهد و فقط از schema اصلاح‌شده UserCreate استفاده می‌کند. خارج از scope این مرحله، تغییر خود auth_service است.
**Excerpt:**
```
Implement explicit logic within `app/services/auth_service.py` (or the API endpoint calling it) to securely assign roles and permissions during local user registration.
```

### Step 8: نوشتن تست‌های واحد برای سناریوهای امنیتی ثبت‌نام
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد (با pytest یا فریمورک مشابه) برای سناریوهای زیر است: 1) ثبت‌نام عادی با نقش پیش‌فرض user. 2) تلاش برای ثبت‌نام با نقش admin در schema (که باید رد شود). 3) تلاش برای ثبت‌نام با نقش نامعتبر. 4) اطمینان از اینکه نقش پیش‌فرض در دیتابیس ذخیره می‌شود. خارج از scope این مرحله، تست‌های integration یا end-to-end است.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 9: بررسی وابستگی‌های upstream و downstream برای اطمینان از عدم شکستن سایر بخش‌ها
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی کامل وابستگی‌هاست: upstream (چه چیزهایی به auth_service و UserCreate وابسته هستند) و downstream (auth_service به چه چیزهایی وابسته است). باید اطمینان حاصل شود که تغییرات ایجادشده (حذف فیلد role از schema، تغییر منطق ثبت‌نام) باعث شکستن سایر بخش‌های سیستم (مثلاً پنل ادمین، سیستم invite، یا سرویس‌های دیگر) نمی‌شود. خارج از scope این مرحله، ایجاد تغییرات جدید است.
**Excerpt:**
```
پیش از و حین تغییر، تمام وابستگی‌ها را در **چهار جهت** به‌طور **کامل و بدون هیچ خلاصه‌سازی** شناسایی و همگام کن: ۱. وابستگی‌های upstream (این تسک به چه چیزهایی متکی است): - چه فایل‌ها، توابع، کلاس‌ها، API endpoint ها، schema های دیتابیس، env vars، یا config هایی که این تسک نیاز دارد؟ - آیا قرار است چیزی را ویرایش/حذف کنی که جای دیگر (signature، رفتار، return type، side effect) از آن انتظار خاصی می‌رود؟ ۲. وابستگی‌های downstream (چه چیزهایی به این تسک متکی‌اند؟)
```
