...
title: Align AuthContext user.id contract
type: logic_audit
priority: critical
...
created_at: '2026-06-02T20:55:08.218550+00:00'
updated_at: '2026-06-05T20:05:58.138265+00:00'
archived: true
archived_at: '2026-06-05T20:05:58.138247+00:00'
---
...
[منطق] AuthContext 'user' object lacks explicit user_id
...
### Step 1: تعریف صریح فیلد `id` در خروجی `AuthContext`
**Status:** `done` (100%)
**Scope:** این مرحله شامل اصلاح کامپوننت `AuthContext.jsx` (یا معادل آن در فرانت‌اند) است تا خروجی `user: object` به‌طور صریح شامل فیلد `id` (یا `user_id`) باشد که با کلید اصلی جدول `users` در بک‌اند مطابقت دارد. این مرحله شامل تغییرات در بک‌اند یا مدل `UserContext` نمی‌شود. نکته حیاتی: باید اطمینان حاصل شود که فیلد `id` از نوع `string` (UUID) است و با شناسه‌ی کاربر در backend هماهنگ است. همچنین باید مستندات یا TypeScript type مربوط به `AuthContext` به‌روزرسانی شود تا این قرارداد را منعکس کند.
**Excerpt:**
```
Explicitly define that the `user: object` output by `AuthContext` will include a `id` or `user_id` field (e.g., `user: { id: str, email: str, username: str, ... }`) that corresponds to the primary key of the `users` table in the backend.
```

### Step 2: به‌روزرسانی TypeScript type/interface برای `AuthContext`
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یا به‌روزرسانی TypeScript type یا interface مربوط به خروجی `AuthContext` است تا فیلد `id` (از نوع `string`) به‌طور صریح تعریف شود. این type باید در فایل مربوط به types (مثلاً `types/auth.ts` یا داخل خود `AuthContext.tsx`) قرار گیرد. این مرحله شامل تغییر در منطق کامپوننت نمی‌شود، فقط type definition را اصلاح می‌کند.
**Excerpt:**
```
Explicitly define that the `user: object` output by `AuthContext` will include a `id` or `user_id` field (e.g., `user: { id: str, email: str, username: str, ... }`) that corresponds to the primary key of the `users` table in the backend.
```

### Step 3: بررسی و هماهنگی فیلد `id` در `app/models/context.py`
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی مدل `UserContext` در فایل `app/models/context.py` است تا اطمینان حاصل شود که فیلد `user_id` (ForeignKey به جدول users) با فیلد `id` که از `AuthContext` می‌آید، هماهنگ است. اگر `UserContext` از `user_id` استفاده می‌کند، باید اطمینان حاصل شود که این فیلد با `id` خروجی `AuthContext` مطابقت دارد. این مرحله شامل تغییر در مدل `UserContext` نمی‌شود مگر اینکه ناسازگاری آشکاری یافت شود.
**Excerpt:**
```
The `AuthContext.jsx` component outputs a generic `user: object (user data)` upon successful authentication. However, `app/models/context.py`'s `UserContext` model relies on a `user_id` (via `ForeignKey` to a `users table`) to link contextual data to a specific user. The current description of `AuthContext`'s output does not explicitly guarantee that the `user: object` will contain a field like `id` or `user_id` that is consistent with the backend's user identification scheme.
```

### Step 4: اضافه کردن تست واحد برای اطمینان از وجود `id` در خروجی `AuthContext`
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن یک تست واحد (unit test) برای کامپوننت `AuthContext` است که بررسی می‌کند پس از احراز هویت موفق، شیء `user` شامل فیلد `id` (از نوع string) باشد. این تست باید در فایل تست مربوط به `AuthContext` (مثلاً `AuthContext.test.tsx`) قرار گیرد. این مرحله شامل تست‌های integration یا end-to-end نمی‌شود.
**Excerpt:**
```
Without a guaranteed and consistent `user_id` from the frontend authentication flow, it becomes difficult or impossible for downstream components (e.g., a service layer on the backend) to reliably fetch or store `UserContext` data for the currently authenticated user. This breaks the logical link be
```

### Step 5: اضافه کردن تست واحد برای اطمینان از هماهنگی `user_id` در `UserContext`
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن یک تست واحد برای مدل `UserContext` در فایل `app/models/context.py` است که بررسی می‌کند فیلد `user_id` با یک `id` معتبر (که از `AuthContext` می‌آید) قابل مقداردهی است و نوع داده‌ی آن (مثلاً UUID) با نوع `id` در فرانت‌اند سازگار است. این تست باید در فایل تست مربوط به models (مثلاً `tests/test_models.py`) قرار گیرد.
**Excerpt:**
```
Without a guaranteed and consistent `user_id` from the frontend authentication flow, it becomes difficult or impossible for downstream components (e.g., a service layer on the backend) to reliably fetch or store `UserContext` data for the currently authenticated user. This breaks the logical link be
```

### Step 6: مستندسازی قرارداد `user.id` در README یا docs مرتبط
**Status:** `partial` (80%)
**Scope:** این مرحله شامل به‌روزرسانی مستندات پروژه (مانند README.md یا فایل docs مخصوص auth) است تا به‌طور صریح بیان کند که خروجی `AuthContext` شامل فیلد `id` است که با `user_id` در `UserContext` مطابقت دارد. این مستندات باید شامل مثال‌هایی از استفاده باشد. این مرحله شامل تغییر در کد نمی‌شود.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```
