---
task_id: 05142926-3256-4a9d-97cc-3c84245f2394
title: عدم اعتبارسنجی جامع ورودی در نقاط پایانی API
type: bug
priority: medium
...
created_at: '2026-06-09T21:32:01.659864+00:00'
updated_at: '2026-06-09T22:35:39.252043+00:00'
target_files:
- app/api/routes/items.py
- app/api/routes/users.py
---

# عدم اعتبارسنجی جامع ورودی در نقاط پایانی API

## Raw Idea

پروژه‌های FastAPI از Pydantic برای اعتبارسنجی مدل داده استفاده می‌کنند، اما اغلب اعتبارسنجی‌های پیچیده‌تر مربوط به منطق کسب‌وکار، محدودیت‌های خاص (مانند طول رشته، فرمت‌های خاص، یا اعتبارسنجی متقابل فیلدها) یا مدیریت edge case‌ها (مانند ورودی‌های Unicode خاص یا مقادیر بسیار بزرگ/کوچک) ممکن است نادیده گرفته شوند. بدون بررسی کد، این یک نقطه ضعف رایج است که می‌تواند منجر به داده‌های نامعتبر، رفتار غیرمنتظره برنامه، یا حتی آسیب‌پذیری‌های امنیتی شود. نیاز به تأیید با بررسی کد.
...
- `app/api/routes/items.py` — `create_item` — مثالی از یک نقطه پایانی که ممکن است نیاز به اعتبارسنجی ورودی بیشتری داشته باشد. مسیر و نام تابع فرضی هستند.
- `app/api/routes/users.py` — `update_user_profile` — مثالی دیگر از یک نقطه پایانی که ممکن است نیاز به اعتبارسنجی ورودی بیشتری داشته باشد. مسیر و نام تابع فرضی هستند.
...
- `app/schemas.py` — فایل شمای Pydantic که مدل‌های ورودی را تعریف می‌کند و ممکن است نیاز به اعتبارسنجی‌های بیشتری داشته باشد.
- `app/main.py` — فایل اصلی FastAPI که روترها را شامل می‌شود.

## 🌐 نقشهٔ وابستگی‌ها
این یافته به نحوه تعامل API با داده‌های ورودی مربوط می‌شود و بر تمام نقاط پایانی که ورودی کاربر را می‌پذیرند تأثیر می‌گذارد. تغییرات در اعتبارسنجی می‌تواند بر رفتار کلاینت‌ها تأثیر بگذارد.
...
- [ ] تمام نقاط پایانی API ورودی‌های خود را در برابر محدودیت‌های منطق کسب‌وکار (مانند طول، فرمت، محدوده) اعتبارسنجی می‌کنند.
- [ ] ورودی‌های نامعتبر با خطاهای HTTP مناسب (مانند 400 Bad Request) پاسخ داده می‌شوند.
- [ ] تست‌های واحد برای پوشش edge case‌های اعتبارسنجی ورودی اضافه می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. بررسی دقیق تمام نقاط پایانی API برای اطمینان از اینکه ورودی‌ها نه تنها از نظر شمای Pydantic، بلکه از نظر منطق کسب‌وکار و edge case‌های احتمالی نیز به طور کامل اعتبارسنجی می‌شوند. پیاده‌سازی اعتبارسنجی‌های سفارشی در لایه سرویس یا با استفاده از `Field` validators در Pydantic در صورت لزوم.

## 💡 نمونه‌های قبل/بعد
**نمونه اعتبارسنجی طول رشته**
...
class ItemCreate(BaseModel):
    name: str = Field(min_length=3, max_length=50, description='نام آیتم باید بین 3 تا 50 کاراکتر باشد.')
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest app/tests/api/test_items.py`
- `pytest app/tests/api/test_users.py`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال شکستن کلاینت‌های موجود که ممکن است ورودی‌های نامعتبر را ارسال می‌کردند؛ نیاز به ارتباط با توسعه‌دهندگان فرانت‌اند.
...
1. تمام نقاط پایانی API ورودی‌های خود را در برابر محدودیت‌های منطق کسب‌وکار (مانند طول، فرمت، محدوده) اعتبارسنجی می‌کنند. _(verify: api_response)_
2. ورودی‌های نامعتبر با خطاهای HTTP مناسب (مانند 400 Bad Request) پاسخ داده می‌شوند. _(verify: api_response)_
3. تست‌های واحد برای پوشش edge case‌های اعتبارسنجی ورودی اضافه می‌شوند. _(verify: backend_test)_
