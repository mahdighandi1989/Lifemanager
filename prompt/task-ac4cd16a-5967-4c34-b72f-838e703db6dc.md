---
task_id: ac4cd16a-5967-4c34-b72f-838e703db6dc
title: 'endpoint بک‌اند بلااستفاده: GET /api/files/{file_id}'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:59.096862+00:00'
updated_at: '2026-06-02T21:53:06.166051+00:00'
target_files:
- app/routes/files.py
---

# endpoint بک‌اند بلااستفاده: GET /api/files/{file_id}

## Raw Idea

## 📋 شرح
endpoint `GET /api/files/{file_id}` در `app/routes/files.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/files/{file_id}`
- فایل: `app/routes/files.py`
...
- `app/database.py` — `files.py` این فایل را import می‌کند
- `app/dependencies/auth.py` — `files.py` این فایل را import می‌کند
- `app/middleware.py` — `files.py` این فایل را import می‌کند
- `app/models/drive_file.py` — `files.py` این فایل را import می‌کند
- `tests/test_drive_sheets_tiering_7367c6f0.py` — این فایل `files.py` را import می‌کند (caller)
- `tests/test_drive_tiering_7367c6f0.py` — این فایل `files.py` را import می‌کند (caller)
...
- [ ] مشخص شد endpoint `GET /api/files/{file_id}` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/api/files/{file_id}` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/files.py`
- `ruff check app/routes/files.py`
- `pytest -x`
...
1. مشخص شد endpoint `GET /api/files/{file_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
