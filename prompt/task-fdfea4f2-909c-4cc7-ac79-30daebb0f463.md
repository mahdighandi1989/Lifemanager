---
task_id: fdfea4f2-909c-4cc7-ac79-30daebb0f463
title: پروژه فاقد فایل مدیریت وابستگی‌های صریح (مانند requirements.txt) است
type: security
priority: high
...
# پروژه فاقد فایل مدیریت وابستگی‌های صریح (مانند requirements.txt) است

## Raw Idea

لیست فایل‌های ارائه شده، پکیج‌های پایتون نصب شده در یک محیط مجازی (`.venv_t`) را نشان می‌دهد، اما هیچ فایل مانیفست وابستگی صریحی مانند `requirements.txt`، `pyproject.toml` (برای Poetry/PDM) یا `Pipfile` (برای Pipenv) در ریشه مخزن وجود ندارد. این موضوع ردیابی دقیق، بازتولید و ممیزی نسخه‌های دقیق وابستگی‌های پروژه را دشوار می‌کند. بدون یک لیست قطعی، اطمینان از اینکه همه توسعه‌دهندگان از نسخه‌های یکسان استفاده می‌کنند، دشوار است که می‌تواند منجر به مشکلات 'روی سیستم من کار می‌کند' و مهم‌تر از آن، از اسکن خودکار ابزارها برای آسیب‌پذیری‌های شناخته شده جلوگیری می‌کند.
...
- `requirements.txt` — این فایل باید در ریشه مخزن ایجاد شود.
...
پروژه پایتون، `pip`، محیط‌های مجازی، مدیریت وابستگی‌ها (FastAPI, SQLAlchemy, Celery).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.dockerignore` — وابستگی‌ها اغلب در طول ساخت ایمیج داکر نصب می‌شوند و نیاز به `requirements.txt` دارند.
- `.github/workflows/claude-auto-task.yml` — ورک‌فلوهای CI/CD برای نصب وابستگی‌ها و اجرای اسکن‌های امنیتی به این فایل نیاز خواهند داشت.

## 🌐 نقشهٔ وابستگی‌ها
این یک تغییر اساسی است که بر نحوه مدیریت و نصب تمام وابستگی‌های پایتون در محیط‌های توسعه، تست و تولید تأثیر می‌گذارد.
...
- [ ] یک فایل `requirements.txt` در ریشه مخزن وجود دارد.
- [ ] فایل تمام وابستگی‌های مستقیم پروژه را با نسخه‌های پین شده لیست می‌کند.
- [ ] پروژه را می‌توان با استفاده از `pip install -r requirements.txt` با موفقیت نصب و اجرا کرد.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل `requirements.txt` در ریشه مخزن با فریز کردن وابستگی‌های نصب شده فعلی (مثلاً با `pip freeze > requirements.txt`) ایجاد کنید. سپس، این فایل را با افزودن صریح وابستگی‌های جدید و به‌روزرسانی موارد موجود، نگهداری کنید. برای مدیریت بهتر وابستگی‌ها و فایل‌های قفل، استفاده از ابزارهای مدرن‌تر مانند Poetry یا PDM با `pyproject.toml` را در آینده در نظر بگیرید.
...
_قبل:_
```
(فایل requirements.txt وجود ندارد)
```
...
هیچ، ثبات و وضعیت امنیتی پروژه را بهبود می‌بخشد.
