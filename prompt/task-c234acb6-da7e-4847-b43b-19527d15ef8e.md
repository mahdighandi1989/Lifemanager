---
task_id: c234acb6-da7e-4847-b43b-19527d15ef8e
title: مدیریت خطای ناکافی و لاگ‌برداری برای عملیات‌های حیاتی پایگاه داده
type: bug
priority: medium
...
created_at: '2026-06-09T21:32:01.771432+00:00'
updated_at: '2026-06-09T22:35:46.498156+00:00'
target_files:
- app/services/database.py
- app/models/base.py
---

# مدیریت خطای ناکافی و لاگ‌برداری برای عملیات‌های حیاتی پایگاه داده

## Raw Idea

عملیات‌های پایگاه داده (مانند درج، به‌روزرسانی، حذف، یا حتی اتصال) نقاط حیاتی هستند که می‌توانند با شکست مواجه شوند. مدیریت خطای ضعیف (مانند استفاده از `except Exception:` عمومی بدون لاگ‌برداری کافی، یا نادیده گرفتن استثنائات) می‌تواند منجر به شکست‌های خاموش، داده‌های ناسازگار، یا دشواری در اشکال‌زدایی شود. همچنین، عدم اطمینان از rollback تراکنش‌ها در صورت خطا می‌تواند به یکپارچگی داده‌ها آسیب برساند. نیاز به تأیید با بررسی کد.
...
- `app/services/database.py` — `get_db_session` — فایل یا ماژولی که اتصال به پایگاه داده و مدیریت Session را انجام می‌دهد. مسیر و نام تابع فرضی هستند.
- `app/models/base.py` — `save_instance` — متدهای ORM که عملیات‌های درج/به‌روزرسانی را انجام می‌دهند. مسیر و نام تابع فرضی هستند.
...
FastAPI + SQLAlchemy + aiosqlite (احتمالاً برای توسعه/تست)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `app/main.py` — فایل اصلی که ممکن است وابستگی‌های پایگاه داده را تزریق کند.
- `app/config.py` — فایل پیکربندی که تنظیمات پایگاه داده را نگه می‌دارد.

## 🌐 نقشهٔ وابستگی‌ها
این یافته بر پایداری و یکپارچگی داده‌های برنامه تأثیر می‌گذارد و هر بخشی که با پایگاه داده تعامل دارد را تحت تأثیر قرار می‌دهد.
...
- [ ] تمام عملیات‌های پایگاه داده در `try/except` بلاک‌های مناسب قرار دارند.
- [ ] خطاهای پایگاه داده با جزئیات کافی (شامل traceback) لاگ می‌شوند.
- [ ] تراکنش‌های پایگاه داده در صورت بروز خطا به درستی rollback می‌شوند.
- [ ] تست‌های واحد برای سناریوهای شکست پایگاه داده اضافه می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. پیاده‌سازی `try/except` بلاک‌های مشخص‌تر برای عملیات‌های پایگاه داده. اطمینان از لاگ‌برداری دقیق جزئیات خطا (شامل traceback) در صورت بروز مشکل. استفاده از context manager‌ها یا بلوک‌های `finally` برای اطمینان از rollback تراکنش‌ها در صورت شکست. بررسی استفاده از SQLAlchemy `Session` و مدیریت آن.

## 💡 نمونه‌های قبل/بعد
**نمونه مدیریت خطا با لاگ‌برداری و rollback**
...
try:
    ...
    raise HTTPException(status_code=500, detail='خطا در ذخیره داده.')
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest app/tests/services/test_database.py`
- `pytest app/tests/models/test_items.py`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به بازبینی گسترده کد در لایه دسترسی به داده؛ احتمال معرفی رگرسیون در مدیریت تراکنش‌ها.
...
1. تمام عملیات‌های پایگاه داده در `try/except` بلاک‌های مناسب قرار دارند. _(verify: static)_
2. خطاهای پایگاه داده با جزئیات کافی (شامل traceback) لاگ می‌شوند. _(verify: static)_
3. تراکنش‌های پایگاه داده در صورت بروز خطا به درستی rollback می‌شوند. _(verify: static)_
4. تست‌های واحد برای سناریوهای شکست پایگاه داده اضافه می‌شوند. _(verify: backend_test)_
