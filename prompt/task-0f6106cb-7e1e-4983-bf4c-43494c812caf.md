---
task_id: 0f6106cb-7e1e-4983-bf4c-43494c812caf
title: 'endpoint بک‌اند بلااستفاده: POST /api/external-projects'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:58.862406+00:00'
updated_at: '2026-06-03T18:44:45.363188+00:00'
target_files:
- app/routes/external_projects.py
---

# endpoint بک‌اند بلااستفاده: POST /api/external-projects

## Raw Idea

## 📋 شرح
endpoint `POST /api/external-projects` در `app/routes/external_projects.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `app/database.py` — `external_projects.py` این فایل را import می‌کند
- `app/dependencies/auth.py` — `external_projects.py` این فایل را import می‌کند
- `app/middleware.py` — `external_projects.py` این فایل را import می‌کند
- `app/schemas/external_project_schema.py` — `external_projects.py` این فایل را import می‌کند
...
- [ ] مشخص شد endpoint `POST /api/external-projects` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/api/external-projects` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/external_projects.py`
- `ruff check app/routes/external_projects.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /api/external-projects` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
