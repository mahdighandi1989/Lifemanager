---
task_id: 5ee9978c-22cf-4fba-8aca-07e79a39ab74
title: Frontend API calls use `/api/` prefix but backend routes are mounted at root level
type: bug
priority: high
...
created_at: '2026-06-02T21:10:50.514865+00:00'
updated_at: '2026-06-03T18:43:40.155504+00:00'
target_files:
- app/routes/ai.py
- app/routes/auth.py
...
# Frontend API calls use `/api/` prefix but backend routes are mounted at root level

## Raw Idea

The frontend pages (e.g., `AssetsPage.jsx`, `BudgetPage.jsx`, `DriveFiles.jsx`) use `API_BASE` from `frontend/src/lib/api.js` which defaults to `/api`. However, many backend routers in `app/routes/` are mounted at root level (e.g., `router = APIRouter()` without prefix) and define paths like `/api/assets`, `/api/finance/incomes`, `/api/drive/files`. This works because the paths are hardcoded with `/api/` prefix. But some routers like `app/routes/ai.py` use `prefix="/ai"` and define paths like `/generate`, resulting in `/ai/generate` — which the frontend calls as `/api/ai/generate`. This inconsistency means some endpoints work and others don't depending on how the prefix is applied.
...
- `app/routes/ai.py:42-42` — `router` — This router uses prefix="/ai" without /api, but frontend calls /api/ai/...
  ```python
  router = APIRouter(prefix="/ai", tags=["ai"])
  ```
- `app/routes/auth.py:31-31` — `router` — Same issue: prefix without /api
  ```python
  router = APIRouter(prefix="/auth", tags=["auth"])
  ```
- `app/routes/assets.py:40-40` — `router` — This one hardcodes /api/ in the path — inconsistent approach
  ```python
  @router.get("/api/assets", ...)
  ```
- `frontend/src/lib/api.js:1-20` — `API_BASE` — Frontend always prepends /api to all calls
  ```jsx
...
FastAPI + React with Vite. Frontend uses axios with base URL '/api'.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `app/main.py` (سطر 19) — Router mounting point — shows how routers are included
- `app/routes/ai_profile.py` (سطر 30) — Another router with prefix="/ai"
- `app/routes/ai_stream.py` (سطر 13) — WebSocket router with no prefix
...
- `tests/test_ai_dynamic_analyze.py` — این فایل `ai.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
Affects all frontend-backend communication. Every page that makes API calls will either work or fail depending on whether the backend router uses /api/ prefix or not.
...
- [ ] All backend routers use consistent prefix strategy (either all /api/ or none)
- [ ] Frontend can successfully call all endpoints without 404 errors
- [ ] No duplicate /api/api/ paths in generated URLs
...
1. Standardize all backend routers to either all use explicit `/api/` prefix in paths or all use `prefix="/api"` on the router. Currently there's a mix: some routers have hardcoded `/api/` in paths, others use `prefix` without `/api/`. The frontend consistently uses `/api/` prefix.

## 💡 نمونه‌های قبل/بعد
**Fix for ai.py router**
...
1. All backend routers use consistent prefix strategy (either all /api/ or none) _(verify: static)_
2. Frontend can successfully call all endpoints without 404 errors _(verify: ui_interaction)_
3. No duplicate /api/api/ paths in generated URLs _(verify: manual_only)_
4. All existing tests pass after the change _(verify: backend_test)_

## Task Steps

### Step 1: بررسی و مستندسازی وضعیت فعلی prefix و مسیرهای backend routers
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی کامل تمام فایل‌های router در `app/routes/` برای شناسایی نحوه تعریف prefix و مسیرها است. باید مشخص شود کدام routerها از `prefix` استفاده می‌کنند و کدام مسیرها را hardcoded با `/api/` دارند. همچنین باید مسیرهای واقعی که FastAPI ثبت می‌کند (با استفاده از دستور `app.routes`) مستند شوند. این مرحله فقط بررسی و مستندسازی است و هیچ تغییری در کد ایجاد نمی‌کند. خروجی این مرحله یک گزارش متنی از وضعیت فعلی است.
**Excerpt:**
```
However, many backend routers in `app/routes/` are mounted at root level (e.g., `router = APIRouter()` without prefix) and define paths like `/api/assets`, `/api/finance/incomes`, `/api/drive/files`. This works because the paths are hardcoded with `/api/` prefix. But some routers like `app/routes/ai.py` use `prefix="/ai"` and define paths like `/generate`, resulting in `/ai/generate` — which the frontend calls as `/api/ai/generate`.
```

### Step 2: اصلاح routerهای بدون prefix برای حذف `/api/` hardcoded از مسیرها و افزودن prefix یکپارچه
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح تمام routerهایی است که در `app/routes/` بدون prefix تعریف شده‌اند و مسیرهایشان با `/api/` شروع می‌شود (مانند `/api/assets`, `/api/finance/incomes`, `/api/drive/files`). برای هر کدام، باید `prefix="/api"` به `APIRouter()` اضافه شود و `/api/` از ابتدای مسیرها حذف شود. این کار باعث یکپارچگی با routerهایی می‌شود که از prefix استفاده می‌کنند. نکته حیاتی: باید دقت شود که مسیرها به درستی اصلاح شوند و هیچ مسیر دیگری به اشتباه تغییر نکند.
**Excerpt:**
```
many backend routers in `app/routes/` are mounted at root level (e.g., `router = APIRouter()` without prefix) and define paths like `/api/assets`, `/api/finance/incomes`, `/api/drive/files`. This works because the paths are hardcoded with `/api/` prefix.
```

### Step 3: اصلاح router `app/routes/ai.py` برای استفاده از prefix یکپارچه `/api/ai`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `app/routes/ai.py` است. در حال حاضر این router از `prefix="/ai"` استفاده می‌کند و مسیرهایی مانند `/generate` تعریف می‌کند که منجر به endpoint نهایی `/ai/generate` می‌شود. frontend این endpoint را به صورت `/api/ai/generate` صدا می‌زند که ناسازگار است. در این مرحله، prefix باید به `prefix="/api/ai"` تغییر یابد تا endpoint نهایی `/api/ai/generate` شود. مسیرهای داخلی (مانند `/generate`) تغییری نمی‌کنند.
**Excerpt:**
```
But some routers like `app/routes/ai.py` use `prefix="/ai"` and define paths like `/generate`, resulting in `/ai/generate` — which the frontend calls as `/api/ai/generate`.
```

### Step 4: بررسی و اصلاح سایر routerهای دارای prefix غیراستاندارد در `app/routes/`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی تمام routerهای باقی‌مانده در `app/routes/` است که از prefix استفاده می‌کنند (غیر از `ai.py` که قبلاً اصلاح شد). باید مشخص شود آیا prefix آن‌ها با الگوی `/api/...` مطابقت دارد یا خیر. اگر routerای دارای prefix غیراستاندارد (مانند `/something` بدون `/api/`) باشد، باید اصلاح شود تا prefix آن به `/api/something` تغییر یابد. این مرحله تضمین می‌کند که تمام endpoints backend تحت `/api/` قرار دارند.
**Excerpt:**
```
But some routers like `app/routes/ai.py` use `prefix="/ai"` and define paths like `/generate`, resulting in `/ai/generate` — which the frontend calls as `/api/ai/generate`. This inconsistency means some endpoints work and others don't depending on how the prefix is applied.
```

### Step 5: به‌روزرسانی frontend برای استفاده از `API_BASE` به صورت پویا و حذف hardcoded `/api/` از فراخوانی‌ها
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی فایل‌های frontend (`AssetsPage.jsx`, `BudgetPage.jsx`, `DriveFiles.jsx` و سایر فایل‌های مشابه) است تا مشخص شود آیا فراخوانی‌های API به صورت hardcoded با `/api/` نوشته شده‌اند یا از `API_BASE` استفاده می‌کنند. اگر hardcoded هستند، باید به استفاده از `API_BASE` تغییر یابند. همچنین باید اطمینان حاصل شود که `API_BASE` در `frontend/src/lib/api.js` به درستی مقداردهی شده است (پیش‌فرض `/api`). این مرحله تضمین می‌کند که frontend از یک منبع واحد برای base URL استفاده می‌کند.
**Excerpt:**
```
The frontend pages (e.g., `AssetsPage.jsx`, `BudgetPage.jsx`, `DriveFiles.jsx`) use `API_BASE` from `frontend/src/lib/api.js` which defaults to `/api`.
```

### Step 6: تست یکپارچه‌سازی (integration test) برای تمام endpoints اصلاح‌شده
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن و اجرای تست‌های یکپارچه‌سازی است که تمام endpoints backend را پس از اصلاحات بررسی می‌کنند. تست‌ها باید مطمئن شوند که: (1) endpoints با مسیرهای جدید (تحت `/api/`) در دسترس هستند، (2) endpoints با مسیرهای قدیمی (بدون `/api/`) در دسترس نیستند یا به درستی redirect می‌شوند، (3) frontend با مسیرهای جدید به درستی کار می‌کند. این تست‌ها باید به صورت خودکار قابل اجرا باشند.
**Excerpt:**
```
This inconsistency means some endpoints work and others don't depending on how the prefix is applied.
```
