...
updated_at: '2026-06-09T21:30:10.733621+00:00'
target_files:
- app/main.py
- app/database.py
- app/models.py
...
1. دایرکتوری `app/` در ریشه پروژه ایجاد شود.
2. فایل‌های `app/main.py`, `app/database.py`, `app/models.py`, `app/schemas.py` و `app/routers/items.py` (به عنوان نمونه) ایجاد شوند.
3. فایل `app/main.py` یک نمونه اولیه از برنامه FastAPI را شامل شود.
4. یک endpoint ساده (مثلاً `/health`) در `main.py` تعریف شود که یک پاسخ JSON برگرداند.
