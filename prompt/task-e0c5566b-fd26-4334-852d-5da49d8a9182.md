---
task_id: e0c5566b-fd26-4334-852d-5da49d8a9182
title: فایل‌های کد منبع اصلی برنامه (FastAPI) مفقود هستند.
...
created_at: '2026-06-09T21:30:10.733618+00:00'
updated_at: '2026-06-09T22:31:39.288741+00:00'
target_files:
- app/main.py
- app/database.py
- app/models.py
...
# فایل‌های کد منبع اصلی برنامه (FastAPI) مفقود هستند.

## Raw Idea

لیست فایل‌های ارائه شده شامل فایل‌های محیط مجازی، فایل‌های پیکربندی پایه (مثل `.env.example`) و فایل‌های GitHub Actions است. با این حال، هیچ فایل پایتون (`.py`) که منطق اصلی برنامه Lifemanager را پیاده‌سازی کند (مانند مسیرهای FastAPI، مدل‌های SQLAlchemy، منطق تجاری) وجود ندارد. این نشان می‌دهد که هسته اصلی برنامه هنوز پیاده‌سازی نشده یا در اسکن اولیه گنجانده نشده است. بدون این فایل‌ها، پروژه عملاً یک پوسته خالی است.
...
- `app/main.py:N/A` — `N/A` — فایل اصلی برنامه FastAPI
  ```python
  N/A
  ```
- `app/database.py:N/A` — `N/A` — فایل پیکربندی پایگاه داده
  ```python
  N/A
  ```
- `app/models.py:N/A` — `N/A` — فایل تعریف مدل‌های SQLAlchemy
  ```python
  N/A
  ```
- `app/schemas.py:N/A` — `N/A` — فایل تعریف مدل‌های Pydantic
  ```python
  N/A
  ```
- `app/routers/items.py:N/A` — `N/A` — نمونه فایل برای تعریف مسیرهای API
  ```python
  N/A
  ```
...
- `.env.example` — نیاز به متغیرهای محیطی برای پیکربندی برنامه
- `.dockerignore` — ممکن است نیاز به به‌روزرسانی برای نادیده گرفتن فایل‌های جدید باشد

## 🌐 نقشهٔ وابستگی‌ها
این پروژه بر اساس FastAPI، SQLAlchemy و Pydantic ساخته خواهد شد. ایجاد فایل‌های کد منبع برای استفاده از این فریم‌ورک‌ها ضروری است.
...
- [ ] دایرکتوری `app/` در ریشه پروژه ایجاد شود.
- [ ] فایل‌های `app/main.py`, `app/database.py`, `app/models.py`, `app/schemas.py` و `app/routers/items.py` (به عنوان نمونه) ایجاد شوند.
- [ ] فایل `app/main.py` یک نمونه اولیه از برنامه FastAPI را شامل شود.
- [ ] یک endpoint ساده (مثلاً `/health`) در `main.py` تعریف شود که یک پاسخ JSON برگرداند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ساختار دایرکتوری پایه برای یک برنامه FastAPI را ایجاد کنید. این شامل ایجاد یک دایرکتوری `app/` حاوی `main.py` برای نمونه برنامه FastAPI، `database.py` برای اتصال به پایگاه داده، `models.py` برای تعریف مدل‌های SQLAlchemy، `schemas.py` برای مدل‌های Pydantic و یک دایرکتوری `routers/` برای تعریف endpointهای API خواهد بود.
...
هیچ، زیرا کد جدیدی ایجاد می‌شود.
...
1. دایرکتوری `app/` در ریشه پروژه ایجاد شود. _(verify: static)_
2. فایل‌های `app/main.py`, `app/database.py`, `app/models.py`, `app/schemas.py` و `app/routers/items.py` (به عنوان نمونه) ایجاد شوند. _(verify: static)_
3. فایل `app/main.py` یک نمونه اولیه از برنامه FastAPI را شامل شود. _(verify: static)_
4. یک endpoint ساده (مثلاً `/health`) در `main.py` تعریف شود که یک پاسخ JSON برگرداند. _(verify: api_response)_

## Task Steps

### Step 1: پیاده‌سازی مدل‌های SQLAlchemy برای Lifemanager
**Status:** `pending` (0%)
**Scope:** این مرحله بر تعریف ساختار داده‌های اصلی برنامه Lifemanager با استفاده از ORM SQLAlchemy تمرکز دارد. شامل ایجاد فایل‌های پایتون برای تعریف مدل‌ها، مشخص کردن جداول، ستون‌ها، روابط بین موجودیت‌ها (مانند کاربر، وظیفه، هدف) و انواع داده‌های مربوطه است. هدف این است که لایه داده‌ای بنیادی برای ذخیره‌سازی پایدار اطلاعات برنامه ایجاد شود. این مرحله شامل پیاده‌سازی مهاجرت‌های دیتابیس (migrations) یا ایجاد endpointهای API برای تعامل با این مدل‌ها نیست.
**Excerpt:**
```
هیچ فایل پایتون (`.py`) که منطق اصلی برنامه Lifemanager را پیاده‌سازی کند (مانند مسیرهای FastAPI، مدل‌های SQLAlchemy، منطق تجاری) وجود ندارد. این نشان می‌دهد که هسته اصلی برنامه هنوز پیاده‌سازی نشده یا در اسکن اولیه گنجانده نشده است. بدون این فایل‌ها، پروژه عملاً یک پوسته خالی است.
```

### Step 2: پیاده‌سازی منطق تجاری (Business Logic) اصلی Lifemanager
**Status:** `pending` (0%)
**Scope:** این مرحله به پیاده‌سازی قوانین و عملیات تجاری اصلی برنامه Lifemanager اختصاص دارد. شامل توسعه توابع و کلاس‌هایی است که رفتارهای خاص برنامه را کپسوله می‌کنند، مانند ایجاد یک وظیفه جدید، به‌روزرسانی وضعیت یک هدف، یا مدیریت پروفایل‌های کاربری. این منطق با مدل‌های SQLAlchemy تعریف شده در مرحله قبل تعامل خواهد داشت، اما عملیات را مستقیماً از طریق API در معرض نمایش قرار نمی‌دهد. هدف، جداسازی منطق اصلی از لایه‌های دسترسی به داده و رابط کاربری است.
**Excerpt:**
```
هیچ فایل پایتون (`.py`) که منطق اصلی برنامه Lifemanager را پیاده‌سازی کند (مانند مسیرهای FastAPI، مدل‌های SQLAlchemy، منطق تجاری) وجود ندارد. این نشان می‌دهد که هسته اصلی برنامه هنوز پیاده‌سازی نشده یا در اسکن اولیه گنجانده نشده است. بدون این فایل‌ها، پروژه عملاً یک پوسته خالی است.
```

### Step 3: پیاده‌سازی مسیرهای FastAPI برای Lifemanager
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد endpointهای API با استفاده از فریم‌ورک FastAPI تمرکز دارد که قابلیت‌های پیاده‌سازی شده در لایه منطق تجاری را در معرض نمایش قرار می‌دهد. شامل تعریف متدهای HTTP (GET, POST, PUT, DELETE)، مدل‌های درخواست و پاسخ (با استفاده از Pydantic)، و اتصال این endpointها به توابع اصلی منطق تجاری است. این مرحله برنامه را از طریق یک API وب قابل دسترسی می‌کند و امکان تعامل با آن را برای کلاینت‌های خارجی فراهم می‌آورد.
**Excerpt:**
```
هیچ فایل پایتون (`.py`) که منطق اصلی برنامه Lifemanager را پیاده‌سازی کند (مانند مسیرهای FastAPI، مدل‌های SQLAlchemy، منطق تجاری) وجود ندارد. این نشان می‌دهد که هسته اصلی برنامه هنوز پیاده‌سازی نشده یا در اسکن اولیه گنجانده نشده است. بدون این فایل‌ها، پروژه عملاً یک پوسته خالی است.
```
