---
task_id: 86a8864a-4069-4424-ba34-8c9c5defd81f
title: یکپارچه‌سازی گزارش‌دهی پوشش تست در پایپ‌لاین CI/CD
type: refactor
priority: medium
...
created_at: '2026-06-09T21:32:00.673684+00:00'
updated_at: '2026-06-09T22:34:31.428250+00:00'
target_files:
- .github/workflows/claude-auto-task.yml
---

# یکپارچه‌سازی گزارش‌دهی پوشش تست در پایپ‌لاین CI/CD

## Raw Idea

پروژه از `pytest` برای تست و `coverage.py` برای اندازه‌گیری پوشش تست استفاده می‌کند، همانطور که با حضور آن‌ها در محیط مجازی مشخص است. با این حال، بدون یکپارچه‌سازی صریح گزارش‌دهی پوشش در پایپ‌لاین CI/CD (مانند `.github/workflows/claude-auto-task.yml`)، خطر قابل توجهی وجود دارد که تغییرات کد ممکن است به طور ناخواسته پوشش تست را بدون اطلاع کاهش دهند. این می‌تواند به مرور زمان منجر به یک برنامه کمتر قوی شود، با معرفی باگ‌های جدید در مناطق تست نشده.
...
- `.github/workflows/claude-auto-task.yml:N/A` — `N/A` — فایل گردش کار موجود است، اما مراحل خاص برای گزارش‌دهی پوشش بر اساس ماهیت کلی این فاز یکپارچگی cross-cutting فرض می‌شود که از دست رفته‌اند. خطوط دقیق برای اصلاح به ساختار فعلی گردش کار بستگی دارد.
  ```
  N/A
  ```
...
پایتون، Pytest، Coverage.py، GitHub Actions.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.venv_t/bin/pytest` — فایل اجرایی Pytest، نشان‌دهنده چارچوب تست مورد استفاده.
- `.venv_t/bin/coverage` — فایل اجرایی Coverage.py، نشان‌دهنده ابزار اندازه‌گیری پوشش کد.
- `.venv_t/lib/python3.12/site-packages/_pytest/__init__.py` — فایل‌های اصلی کتابخانه Pytest.
- `.venv_t/lib/python3.12/site-packages/coverage-7.5.4.dist-info/METADATA` — فایل‌های اصلی کتابخانه Coverage.py.

## 🌐 نقشهٔ وابستگی‌ها
بر کیفیت کلی کد، قابلیت اطمینان و اعتماد توسعه‌دهنده در انجام تغییرات تأثیر می‌گذارد. یک مجموعه تست قوی با پوشش خوب، خطر رگرسیون را کاهش می‌دهد و تلاش‌های بازسازی را ساده می‌کند.
...
- [ ] پایپ‌لاین CI/CD، `pytest` را با `coverage.py` در هر push و pull request اجرا می‌کند.
- [ ] در صورت کاهش پوشش تست کلی به زیر یک آستانه پیکربندی شده (مانند 80%)، بیلد با شکست مواجه می‌شود.
- [ ] گزارش‌های پوشش دقیق (مانند HTML، XML) تولید و قابل دسترسی هستند (مانند در خلاصه GitHub Actions یا از طریق سرویسی مانند Codecov).
- [ ] درخواست‌های کشش، تغییرات پوشش را به عنوان بخشی از بررسی‌های وضعیت خود نشان می‌دهند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. گردش کار GitHub Actions موجود (`.github/workflows/claude-auto-task.yml`) را برای شامل کردن مراحل اجرای `pytest` با `coverage.py` و تولید یک گزارش جامع اصلاح کنید. گردش کار را طوری پیکربندی کنید که در صورت کاهش پوشش تست کلی به زیر یک آستانه از پیش تعریف شده و قابل قبول (مانند 80%)، با شکست مواجه شود. علاوه بر این، ادغام با یک سرویس پوشش کد مانند Codecov یا Coveralls را برای ارائه ردیابی تاریخی، گزارش‌های دقیق و بررسی وضعیت درخواست‌های کشش (Pull Request)، که تغییرات پوشش را در طول بررسی کد قابل مشاهده می‌کند، در نظر بگیرید.

## 💡 نمونه‌های قبل/بعد
**افزودن مراحل پوشش تست به گردش کار GitHub Actions**
...
- name: Run tests with coverage
        run: |
          pip install pytest-cov # اطمینان از نصب pytest-cov
          pytest --cov=. --cov-report=xml --cov-report=term-missing
          coverage report --fail-under=80 # شکست CI در صورت کاهش پوشش به زیر 80%
      - name: Upload coverage to Codecov
        uses: codecov/codecov-action@v4
        with:
          token: ${{ secrets.CODECOV_TOKEN }} # نیاز به secret CODECOV_TOKEN
```
...
## 🧪 دستورات اعتبارسنجی
- `یک اجرای گردش کار GitHub Actions را با push کردن یک commit یا باز کردن یک pull request فعال کنید.`
- `به صورت محلی `pytest --cov=. --cov-report=term-missing` را اجرا کنید تا تولید پوشش را تأیید کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
ممکن است نیاز به تنظیم تست‌های موجود یا افزودن تست‌های جدید برای رسیدن به آستانه‌های پوشش داشته باشد؛ احتمال خرابی پایپ‌لاین CI/CD در صورت تنظیم اولیه آستانه پوشش بیش از حد بالا یا پیکربندی نادرست.
...
1. پایپ‌لاین CI/CD، `pytest` را با `coverage.py` در هر push و pull request اجرا می‌کند. _(verify: static)_
2. در صورت کاهش پوشش تست کلی به زیر یک آستانه پیکربندی شده (مانند 80%)، بیلد با شکست مواجه می‌شود. _(verify: static)_
3. گزارش‌های پوشش دقیق (مانند HTML، XML) تولید و قابل دسترسی هستند (مانند در خلاصه GitHub Actions یا از طریق سرویسی مانند Codecov). _(verify: ui_interaction)_
4. درخواست‌های کشش، تغییرات پوشش را به عنوان بخشی از بررسی‌های وضعیت خود نشان می‌دهند. _(verify: ui_interaction)_
