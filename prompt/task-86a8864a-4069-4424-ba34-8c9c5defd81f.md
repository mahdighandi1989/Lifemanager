...
updated_at: '2026-06-09T21:32:00.673688+00:00'
target_files:
...
1. پایپ‌لاین CI/CD، `pytest` را با `coverage.py` در هر push و pull request اجرا می‌کند.
2. در صورت کاهش پوشش تست کلی به زیر یک آستانه پیکربندی شده (مانند 80%)، بیلد با شکست مواجه می‌شود.
3. گزارش‌های پوشش دقیق (مانند HTML، XML) تولید و قابل دسترسی هستند (مانند در خلاصه GitHub Actions یا از طریق سرویسی مانند Codecov).
4. درخواست‌های کشش، تغییرات پوشش را به عنوان بخشی از بررسی‌های وضعیت خود نشان می‌دهند.
