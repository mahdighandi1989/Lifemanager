---
task_id: 78d90bc3-1616-4b69-b2f0-6429c9a4f1ab
title: 'دکمه‌ی UI بدون handler: testProvider(p.id)} className="text-xs text-indigo-600 hover:underline">تست'
type: bug_fix
priority: medium
...
created_at: '2026-06-02T21:12:47.076946+00:00'
updated_at: '2026-06-02T21:48:51.814623+00:00'
target_files:
- frontend/src/pages/AISettings.jsx
---

# دکمه‌ی UI بدون handler: testProvider(p.id)} className="text-xs text-indigo-600 hover:underline">تست

## Raw Idea

## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/pages/AISettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `testProvider(p.id)} className="text-xs text-indigo-600 hover:underline">تست`
- فایل: `frontend/src/pages/AISettings.jsx`
...
---
[scan #2 at 2026-06-02T21:12:47.341526+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/pages/AISettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `deleteProvider(p.id)} className="text-xs text-red-600 hover:underline">حذ
---
[scan #3 at 2026-06-02T21:12:47.466724+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/pages/ListDetail.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setEditing(true)}
              className="text-xs text-indigo-600 hover:
---
[scan #4 at 2026-06-02T21:12:47.603787+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/pages/ListDetail.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setShowFull((v) => !v)}
                  className="text-xs text-indigo-
---
[scan #5 at 2026-06-02T21:12:47.719890+00:00]
## 📋 شرح
...
- `frontend/src/context/AuthContext.jsx` — `AISettings.jsx` این فایل را import می‌کند
- `frontend/src/components/AIFeedbackWidget.jsx` — `AISettings.jsx` این فایل را import می‌کند
- `frontend/src/pages/__tests__/AISettings.test.jsx` — این فایل `AISettings.jsx` را import می‌کند (caller)
- `frontend/src/App.jsx` — این فایل `AISettings.jsx` را import می‌کند (caller)
...
- [ ] git blame مشخص می‌کند چرا این دکمه `testProvider(p.id)} className="text-xs text-indigo-600 hover:underline">تست` فاقد handler است
...
1. git blame مشخص می‌کند چرا این دکمه `testProvider(p.id)} className="text-xs text-indigo-600 hover:underline">تست` فاقد handler است _(verify: manual_only)_
