---
task_id: f055b9e7-d62b-4940-b1e4-368bbf909354
title: 'endpoint بک‌اند بلااستفاده: GET /api/assets/external-drives'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:57.675077+00:00'
updated_at: '2026-06-02T21:51:32.282741+00:00'
target_files:
- app/routes/assets.py
---

# endpoint بک‌اند بلااستفاده: GET /api/assets/external-drives

## Raw Idea

## 📋 شرح
endpoint `GET /api/assets/external-drives` در `app/routes/assets.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/assets/external-drives`
...
---
[scan #2 at 2026-06-02T21:12:57.902305+00:00]
## 📋 شرح
endpoint `POST /api/assets/sync` در `app/routes/assets.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
## 🤔 
---
[scan #3 at 2026-06-02T21:12:58.016705+00:00]
## 📋 شرح
endpoint `WEBSOCKET /api/assets/scan-status` در `app/routes/assets.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `WEBSOCKET`
- path: `/api/assets/scan-status`
- فایل: `app/routes/assets.py`
- علت: no frontend fetch + no re
---
[scan #4 at 2026-06-02T21:12:59.096556+00:00]
## 📋 شرح
endpoint `GET /api/external-projects` در `app/routes/external_projects.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/external-projects`
- فایل: `app/routes/external_projects.py`
...
- `app/database.py` — `assets.py` این فایل را import می‌کند
- `app/dependencies/auth.py` — `assets.py` این فایل را import می‌کند
- `app/middleware.py` — `assets.py` این فایل را import می‌کند
- `app/models/task.py` — `assets.py` این فایل را import می‌کند
...
- [ ] مشخص شد endpoint `GET /api/assets/external-drives` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/api/assets/external-drives` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/assets.py`
- `ruff check app/routes/assets.py`
- `pytest -x`
...
1. مشخص شد endpoint `GET /api/assets/external-drives` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
