---
task_id: 1f8e1a1e-30b4-4ef2-a97c-3f8abb5400c1
title: exception swallowed در startup_event بدون لاگ کافی برای خطاهای بحرانی
type: bug
priority: high
...
created_at: '2026-06-02T21:10:59.144021+00:00'
updated_at: '2026-06-03T18:43:47.962951+00:00'
target_files:
- app/main.py
- app/main.py
- app/main.py
...
# exception swallowed در startup_event بدون لاگ کافی برای خطاهای بحرانی

## Raw Idea

در app/main.py خطوط 220-222، خطای اتصال دیتابیس در startup_event با logger.critical لاگ می‌شود اما برنامه بدون دیتابیس ادامه می‌دهد. این رفتار برای health check و تست‌ها طراحی شده، اما مشکل اینجاست که در خطوط 228-233، 244-252، 267-274، 283-290، 327-334، 339-345، 365-372، 383-390، و 400-402، تمام exceptionهای ALTER TABLE و migrationها با logger.debug یا logger.error ساده swallow می‌شوند بدون اینکه به کاربر نهایی (API client) هیچ سیگنالی داده شود. در شرایطی که migration حیاتی (مثلاً افزودن ستون user_id به tasks) fail کند، endpointهای بعدی با 500 کرش می‌کنند اما لاگ در سطح debug پنهان می‌ماند.
...
- `app/main.py:220-222` — `startup_event` — خطای بحرانی دیتابیس — acceptable
  ```python
  ...
  ```
- `app/main.py:232-233` — `run_migrations_if_enabled` — migration hook error — باید warning باشد
  ```python
  ...
  ```
- `app/main.py:251-252` — `ALTER COLUMN user_id DROP NOT NULL` — بحرانی‌ترین migration — debug level پنهان است
  ```python
  except Exception as exc:
              logger.debug("skip user_id NOT NULL relaxation on %s: %s", table, exc)
  ```
- `app/main.py:273-274` — `ADD COLUMN notifications` — باید warning باشد
  ```python
  except Exception as exc:
              logger.debug("skip notifications.%s migration: %s", col_name, exc)
  ```
- `app/main.py:289-290` — `ADD COLUMN users profile` — باید warning باشد
  ```python
  except Exception as exc:
              logger.debug("skip users.%s migration: %s", col_name, exc)
  ```
- `app/main.py:333-334` — `ADD COLUMN profiling columns` — باید warning باشد
  ```python
  except Exception as exc:
              logger.debug("skip %s.%s migration: %s", table, col_name, exc)
  ```
- `app/main.py:344-345` — `ALTER COLUMN ai_assessments.person_id DROP NOT NULL` — باید warning باشد
  ```python
  except Exception as exc:
          logger.debug("skip ai_assessments.person_id NOT NULL relaxation: %s", exc)
  ```
- `app/main.py:371-372` — `ADD COLUMN tasks planning` — باید
  ```python
  except Exception as exc:
...
1. تمام swallow شده‌های migration در startup_event را به logger.warning ارتقا دهید تا در لاگ‌های پیش‌فرض visible باشند. برای migrationهای بحرانی (مثل user_id NOT NULL relaxation) یک health check endpoint اضافه کنید که وضعیت schema را گزارش دهد.
...
### Step 1: ارتقای سطح لاگ migrationهای حیاتی از debug به warning/error و اضافه کردن exception propagation برای migrationهای شکست‌خورده
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تغییر لاگ‌های exception در خطوط 228-233، 244-252، 267-274، 283-290، 327-334، 339-345، 365-372، 383-390، و 400-402 از logger.debug یا logger.error ساده به logger.warning یا logger.critical است. همچنین شامل اضافه کردن raise یا propagation exception برای migrationهایی است که failure آن‌ها باعث crash endpointهای بعدی می‌شود (مثلاً افزودن ستون user_id به tasks). خارج از scope: تغییر startup_event اصلی (خطوط 220-222)، تغییر health check logic، تغییر تست‌ها، تغییر رفتار دیتابیس در حالت disconnected. نکته حیاتی: migrationهای غیرحیاتی که failure آن‌ها بی‌خطر است (مثلاً افزودن index غیرضروری) باید فقط لاگ warning داشته باشند بدون raise. تشخیص حیاتی/غیرحیاتی بودن migration باید بر اساس تحلیل dependency انجام شود.
**Excerpt:**
```
در خطوط 228-233، 244-252، 267-274، 283-290، 327-334، 339-345، 365-372، 383-390، و 400-402، تمام exceptionهای ALTER TABLE و migrationها با logger.debug یا logger.error ساده swallow می‌شوند بدون اینکه به کاربر نهایی (API client) هیچ سیگنالی داده شود. در شرایطی که migration حیاتی (مثلاً افزودن ستون user_id به tasks) fail کند، endpointهای بعدی با 500 کرش می‌کنند اما لاگ در سطح debug پنهان می‌ماند.
```
