---
task_id: e3d3b3f3-4aa6-4da4-8170-fd67bc57622f
title: 'route فرانت‌اند بلااستفاده: /AISettings'
type: cleanup
priority: medium
...
created_at: '2026-06-02T21:12:49.949686+00:00'
updated_at: '2026-06-02T21:12:53.359104+00:00'
---

# route فرانت‌اند بلااستفاده: /AISettings

## Raw Idea

## 📋 شرح
route `/AISettings` در فایل `` تعریف شده ولی هیچ `Link`، `router.push`، یا redirect در کدبیس به آن اشاره نمی‌کند.
...
---
[scan #2 at 2026-06-02T21:12:50.167890+00:00]
## 📋 شرح
route `/AssetsPag` در فایل `` تعریف شده ولی هیچ `Link`، `router.push`، یا redirect در کدبیس به آن اشاره نمی‌کند.

## 🔍 جزئیات
- route path: `/AssetsPag`
- فایل: ``
- علت: route exists in app router but no Link/router.push/nav-config references it
...
---
[scan #3 at 2026-06-02T21:12:50.281501+00:00]
## 📋 شرح
route `/BudgetPag` در فایل `` تعریف شده ولی هیچ `Link`، `router.push`، یا redirect در کدبیس به آن اشاره نمی‌کند.

## 🔍 جزئیات
- route path: `/BudgetPag`
- فایل: ``
- علت: route exists in app router but no Link/router.push/nav-config references it
...
route orphan یعنی صفحه‌ای 
---
[scan #4 at 2026-06-02T21:12:50.392129+00:00]
## 📋 شرح
route `/CareerPlanningPag` در فایل `` تعریف شده ولی هیچ `Link`، `router.push`، یا redirect در کدبیس به آن اشاره نمی‌کند.

## 🔍 جزئیات
- route path: `/CareerPlanningPag`
- فایل: ``
- علت: route exists in app router but no Link/router.push/nav-config references it
...
route orph
---
[scan #5 at 2026-06-02T21:12:50.505838+00:00]
## 📋 شرح
...
## 🤔 چرا مهم است
route orphan یعنی صفحه‌ای که
---
[scan #6 at 2026-06-02T21:12:50.618938+00:00]
## 📋 شرح
route `/DriveFiles` در فایل `` تعریف شده ولی هیچ `Link`، `router.push`، یا redirect در کدبیس به آن اشاره نمی‌کند.

## 🔍 جزئیات
- route path: `/DriveFiles`
- فایل: ``
- علت: route exists in app router but no Link/router.push/nav-config references it
...
- [ ] تأیید شد که `/AISettings` orphan است (هیچ Link/router.push اشاره نمی‌کند)
...
1. گام ۱: بررسی کن آیا این route از طریق dynamic URL (مثل sidebar config) اشاره می‌شود — grep روی `/AISettings` در کل کدبیس بزن.
...
1. تأیید شد که `/AISettings` orphan است (هیچ Link/router.push اشاره نمی‌کند)
2. یا navigation link اضافه شد، یا route حذف/redirect شد
3. تست navigation: کاربر بتواند به این صفحه (یا destination) برسد
