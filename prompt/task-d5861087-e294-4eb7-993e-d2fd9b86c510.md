---
task_id: d5861087-e294-4eb7-993e-d2fd9b86c510
title: متغیر محیطی GOOGLE_SHEETS_REFRESH_TOKEN در .env.example تعریف نشده است
type: bug
priority: medium
...
created_at: '2026-06-02T21:12:46.870611+00:00'
updated_at: '2026-06-02T21:48:24.397836+00:00'
target_files:
- .env.example
---

# متغیر محیطی GOOGLE_SHEETS_REFRESH_TOKEN در .env.example تعریف نشده است

## Raw Idea

در app/routes/drive.py (خط 97-107)، تابع record_index_entry از sheets_service برای ثبت فایل‌ها در Google Sheet استفاده می‌کند. این تابع به متغیر محیطی GOOGLE_SHEETS_REFRESH_TOKEN نیاز دارد، اما این متغیر در .env.example تعریف نشده است. این باعث می‌شود که توسعه‌دهندگان جدید از وجود این قابلیت مطلع نشوند و در صورت عدم تنظیم، خطاهای بی‌صدا (silent errors) رخ دهد.
...
- `.env.example:1-50` — `GOOGLE_SHEETS_REFRESH_TOKEN`
  ```
  # OAuth refresh token for the central
  ```
...
1. متغیر GOOGLE_SHEETS_REFRESH_TOKEN را به .env.example اضافه کنید با یک کامنت توضیحی.
