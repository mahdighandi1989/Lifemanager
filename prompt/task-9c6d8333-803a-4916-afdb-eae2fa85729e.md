---
task_id: 9c6d8333-803a-4916-afdb-eae2fa85729e
title: Create `requirements.txt` for explicit dependency management
type: refactor
priority: high
...
created_at: '2026-06-09T21:30:59.135324+00:00'
updated_at: '2026-06-09T22:32:28.282057+00:00'
target_files:
- .
---

# Create `requirements.txt` for explicit dependency management

## Raw Idea

The project's virtual environment (`.venv_t`) contains numerous Python packages, indicating a Python application. However, there is no `requirements.txt`, `pyproject.toml`, or `Pipfile` present in the repository root. This makes it impossible to reproduce the exact development environment, manage dependencies effectively, or ensure consistent deployments. Without an explicit dependency list, other developers or CI/CD pipelines cannot reliably install the correct versions of packages, leading to potential runtime inconsistencies.
...
- `.:N/A` — `N/A` — The `requirements.txt` file needs to be created in the root directory of the repository.
  ```
...
Python 3.12, FastAPI, SQLAlchemy, Alembic, Celery, Uvicorn.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.venv_t/lib/python3.12/site-packages/` (سطر N/A) — This directory contains all installed packages from which `requirements.txt` should be generated.

## 🌐 نقشهٔ وابستگی‌ها
This finding addresses the fundamental dependency management for the entire Python application, ensuring reproducibility and consistency across development and deployment environments.
...
- [ ] A `requirements.txt` file exists in the repository root.
- [ ] The `requirements.txt` file lists all direct project dependencies with pinned versions (e.g., `package==X.Y.Z`).
- [ ] Running `pip install -r requirements.txt` in a fresh virtual environment successfully installs all necessary packages without errors.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Generate a `requirements.txt` file from the currently installed packages in the virtual environment. This file should then be committed to the repository to provide a clear and reproducible list of project dependencies. It is recommended to pin exact versions for all direct dependencies.

## 💡 نمونه‌های قبل/بعد
**Before (no dependency file)**
...
**After (example `requirements.txt`)**
...
- `python -m venv new_venv && source new_venv/bin/activate && pip install -r requirements.txt`

## ⚠️ ریسک‌ها و موارد احتیاط
None, this is a foundational improvement for project stability.
...
1. A `requirements.txt` file exists in the repository root. _(verify: static)_
2. The `requirements.txt` file lists all direct project dependencies with pinned versions (e.g., `package==X.Y.Z`). _(verify: static)_
3. Running `pip install -r requirements.txt` in a fresh virtual environment successfully installs all necessary packages without errors. _(verify: backend_test)_
