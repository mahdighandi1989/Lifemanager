---
task_id: 5288522f-4277-4aff-8a06-c1f78fbe6f17
title: 'endpoint بک‌اند بلااستفاده: POST /api/context/physiological'
type: audit
priority: medium
...
created_at: '2026-06-02T21:12:58.017015+00:00'
updated_at: '2026-06-02T21:51:56.617405+00:00'
target_files:
- app/routes/context.py
---

# endpoint بک‌اند بلااستفاده: POST /api/context/physiological

## Raw Idea

## 📋 شرح
endpoint `POST /api/context/physiological` در `app/routes/context.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/api/context/physiological`
- فایل: `app/routes/context.py`
...
---
[scan #2 at 2026-06-02T21:12:58.269478+00:00]
## 📋 شرح
endpoint `POST /api/context/voice` در `app/routes/context.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/api/context/voice`
- فایل: `app/routes/context.py`
- علت: no frontend fetch + no recent call in logs

---
[scan #3 at 2026-06-02T21:12:58.383535+00:00]
## 📋 شرح
endpoint `PATCH /api/recommendations/{rec_id}/read` در `app/routes/context.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `PATCH`
- path: `/api/recommendations/{rec_id}/read`
- فایل: `app/routes/context.py`
- علت: no fronte
---
[scan #4 at 2026-06-02T21:12:58.497355+00:00]
## 📋 شرح
endpoint `GET /api/context/recommendations` در `app/routes/context.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/api/context/recommendations`
- فایل: `app/routes/context.py`
- علت: no frontend fetch + no re
---
[scan #5 at 2026-06-02T21:12:58.621135+00:00]
## 📋 شرح
endpoint `GET /api/recommendations` در `app/routes/context.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `app/database.py` — `context.py` این فایل را import می‌کند
- `app/dependencies/auth.py` — `context.py` این فایل را import می‌کند
- `app/middleware.py` — `context.py` این فایل را import می‌کند
- `app/services/context_engine/__init__.py` — `context.py` این فایل را import می‌کند
- `tests/test_context_signals_2165524b.py` — این فایل `context.py` را import می‌کند (caller)
...
- [ ] مشخص شد endpoint `POST /api/context/physiological` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/api/context/physiological` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/routes/context.py`
- `ruff check app/routes/context.py`
- `pytest -x`
...
1. مشخص شد endpoint `POST /api/context/physiological` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
