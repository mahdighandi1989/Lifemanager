---
task_id: 007b0ba7-3218-4eda-b13d-d1e6433b980f
...
created_at: '2026-06-02T21:12:46.563003+00:00'
updated_at: '2026-06-02T21:47:44.112679+00:00'
target_files:
...
The `message` parameter is truncated in the provided snippet, making the notification incomplete. For a critical system event like `verify_failed`, an action link to relevant logs or webhook configuration settings is crucial for an administrator to investigate and resolve the issue. The `user_id=0` suggests a system-level notification, reinforcing the need for administrative actions.

🛠 پیشنهاد: Title: تأیید امضای وب‌هوک ناموفق. Message: یک درخواست وب‌هوک با امضای نامعتبر دریافت شد. لطفاً تنظیمات وب‌هوک را بررسی کنید. Action: [مشاهده لاگ‌های وب‌هوک] [تنظیمات وب‌هوک]
...
- `app/database.py` — `webhook.py` این فایل را import می‌کند
- `app/models/webhook_event.py` — `webhook.py` این فایل را import می‌کند
- `app/services/integration_service.py` — `webhook.py` این فایل را import می‌کند
- `app/services/notification_service.py` — `webhook.py` این فایل را import می‌کند
- `app/routes/__init__.py` — این فایل `webhook.py` را import می‌کند (caller)
...
- `ruff check app/routes/webhook.py`
- `pytest -x`
