---
task_id: 015fe1f8-30bf-4e81-be87-0f55850bea3f
title: عدم وجود اسکن خودکار آسیب‌پذیری وابستگی‌ها در CI/CD
type: security
priority: high
...
created_at: '2026-06-09T21:30:58.923525+00:00'
updated_at: '2026-06-09T22:32:13.900460+00:00'
target_files:
- .github/workflows/claude-auto-task.yml
---

# عدم وجود اسکن خودکار آسیب‌پذیری وابستگی‌ها در CI/CD

## Raw Idea

پروژه از GitHub Actions استفاده می‌کند، همانطور که توسط `.github/workflows/claude-auto-task.yml` مشهود است. با این حال، هیچ نشانه‌ای از یک فرآیند خودکار برای اسکن وابستگی‌های پروژه برای آسیب‌پذیری‌های امنیتی شناخته شده (CVEs) وجود ندارد. بدون چنین مکانیزمی، پروژه در معرض خطر ناخواسته گنجاندن و استقرار کتابخانه‌های دارای نقص‌های امنیتی حیاتی است که می‌تواند منجر به نقض داده‌ها، به خطر افتادن سرویس یا سایر حوادث امنیتی شود. بررسی‌های دستی ناکافی و مستعد خطای انسانی هستند.
...
- `.github/workflows/claude-auto-task.yml` — یک مرحله یا شغل جدید به این ورک‌فلو برای انجام اسکن وابستگی‌ها اضافه کنید.
...
GitHub Actions، پایتون، `pip-audit` (یا ابزارهای مشابه).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `requirements.txt` — اسکنر از این فایل برای شناسایی وابستگی‌ها استفاده خواهد کرد.

## 🌐 نقشهٔ وابستگی‌ها
این یک مرحله جدید CI/CD است که فایل `requirements.txt` را مصرف کرده و وضعیت امنیتی تمام وابستگی‌های پایتون را گزارش می‌کند.
...
- [ ] یک ورک‌فلو GitHub Actions یک اسکنر آسیب‌پذیری وابستگی‌ها را اجرا می‌کند.
- [ ] اسکنر با موفقیت هرگونه آسیب‌پذیری شناخته شده در وابستگی‌های پروژه را شناسایی می‌کند.
- [ ] خط لوله CI/CD در صورت معرفی آسیب‌پذیری‌های با شدت بالا، شکست می‌خورد.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک ابزار اسکن آسیب‌پذیری وابستگی‌ها (مانند `pip-audit`، Snyk، Trivy یا Dependabot گیت‌هاب) را در ورک‌فلو GitHub Actions موجود ادغام کنید یا یک ورک‌فلو امنیتی اختصاصی جدید ایجاد کنید. این مرحله باید در هر push یا pull request اجرا شود تا آسیب‌پذیری‌ها را در مراحل اولیه چرخه توسعه شناسایی و گزارش کند. ورک‌فلو باید در صورت شناسایی آسیب‌پذیری‌های با شدت بالا، شکست بخورد تا یک خط لوله توسعه امن را اعمال کند.

## 💡 نمونه‌های قبل/بعد
**نمونه مرحله GitHub Actions برای `pip-audit`**

_قبل:_
```
(مرحله اسکن امنیتی وجود ندارد)
```
...
## 🧪 دستورات اعتبارسنجی
- `با افزودن یک پکیج آسیب‌پذیر شناخته شده به `requirements.txt` (مثلاً یک نسخه قدیمی از `requests`) و push کردن، ورک‌فلو را فعال کنید و شکست ورک‌فلو و گزارش امنیتی را مشاهده کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال مثبت کاذب که نیاز به بررسی دستی دارد؛ افزایش زمان اجرای CI/CD.
...
1. یک ورک‌فلو GitHub Actions یک اسکنر آسیب‌پذیری وابستگی‌ها را اجرا می‌کند. _(verify: static)_
2. اسکنر با موفقیت هرگونه آسیب‌پذیری شناخته شده در وابستگی‌های پروژه را شناسایی می‌کند. _(verify: backend_test)_
3. خط لوله CI/CD در صورت معرفی آسیب‌پذیری‌های با شدت بالا، شکست می‌خورد. _(verify: backend_test)_
