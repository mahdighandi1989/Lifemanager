---
task_id: db70deaa-5ca7-451f-84c5-5e11e7942f58
...
created_at: '2026-06-02T21:11:01.585046+00:00'
updated_at: '2026-06-03T18:43:56.047049+00:00'
target_files:
...
The `status` column is defined as `String(32), default='pending', nullable=True`. For a critical delivery tracking field that powers 'delivery-tracking and race-resolution features', `nullable=True` is a significant logical flaw. A notification should always have a defined status (e.g., 'pending', 'processing', 'sent', 'failed'). Allowing `None` breaks the state machine logic and can lead to ambig

📁 file: app/models/notification.py (line 40)
...
- `app/database.py` — `notification.py` این فایل را import می‌کند
- `app/services/notification_service.py` — این فایل `notification.py` را import می‌کند (caller)
- `app/models/__init__.py` — این فایل `notification.py` را import می‌کند (caller)
- `app/services/ai/ai_data_access_service.py` — این فایل `notification.py` را import می‌کند (caller)
- `tests/test_notification_service.py` — این فایل `notification.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/models/notification.py`
- `ruff check app/models/notification.py`
- `pytest -x`
