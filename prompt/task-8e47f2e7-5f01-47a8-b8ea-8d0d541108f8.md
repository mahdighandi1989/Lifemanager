---
task_id: 8e47f2e7-5f01-47a8-b8ea-8d0d541108f8
title: استانداردسازی و اعتبارسنجی پیکربندی متغیرهای محیطی
type: refactor
priority: medium
...
created_at: '2026-06-09T21:32:00.557242+00:00'
updated_at: '2026-06-09T22:34:11.295296+00:00'
target_files:
- .env.example
---

# استانداردسازی و اعتبارسنجی پیکربندی متغیرهای محیطی

## Raw Idea

حضور فایل `.env.example` نشان می‌دهد که پروژه از متغیرهای محیطی برای پیکربندی استفاده می‌کند. بدون یک مکانیسم واضح و متمرکز برای بارگذاری و اعتبارسنجی این متغیرها، خطر رفتار ناسازگار در محیط‌های مختلف (توسعه، تست، تولید)، از دست دادن متغیرهای مورد نیاز، یا استفاده از مقادیر پیش‌فرض ناامن وجود دارد. این یک نگرانی حیاتی برای پایداری، امنیت و سهولت استقرار برنامه است.
...
- `.env.example:1-N` — `N/A` — فایل `.env.example` موجود است، اما کد برنامه که این متغیرها را مصرف و اعتبارسنجی می‌کند (مانند `app/config.py` یا `app/settings.py`) در دسترس نیست. این یافته به رویه کلی مدیریت پیکربندی می‌پردازد.
  ```
  N/A
  ```
...
پایتون، FastAPI، Pydantic، متغیرهای محیطی، Docker (با توجه به .dockerignore ضمنی).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.venv_t/lib/python3.12/site-packages/pydantic_settings-2.3.4.dist-info/METADATA` — تنظیمات Pydantic یک راه‌حل رایج و توصیه شده برای پیکربندی قوی در پروژه‌های پایتون، به ویژه با FastAPI است.
- `.dockerignore` — پیکربندی اغلب بر کانتینرسازی و استراتژی‌های استقرار تأثیر می‌گذارد.

## 🌐 نقشهٔ وابستگی‌ها
بر راه‌اندازی برنامه، اتصالات پایگاه داده، کلیدهای API، یکپارچه‌سازی سرویس‌های خارجی و سایر پیکربندی‌های حیاتی در تمام محیط‌ها تأثیر می‌گذارد. پیکربندی ناسازگار می‌تواند منجر به خرابی‌های گسترده شود.
...
- [ ] تمام متغیرهای محیطی مورد استفاده توسط برنامه در `.env.example` با توضیحات واضح مستند شده‌اند.
- [ ] در صورت عدم وجود یا نامعتبر بودن متغیرهای محیطی حیاتی، برنامه از راه‌اندازی باز می‌ماند.
- [ ] پیکربندی به طور مداوم در تمام نقاط ورودی برنامه (مانند برنامه FastAPI، کارگران Celery) بارگذاری می‌شود.
- [ ] اطلاعات حساس (مانند کلیدهای API، رمزهای عبور پایگاه داده) هرگز به صورت کد سخت نوشته نمی‌شوند و همیشه از متغیرهای محیطی بارگذاری می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک راه‌حل مدیریت پیکربندی قوی و متمرکز را پیاده‌سازی کنید. با توجه به استفاده احتمالی از Pydantic با FastAPI، `BaseSettings` از Pydantic (یا `pydantic-settings`) یک انتخاب عالی است. این شامل تعریف یک کلاس تنظیمات است که متغیرها را از فایل‌های `.env` و محیط بارگذاری می‌کند، مقادیر پیش‌فرض معقولی را ارائه می‌دهد و اعتبارسنجی نوع را انجام می‌دهد. اطمینان حاصل کنید که `.env.example` همیشه با تمام متغیرهای پیکربندی مورد نیاز و اختیاری، از جمله توضیحات واضح، به‌روز نگه داشته می‌شود.

## 💡 نمونه‌های قبل/بعد
**BaseSettings مفهومی Pydantic برای پیکربندی**
...
class Settings(BaseSettings):
    ...
    SECRET_KEY: str # در صورت عدم تنظیم، خطا ایجاد می‌کند
...
## 🧪 دستورات اعتبارسنجی
- `python -c "from app.config import settings; print(settings.DATABASE_URL)" # نیاز به وجود app.config`
- `برنامه را با متغیرهای محیطی حیاتی از دست رفته/نامعتبر اجرا کنید و خرابی راه‌اندازی را مشاهده کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به بازسازی منطق بارگذاری پیکربندی موجود دارد؛ احتمال معرفی خطاهای راه‌اندازی جدید در صورت عدم تست کامل در تمام محیط‌ها؛ مدیریت دقیق اسرار حیاتی است.
...
1. تمام متغیرهای محیطی مورد استفاده توسط برنامه در `.env.example` با توضیحات واضح مستند شده‌اند. _(verify: manual_only)_
2. در صورت عدم وجود یا نامعتبر بودن متغیرهای محیطی حیاتی، برنامه از راه‌اندازی باز می‌ماند. _(verify: backend_test)_
3. پیکربندی به طور مداوم در تمام نقاط ورودی برنامه (مانند برنامه FastAPI، کارگران Celery) بارگذاری می‌شود. _(verify: backend_test)_
4. اطلاعات حساس (مانند کلیدهای API، رمزهای عبور پایگاه داده) هرگز به صورت کد سخت نوشته نمی‌شوند و همیشه از متغیرهای محیطی بارگذاری می‌شوند. _(verify: static)_
