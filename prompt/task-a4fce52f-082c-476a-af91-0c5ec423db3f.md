---
task_id: a4fce52f-082c-476a-af91-0c5ec423db3f
title: 'Anti-pattern: Broken feedback loop / Conditional inconsistency'
type: bug_fix
priority: high
...
updated_at: '2026-06-03T18:43:57.924893+00:00'
target_files:
- app/models/todo_item.py
---

# Anti-pattern: Broken feedback loop / Conditional inconsistency

## Raw Idea

The `completed_at` column is designed to capture the completion timestamp for analytics ('ops can plot "completions per day"'). However, the model itself does not enforce the logical consistency between `is_completed` and `completed_at`. If `is_completed` is `True` but `completed_at` is `NULL`, or if `is_completed` is `False` but `completed_at` is set, this represents a conditional inconsistency. 

📁 file: app/models/todo_item.py (line 58)
...
- `app/database.py` — `todo_item.py` این فایل را import می‌کند
- `app/models/todo_list.py` — `todo_item.py` این فایل را import می‌کند
- `app/main.py` — این فایل `todo_item.py` را import می‌کند (caller)
- `app/services/self_improvement_service.py` — این فایل `todo_item.py` را import می‌کند (caller)
- `app/models/__init__.py` — این فایل `todo_item.py` را import می‌کند (caller)
- `app/services/ai/ai_data_access_service.py` — این فایل `todo_item.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile app/models/todo_item.py`
- `ruff check app/models/todo_item.py`
- `pytest -x`
