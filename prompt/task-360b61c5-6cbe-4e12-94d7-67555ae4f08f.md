---
task_id: 360b61c5-6cbe-4e12-94d7-67555ae4f08f
title: فایل `requirements.txt` برای مدیریت وابستگی‌ها مفقود است.
type: refactor
priority: high
...
created_at: '2026-06-09T21:30:59.244363+00:00'
updated_at: '2026-06-09T22:32:36.035900+00:00'
target_files:
- requirements.txt
---

# فایل `requirements.txt` برای مدیریت وابستگی‌ها مفقود است.

## Raw Idea

اگرچه یک محیط مجازی (`.venv_t`) با وابستگی‌های نصب شده وجود دارد، اما فایل `requirements.txt` که لیست دقیق وابستگی‌های پروژه را مشخص می‌کند، مفقود است. این فایل برای اطمینان از قابلیت بازتولید محیط توسعه و استقرار برنامه در محیط‌های مختلف (توسعه محلی، CI/CD، تولید) حیاتی است. بدون آن، نصب دقیق وابستگی‌ها دشوار خواهد بود.
...
- `requirements.txt:N/A` — `N/A` — فایل جدید برای وابستگی‌ها
  ```
  N/A
  ```
...
مدیریت وابستگی‌های پایتون با pip.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.dockerignore` — برای اطمینان از عدم کپی شدن محیط مجازی در ایمیج داکر
- `.github/workflows/claude-auto-task.yml` — ورک‌فلوهای CI/CD برای نصب وابستگی‌ها به این فایل نیاز دارند

## 🌐 نقشهٔ وابستگی‌ها
پروژه به بسته‌های متعددی مانند FastAPI، SQLAlchemy، Uvicorn، Pydantic و Celery وابسته است که باید به صورت صریح در `requirements.txt` تعریف شوند.
...
- [ ] فایل `requirements.txt` در ریشه پروژه ایجاد شود.
- [ ] فایل شامل حداقل وابستگی‌های اصلی (fastapi, uvicorn, sqlalchemy, pydantic) باشد.
- [ ] اجرای `pip install -r requirements.txt` با موفقیت وابستگی‌ها را نصب کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل `requirements.txt` در ریشه مخزن ایجاد کنید. این فایل باید شامل تمام وابستگی‌های مستقیم پروژه باشد که برای اجرای برنامه ضروری هستند. می‌توانید با استفاده از `pip freeze > requirements.txt` در محیط مجازی فعال، لیست اولیه را تولید کنید و سپس آن را برای حذف وابستگی‌های غیرضروری یا پین کردن نسخه‌ها به صورت دقیق‌تر ویرایش کنید.
...
جزئی، احتمال فراموش کردن برخی وابستگی‌ها در لیست اولیه.
...
1. فایل `requirements.txt` در ریشه پروژه ایجاد شود. _(verify: static)_
2. فایل شامل حداقل وابستگی‌های اصلی (fastapi, uvicorn, sqlalchemy, pydantic) باشد. _(verify: static)_
3. اجرای `pip install -r requirements.txt` با موفقیت وابستگی‌ها را نصب کند. _(verify: backend_test)_
