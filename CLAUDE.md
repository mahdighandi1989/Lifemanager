...
- **Live system map (owner's standing directive — «هیچ تغییری نباید از نقشه جا بماند»):**
  the `/system-map` page's «دیاگرام زنده» tab is generated by INTROSPECTION
  (`app/services/system_graph_service.py`), so every change MUST show up there instantly and
  automatically. That holds as long as you follow the existing conventions — the map derives
  itself from them: new routers appear because they're mounted in `app/main.py`; new
  models/tables because they're registered in `app/models/__init__.py`; new services because
  they're files under `app/services/`; background loops because their asyncio task is parked
  on `app.state.<name>_task`; scheduler jobs because they're tuples in `jobs_engine.JOBS`;
  new frontend pages because they're entries in `frontend/src/lib/routesMeta.js` (which is
  ALSO what App.jsx routes from — one registry, one truth; its per-line format is parsed by
  the backend, keep it). **Never hand-edit the map to reflect a change** — if something you
  added doesn't appear on the diagram, you bypassed a convention above; fix the registration,
  not the map. The pulse (light on the wires) is real traffic only — never fake activity.
