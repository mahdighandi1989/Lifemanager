...
6. **جذبِ خودکار (auto-intake):** هر متنِ تازه (نوشتهٔ نو، آرزوی تایپ‌شده، آیتمِ
   ...
   نرمال‌شده). **این را در همان حلقهٔ روزانه بگذار، نه در مسیرِ داغِ ساختِ
   محتوا:** چون extract خودش idempotent+dedup است، یک اجرای روزانه محتوای
   تازهٔ «پرچم‌دار» (ستاره‌دار) را می‌گیرد بدون آنکه به create آیتم دست بزنی —
   و یک `reconcile` هم فرمانِ محتوای حذف/trashِ‌شده را آرشیو می‌کند (برگشت‌پذیر).
   سیگنالِ «این مهم است» را از خودِ کاربر بگیر (ستاره) تا هر آیتمِ پیشِ‌پاافتاده
   پیشنهاد نشود.
