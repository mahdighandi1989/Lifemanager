...
## 2026-07-25 — FIX: دو کارتِ غلط بعد از سوییپِ ۲۴ ماهه (دقت، حالا در لایه‌ای که واقعاً شکست)

مالک بعد از زدنِ دکمهٔ تاریخچه: «این ارقام دلاری و ریالی کاملاً غلط… شاید برای بروکرها
اکانت دمو با واقعی رو اشتباه گرفته… اون حساب ریالی اصلاً اونقدر ندارم.» دو کارت:
`XM Global Limited −998.64 USD` و `بانک مرکزی / سامانه اعتبارسنجی 2,343,892,880 IRR`،
هر دو با برچسبِ «به‌روزرسانیِ خودکار از فایل» (مسیرِ پیوست).

- **FINDING ۱ — مدل بر پارسِ قطعی می‌چربید.** در `_feed_finance` ادغام
  `{**det_finance, **fields}` بود، یعنی عددِ **مدل** رویِ عددی می‌نشست که regex واقعاً از
  متن خوانده بود. صورت‌حسابِ بروکر «Floating P/L −998.64» دارد و مدل آن را به‌عنوان
  balance برگرداند. `parse_balance` هرگز عددِ منفی تولید نمی‌کند (regexش «-» را نمی‌گیرد)،
  پس منبعِ منفی فقط می‌توانست مدل باشد. **اصلاح:** برای مبلغ و ارز، قطعی برنده است؛ مدل
  فقط جای خالی را پر می‌کند.
- **FINDING ۲ — «مالی است» با «حسابِ من است» یکی گرفته شده بود.** گیتِ `_FIN_HINT` هر
  متنی با بوی مالی را رد می‌کرد به مرحلهٔ بعد. تا وقتی ورودی دو روز بود این کم‌ضرر بود؛
  با ۲۴ ماه، گزارشِ اعتبارسنجی و تسهیلات و بیمه و حسابِ دموی بروکر هم وارد شدند.
  **اصلاح:** `is_not_an_account()` — گزارش/رتبهٔ اعتباری، استعلام، چک برگشتی، تسهیلات و
  اقساط و وام، مالیات، بیمه‌نامه، و **حساب دمو/آزمایشی** هرگز کارت نمی‌سازند و موجودی
  هیچ کارتی را تکان نمی‌دهند. در هر دو مسیرِ ایمیل و پیوست اعمال شد.
- **FINDING ۳ — موجودیِ منفی.** ساختِ کارت `bal <= 0` را رد می‌کند و به‌روزرسانی هم عددِ
  منفی را نادیده می‌گیرد؛ اگر حسابی واقعاً منفی است، مالک خودش می‌نویسد.
- **FINDING ۴ — راهِ فرار وجود نداشت.** `cleanup-auto-cards` فقط کارتِ **خالی** (بدون
  موجودی و بدون تغییر) را پاک می‌کند، و کارتِ غلط معمولاً هر دو را دارد؛ و `DELETE` برای
  حساب اصلاً وجود نداشت. **اصلاح:** `DELETE /api/finance/accounts/{id}` (حساب + تراکنش‌هایش،
  با ثبت در لاگ فعالیت) و دکمهٔ «✖ این حساب من نیست» روی هر کارت.
- **VERIFY** ۵ تستِ نو در `test_finance_email_scan.py` (نه‌حساب‌بودنِ گزارش اعتباری/وام/دمو،
  ردِ ایمیلِ اعتبارسنجی، ردِ موجودیِ منفی در ساخت و در به‌روزرسانی، ردِ گزارشِ اعتباری در
  مسیرِ فایل، و حذفِ کارت) → این فایل ۱۴ تست؛ گیتِ کامل **۱۳۷۷ پاس / ۱۲ شکستِ baseline**؛
  vitest ۱۳۷ پاس / ۱۵ شکستِ baseline؛ `npm run build` سبز.
- **نکتهٔ الگو (تکرارِ همان درس):** پهن‌کردنِ ورودی، دقتِ استخراج‌گر را به گلوگاه تبدیل
  می‌کند. هر بار که پنجرهٔ داده باز شد، گیتِ precision باید هم‌زمان سفت شود.

## 2026-07-25 — FIX: انتقال از صندوق به «مالی» — خرید، هزینه است نه حساب؛ و راهنمای رمزِ خودِ بانک

مالک (با دو اسکرین‌شات): خریدهای کارفور/طلبات را به مالی منتقل کرده و سیستم برایشان
حسابِ بانکیِ ۰٫۰۰ ساخته («مثل گاو اومده تحلیل کرده و حساب درست کرده»)؛ موجودی باز
اشتباه شده و صورت‌حسابِ قدیمی‌تر لحاظ شده؛ و برای فایل‌های رمزدار نمی‌گوید بانک چه
اجزایی برای رمز خواسته.

- **FINDING ۱ (ریشهٔ سیلِ کارت‌ها)** در منوی «ارسال به…»ِ میز فرمان اصلاً گزینهٔ
  «خرید/هزینه» وجود نداشت — تنها گزینهٔ مالی «حساب مالی» بود. مسیرِ درست
  (`_file_as_transaction` → هزینه روی «نقدی/رسیدها») از قبل در کد بود ولی هیچ دری به
  آن باز نمی‌شد. و `_file_as_finance_account` برای هر payloadِ بی‌سیگنال، «برای اینکه
  تأیید بی‌نتیجه نماند» کورکورانه کارتِ ۰٫۰۰ می‌ساخت — منشأ کارت‌های Carrefour/Talabat/
  First Abu Dhabi و کارتِ «فرد جدید از ایمیل: estatement&lt;…&gt;».
- **CHANGE** `_file_as_finance_account`: کارت فقط با **سیگنالِ حساب** (IBAN/شماره یا
  موجودیِ مثبت)؛ payloadِ دارای مبلغ ولی بدونِ سیگنال → خودکار به `_file_as_transaction`
  می‌رود (هزینه)؛ payloadِ بدونِ هیچ‌چیزِ مالی → 422 با پیامِ روشن، نه کارتِ کور. عنوان‌ها
  قبل از استفاده تمیز می‌شوند (`_clean_provider`: unescape + حذفِ پیشوندِ «فرد جدید از
  ایمیل» و آدرسِ داخل &lt;&gt;). گزینهٔ «خرید / هزینه» به منوی ارسال اضافه شد.
- **FINDING ۲ (صورت‌حسابِ قدیمی)** مسیرِ انتقالِ دستی `occurred_iso=None` می‌فرستاد و
  «بی‌تاریخ» در قاعدهٔ is_newer «جدیدتر» حساب می‌شد → تأییدِ یک صورت‌حسابِ قدیمی، موجودیِ
  فعلی را جابه‌جا می‌کرد. **CHANGE:** تاریخِ سند از ایمیلِ آینه‌شده بازیابی می‌شود
  (`_occurred_from_source` روی `gmail:<mid>:…`)، و در `apply_account_signal` سیگنالِ
  ماشینیِ بی‌تاریخ (source_refدار) دیگر هرگز بر موجودیِ تاریخ‌دار نمی‌چربد؛ ثبتِ دستیِ
  مالک (بدونِ source_ref) مثل قبل برنده است.
- **FINDING ۳ (فردِ قلابی)** `estatement@bankfab.com` از فیلترِ «فرستندهٔ غیرانسانی»
  رد می‌شد. **CHANGE:** `e?statements?@`, `ebill`, `alerts?@`, `notify@`, `service@`,
  `customer(care|service)`, `admin@`, `security@`, `verify@`, `otp@` به `_RE_NONHUMAN`
  اضافه شد.
- **CHANGE (راهنمای رمز)** `password_hint_from(body)`: جملهٔ خودِ بانک دربارهٔ ساختِ
  رمز (password/رمز/گذرواژه/كلمة المرور) از متنِ ایمیل درمی‌آید و در
  `suggestion.password_hint` می‌نشیند؛ کارتِ «رمز لازم» در میز فرمان همان را بالای
  فیلدِ رمز نشان می‌دهد: «💡 بانک نوشته: …». وقتی recipe قابلِ‌استخراج نبود، این کفِ
  صادقانه است.
- **CHANGE (UI)** دکمهٔ «✖ این حساب من نیست» قبلاً فقط روی کارت‌های دارای حرکت رندر
  می‌شد — یعنی دقیقاً روی کارت‌های آشغالِ ۰٫۰۰ دیده نمی‌شد. حالا روی همهٔ کارت‌های زنده
  هست؛ مالک می‌تواند شش کارتِ اشتباهِ موجود را همان‌جا پاک کند.
- **VERIFY** `tests/test_inbox_receipt_filing.py` (۸ تست: رسید→هزینه؛ ردِ بی‌سیگنال؛
  صورت‌حسابِ واقعی→کارت؛ تمیزکردنِ عنوان؛ تاریخ از ایمیل + بی‌تاریخ نمی‌چربد؛
  estatement شخص نیست؛ استخراجِ راهنمای رمز EN/FA؛ ذخیرهٔ hint) سبز؛ گیتِ کامل
  **۱۳۸۵ پاس / ۱۲ شکستِ baseline**؛ vitest ۱۳۷ پاس / ۱۵ baseline؛ build سبز.
- **برای دادهٔ موجود:** کارت‌های اشتباهِ فعلی با دکمهٔ «این حساب من نیست» پاک شوند؛
  کارتِ BSI اگر موجودی‌اش غلط است، پاک شود و «آوردنِ تاریخچه» دوباره زده شود — چون
  backfill قدیمی‌به‌جدید اعمال می‌کند، موجودیِ آخر همان جدیدترین صورت‌حساب می‌شود.

## 2026-07-25 — FIX: «کارت‌ها را پاک کردم، سوییپ زدم، هیچی نیاورد» — بازسازیِ خودترمیم

- **FINDING** حافظهٔ ضدتکرار در دو لایه بود و لایهٔ اشتباهش جلوی بازسازی را گرفت:
  `_already_ingested` (روی InboxItemها، تا مالک دوباره سؤال‌پیچ نشود) در همان ابتدای
  `extract_from_file` همه‌چیز را «تکراری» می‌کرد و برمی‌گشت — در حالی که حافظهٔ ضدتکرارِ
  **مالی** (`source_refs`/`applied_refs`/هشِ سطرها) روی خودِ ردیفِ حساب زندگی می‌کرد و
  با حذفِ کارت مُرد. نتیجه: بعد از «✖ این حساب من نیست»، سوییپِ بعدی هیچ کارتی را
  بازنساخت، چون مسیرِ مالی هرگز اجرا نشد.
- **CHANGE (خودترمیم)** مسیرِ duplicate در `extract_from_file` حالا یک بازخوانیِ
  **فقط‌قطعی** انجام می‌دهد (بدون AI، بدون آیتمِ تازه در صندوق): `prepare_bytes` با رمزِ
  ذخیره‌شده → `extract_text` → `parse_finance_fields` → `_feed_finance`. کارتِ حذف‌شده از
  همان فایل‌ها بازسازی می‌شود؛ کارتِ سالم به‌لطفِ ضدتکرارِ خودش سرتاسر no-op است. همهٔ
  گاردهای دقت (نه‌حساب‌ها، موجودیِ منفی، قطعی‌برنده) سرِ جایشان اعمال می‌شوند — یعنی
  کارت‌های آشغالِ قبلی در بازسازی برنمی‌گردند.
- **CHANGE (شفافیت)** شمارندهٔ `finance_rechecked` از `extract_from_file` تا
  `ingest_email_attachments` → `backfill_attachments` → `POST /api/inbox/deep-sweep` بالا
  می‌آید و پیامِ دکمهٔ سوییپ حالا می‌گوید «N فایلِ مالیِ قبلی دوباره اعمال شد» — «۰ ایمیلِ
  تازه» دیگر شبیهِ «هیچ اتفاقی نیفتاد» خوانده نمی‌شود.
- **VERIFY** تستِ `test_deleted_card_is_rebuilt_on_the_next_sweep` (حذفِ کارت → سوییپ →
  بازسازی با موجودی و ۲ سطرِ ریزگردش → سوییپِ سوم بدونِ هیچ تکراری) سبز؛ گیتِ کامل
  **۱۳۸۶ پاس / ۱۲ شکستِ baseline**؛ build سبز.

## 2026-07-30 — نقشهٔ سیستم بالاخره «زنده» شد: دیاگرام از خودِ کد، نبض از ترافیک واقعی

- **FINDING** خواستهٔ چندبارهٔ مالک («به همه صفحاتش بنگر… دیاگرام فعال برنامه را رسم کن»)
  با نسخهٔ قبلی `/system-map` برآورده نمی‌شد: آن صفحه یک فهرستِ دست‌نویسِ ایستا بود —
  نه خودبه‌روزشونده، نه کامل، نه دارای جریان. ریشهٔ مشکل: هر نقشه‌ای که «دربارهٔ» کد
  نوشته شود، از لحظهٔ نوشتن کهنه است.
- **DECISION** نقشه باید از خودِ برنامه «زاییده» شود، نه دربارهٔ آن نوشته شود:
  روترها از route-table زندهٔ FastAPI، مدل‌ها و روابط FK از رجیستری SQLAlchemy،
  سرویس‌ها از فایل‌های `app/services/`، موتورهای پس‌زمینه از taskهای asyncio روی
  `app.state` (زنده/مرده‌ی واقعی)، کارهای زمان‌بندی از `jobs_engine.JOBS`، و صفحه‌ها از
  رجیستری تازهٔ `frontend/src/lib/routesMeta.js` که **خودِ App.jsx هم از همان route
  می‌سازد** — پس صفحه‌ای بیرون از نقشه اصلاً render نمی‌شود. سیم‌های وابستگی با پیمایش
  کاملِ AST (چون ۳۱ از ۵۹ روتر importِ تنبلِ درون‌تابعی دارند و اسکن سطح-ماژول نصف
  سیم‌ها را می‌انداخت). کش با امضای mtime → هر تغییرِ فایل، در همان لحظه نقشه را تازه می‌کند.
- **CHANGE** بک‌اند: `app/services/system_graph_service.py` (درون‌نگری + اسکن فرانت +
  کش)، `app/services/system_pulse_service.py` (بافر حلقه‌ای نبض + سیم‌های یادگرفته از
  ترافیک، ذخیره در `global_settings` — بدون جدول جدید و بدون migration)،
  میان‌افزار `SystemPulseMiddleware` در `app/main.py` (فقط تماشاچی؛ در مسیر درخواست
  هرگز DB لمس نمی‌شود؛ endpointهای خود نقشه از ثبت مستثنا تا نقشه خودش را روشن نکند)،
  و چهار endpoint تازه روی روتر موجود: `GET /api/system-map/graph`،
  `GET /api/system-map/activity`، `POST /api/system-map/layout`،
  `POST /api/system-map/wires`. endpoint قدیمی `GET /api/system-map` دست‌نخورده ماند.
- **CHANGE** فرانت: `lib/routesMeta.js` (منبع واحد صفحه‌ها؛ فرمتِ هر-ورودی-یک-خط
  load-bearing است چون بک‌اند همان را parse می‌کند)، `routesRegistry.jsx`، بازنویسی
  `App.jsx` روی رجیستری (رفتار حفظ شد)، هدر `X-LM-Page` در `lib/api.js` (الگوی مسیر،
  نه URL خام)، کامپوننت `components/SystemDiagram.jsx` (SVG دست‌ساز: کارت‌ها،
  سیم‌های بزیه، پن/زوم، درگ کارت با ذخیرهٔ چیدمان، کشیدن سیم دستی از پورت کارت،
  نور متحرک `animateMotion` فقط روی سیم‌هایی که ترافیک واقعیِ لحظه‌ای دارند،
  «شما این‌جایید»، پنل جزئیات، فیلتر نوع/جستجو/فقط-فعال‌ها) و `SystemMapPage.jsx`
  با دو تب: «دیاگرام زنده» (پیش‌فرض) + «راهنما» (نمای قدیمی، قرنطینه-نه-حذف).
- **DECISION (صداقت نبض)** فقط چیزی روشن می‌شود که واقعاً اندازه‌گیری می‌شود: روترها
  (درخواست‌های واقعی)، سیم صفحه→روتر (هدر صفحه)، موتورها (`task.done()`). سیم‌های
  ساختاری (import/FK) خاکستری می‌مانند — روشن‌کردن حدسی، همان «نمایش بصری بی‌معنا»یی
  است که مالک صریحاً رد کرد.
- **CHANGE (سیم دستی = همگام با بک‌اند)** کشیدن سیم بین دو کارت در `global_settings`
  (کلید per-user) ذخیره می‌شود و در بارگذاری بعدی برمی‌گردد — پاسخ عملی به پرسش مالک؛
  آنچه ذخیره می‌شود «رابطهٔ اعلام‌شده» است، نه تغییر کد.
- **CHANGE (قاعدهٔ الزام‌آور)** به CLAUDE.md افزوده شد: نقشه introspection-derived است؛
  «اگر چیزی که ساختی روی نقشه نیست، یعنی یک قرارداد ثبت را دور زده‌ای — ثبت را درست کن،
  نه نقشه را»، و ثبت تجربه در
  `experiences/live-architecture-diagram-from-runtime-introspection.md`.
- **FINDING (پیش‌زمینه، خارج از این کار)** پیش از این تغییرات، ۱۳ تست بک‌اند روی درخت
  تمیز قرمز بود (۱۲ در گزارش سشن قبل) — عمدتاً `tests/test_auth_required_user_id_9a5a3b4d.py`
  که `AuthService` را در `app/dependencies/auth.py` monkeypatch می‌کند ولی چنین صفتی
  آن‌جا وجود ندارد؛ و ۵ فایل تست فرانت (Footer/Header/Layout/Notifications/api-401) هم
  از قبل قرمز بودند. دست‌نخورده گذاشتم تا این کار تک-موضوعی بماند.
- **VERIFY** تست‌های تازه: `tests/test_system_map_live.py` — ۹ تست سبز (درون‌نگری گراف،
  اثباتِ سیمِ import تنبل، ثبت نبض + یادگیری سیم از ترافیک، مستثنابودن self-noise،
  ممنوعیت DB در مسیر درخواست، roundtrip چیدمان و سیم دستی، تطابق رجیستری↔دیسک).
  فرانت: `SystemMapPage.test.jsx` بازنویسی (تب دیاگرام + حفظ همهٔ assertionهای نمای
  قدیمی)، `routesRegistry.test.jsx` و دو تست `X-LM-Page` جدید — همه سبز.
  گیت کامل: بک‌اند **۱۳۹۴ پاس / همان ۱۳ شکستِ baseline (بدون خطای جدید)**؛
  vitest **۱۴۴+۹ پاس / همان ۱۵ شکستِ baseline (بدون خطای جدید)**؛ `npm run build` سبز.
- **REVERT/VERIFY (بازبینی پیش از merge)** بازبینی خصمانهٔ چهار-محوره اجرا شد؛ اجرای
  عامل‌های فرعی به نقص زیرساختی خورد (ورودی ابزارها خالی می‌شد)، پس بازبینی همان
  محورها دستی و خط‌به‌خط انجام شد. سه یافتهٔ واقعی، هر سه رفع و تست شدند:
  (۱) هدر `X-LM-Page` ورودیِ نامعتمد بود → رشد بی‌کرانِ سیم‌های یادگرفته؛ حالا فقط
  الگوهای موجود در routesMeta پذیرفته می‌شود + سقف ۳۰۰۰ جفت.
  (۲) payload چیدمان بی‌سقف بود → گارد ۲۵۶KB (خطای 400).
  (۳) `animate-ping` روی عنصر SVG به‌خاطر transform-origin می‌پرید → انیمیشن SMIL.
  گیت نهایی: بک‌اند ۱۳۹۶ پاس / همان ۱۳ شکست baseline؛ vitest همان ۱۵ شکست baseline؛
  build سبز → merge به main.

## 2026-07-30 — FINDING: ممیزی کامل خطِ مالی (تشخیص حساب و موجودی)

- **FINDING** در پی شکایت مجدد مالک («تشخیص حساب‌ها و موجودی هنوز فوق‌العاده خطا دارد»)،
  کل خطِ «ایمیل/فایل → کارت حساب → موجودی» ممیزی شد؛ ۲۷ نقص مشخص با شواهد file:line
  (و راستی‌آزمایی اجرایی regexها) ثبت شد — گزارش کامل و نقشهٔ درمان سه‌فازی در
  `docs/decisions/2026-07-30-finance-pipeline-audit.md`. ریشه: چهار مسیر ورودی مستقل با
  گاردهای نابرابر، استخراج موجودی بدون فهم ارقام فارسی/معنای balance/ارز غایب، نبود
  allow-list «حساب‌های خود مالک»، نبود tombstone حذف، و تست‌هایی که فقط fixture تمیز
  ASCII را می‌پوشانند. **هنوز اصلاحی اعمال نشده** — درمان منتظر تأیید مالک است.

## 2026-07-30 — بازسازی دقتِ خطِ مالی: هر سه فاز درمان اجرا شد

- **CHANGE (فاز ۱ — خودِ موجودی)** `email_parser_service.py` بازنویسی شد: نرمال‌سازی
  ارقام فارسی/عربی و «٬/٫» پیش از regex (۱۲٬۵۰۰٬۰۰۰ دیگر 12.0 نمی‌شود)؛ طبقه‌بندی نوع
  balance — Previous/Opening/Outstanding/rewards/points رد می‌شوند، Available/Current/
  Closing بر balance خام مقدم‌اند؛ عدد بی‌ارزِ بی‌جداکنندهٔ کوتاه (work-life balance 10)
  پول نیست؛ فرمت اروپایی 1.234.567,89 درست خوانده می‌شود؛ ارزهای SAR/QAR/OMR/KWD/BHD/…
  اضافه شد. در `apply_account_signal`: بدون ارز کارت ساخته نمی‌شود (پیش‌فرض USD حذف)؛
  صفر و منفی هرگز کارت زنده را بازنویسی نمی‌کنند؛ سیگنالِ ارزِ متفاوت نه عدد را می‌نویسد
  نه برچسب ارز را عوض می‌کند؛ سیگنال بی‌تاریخ دیگر گاردِ تازگی را خلع سلاح نمی‌کند.
- **CHANGE (فاز ۱ — سنگ قبر)** حذفِ کارت از صفحهٔ مالی حالا هویتش (بانک/ref/IBAN) را در
  `global_settings:finance_account_tombstones` ثبت می‌کند؛ خودترمیمِ d2ddf0e دیگر کارتِ
  حذف‌شده را برنمی‌گرداند. «بازگردانی» (پاک‌کردن سنگ قبر — endpoint + دکمه در پنل مالی)
  همان قابلیت بازسازی را برمی‌گرداند؛ تأیید دستی مالک (`trusted=True`) همیشه مقدم است.
  گاردِ «خرید ≠ حساب» به مسیر خودکار پیوست‌ها هم آمد (فقط دستی داشتیم).
- **CHANGE (فاز ۲ — هویت)** تطبیق ref فقط با سازگاری بانک (••4321 دو بانک = دو کارت)؛
  سیگنال بی‌ref با چند کارت در یک بانک «مبهم» و رد می‌شود (به‌جای حدس)؛ برند دامنه =
  آخرین قطعهٔ غیرعمومی (mail.wise.com → wise؛ notification.adcb.ae → adcb)؛ «حساب‌های
  من» (allow-list در `global_settings:finance_owner_accounts` + endpointها + پنل UI):
  وقتی پر شد، کارت خودکار فقط برای همان‌ها ساخته می‌شود؛ ارقام قطعی بر LLM در
  account_no/iban مقدم شدند؛ ‌_LAST4 چهار رقمِ آخرِ ران را می‌گیرد؛ IBAN فقط با پیشوند
  کشور واقعی؛ توقف‌واژه‌ها روی خودِ نام حساب هم اعمال شد (`_pick_account`).
- **CHANGE (فاز ۳ — حساب‌کتاب)** تراکنش‌های دلتای ماشینی برچسب `_balance_delta` گرفتند و
  گزارش ماهانه آن‌ها (و دلتاهای legacy) را جمع نمی‌زند — پایان دوبل‌شماری؛ مسیر
  ingest-message با هش پیام dedupe و source/source_ref/currency دار شد + ردِ ارز متفاوت؛
  حافظهٔ refs مرتب و بدون حذف تصادفی (`_keep_order_tail`)؛ دو خرید یکسانِ هم‌روز هر دو
  ثبت می‌شوند (پسوند occurrence روی هش) و آپلود مجدد همچنان idempotent؛ share-sheet بانکی
  دیگر جزیره نیست (به کارت واقعی می‌رسد)؛ فهرست‌های bank/broker/exchange و تحلیل AI با
  scope شامل NULL؛ تحلیل AI به تفکیک ارز (جمع بین‌ارزی حذف).
- **VERIFY (چند بار، طبق خواستهٔ مالک)** ۲۳ تست جدید (`tests/test_finance_precision_overhaul.py`)
  همه سبز؛ ۸۰ تست مالی موجود بدون تغییر سبز؛ کل بک‌اند دو بار اجرا شد — **۱۴۱۹ پاس /
  همان ۱۳ شکستِ baseline** (یک flake ثابت‌شده در dev_sync، در ایزوله ۲×سبز)؛ build و
  vitest بدون خطای جدید (همان ۱۵ نام ارثی)؛ و یک آزمون سرتاسری با ۶ ایمیل واقع‌گرایانه
  (بانک ایرانی با ارقام فارسی، mail.wise.com، صورت‌حساب اعتباری، خبرنامه، اعتبارسنجی،
  اعلان صفر): دقیقاً ۲ کارتِ درست ساخته شد، هیچ کارت غلطی نه، صفر بازنویسی نشد، حذف
  دائمی ماند و بازگردانی بازسازی کرد. تجربهٔ به‌روزشده در
  `experiences/self-feeding-account-cards-from-synced-email.md` (## Update 2026-07-30).

## 2026-07-30 — جعبهٔ ترمیم موجودی‌ها + نسخهٔ همراهِ رصدگر (موبایل)

- **FINDING** مالک پس از سه فاز اصلاح هنوز موجودی غلط می‌دید. ریشه: عددهای غلط را
  موتورِ قدیمی در دیتابیس نوشته بود و گاردهای جدید (به‌درستی) از بازنویسی‌شان هم جلوگیری
  می‌کردند — دادهٔ آلوده به ترمیم نیاز داشت، نه فقط موتورِ سالم. باگ چهارمی هم پیدا شد:
  مسیر SMS از parse_sms قدیمی می‌گذشت که ارقام فارسی را نمی‌فهمید (۱۲٬۵۰۰٬۰۰۰ → 12.0)
  و ارز را خام («ریال») برمی‌گرداند.
- **CHANGE (جعبهٔ ترمیم)** ① `POST /api/finance/rebuild-auto-cards` + دکمهٔ «♻️ بازتولید
  از نو»: همهٔ کارت‌های ماشینی (inferred) پاک و با موتورِ دقیقِ جدید از نو ساخته می‌شوند؛
  کارت‌های دستی و tombstoneها دست‌نخورده. ② شفافیت: هر کارت جملهٔ دقیقی که عددش از آن
  خوانده شده را نشان می‌دهد (`balance_evidence` + سطر «خوانده‌شده از…»). ③ `PUT
  /api/finance/accounts/{id}` + دکمهٔ «✍️ اصلاح موجودی»: عددی که مالک وارد کند حقیقت
  نهایی است (`owner_balance_at`) — فقط سیگنالِ جدیدتر از لحظهٔ ویرایش می‌تواند حرکتش دهد.
- **CHANGE (SMS)** `parse_sms` حالا نیمهٔ موجودی‌اش را به همان استخراج‌گر سخت‌شدهٔ ایمیل
  می‌سپارد: ارقام فارسی، طبقه‌بندی balance، ارز канونیک (IRR نه «ریال») — تست پین‌شده
  به‌روز شد (IRR هم مجاز).
- **CHANGE (نسخهٔ همراه)** روتر جدید `app/routes/mobile.py`: `GET /api/mobile/token`
  (جفت‌سازی، قابل چرخش)، `POST sms|notification|usage|heartbeat` (همه با هدر
  `X-Device-Token`، مقایسهٔ constant-time، بدون توکن هیچ‌چیز ثبت نمی‌شود) و
  `GET /api/mobile/status`. پیامک/اعلانِ بانکی از همان موتور مالی سخت‌گیر می‌گذرد
  (dedup هشی، ردِ ارز غلط، دلتای مصنوعی نشانه‌دار)؛ همه‌چیز در لاگ فعالیت می‌نشیند و
  ضربان دستگاه از طریق میان‌افزار نبض روی «دیاگرام زنده» هم دیده می‌شود. سورس کامل اپ
  اندرویدی (Kotlin: SmsReceiver، NotificationListener، UsageWorker، صف WorkManager با
  retry آفلاین) در `mobile/companion-android/` + README فارسی (شامل صداقت دربارهٔ
  iPhone: SMSخوانی در iOS ممکن نیست؛ PWA از قبل نصب‌شدنی است). APK این‌جا کامپایل
  نشده (SDK اندروید در محیط نیست) — سمت سرور کاملاً تست‌شده است.
...
## 2026-07-31 — نگهبان اتصال موبایل + دسته‌بندی رویدادها + ضد دوبله‌خوانی

- **CHANGE** سرویس جدید `mobile_watchdog_service.py` + جاب `mobile_watchdog` در موتور
  زمان‌بندی (هر ~۱۵ دقیقه): گوشی‌ای که بیش از آستانه (پیش‌فرض ۹۰ دقیقه،
  `MOBILE_SILENCE_THRESHOLD_MINUTES`) ساکت بماند → اعلان تلگرام/بحرانی؛ تا وصل نشدن هر
  ~۶ ساعت (`MOBILE_SILENCE_REALERT_MINUTES`) تکرار؛ برگشتن → یک «وصل شد». گزارش روزانه
  (متنی و HTML ایمیل) قطعیِ در-جریان را به‌عنوان موضوع برجستهٔ بالای گزارش می‌آورد
  (`mobile_silent` در digest). حالت هشدار در global_settings.
- **CHANGE** دسته‌بندی قطعی رویدادهای موبایل در سرور (`[دسته]` در ابتدای detail لاگ):
  mirrored/otp/promo/finance/message — OTP دیگر هرگز به مالی نمی‌رود؛ اعلان اپ‌های
  Gmail/Calendar/Drive (که آینهٔ گوگل کاملشان را می‌خواند) فقط لاگ می‌شود تا هیچ سیگنالی
  دوبار شمرده نشود. اندروید: BootReceiver (نبض فوری بعد از خاموش/روشن) + مجوز BOOT.
...
## 2026-07-31 — رهگیریِ عمیق‌تر موبایل (تماس/صفحه/مصرف) + پاسخ صادقانه به سقف رصد

- **DECISION (شفافیت با مالک)** مالک پرسید آیا اپ می‌تواند «همه‌چیز» را ببیند (صدای تماس،
  ویدیوها، پروفایل‌های شبکه‌های اجتماعی). صادقانه توضیح داده شد: اندروید عمداً اپ‌ها را
  sandbox می‌کند — ضبط صدای تماس از اندروید ۱۰ ممنوع است؛ محتوای ویدیو/پروفایلِ داخل اپ‌ها
  هیچ API ندارد؛ تنها راهِ «متنِ روی صفحه» سرویس Accessibility است (شکننده، پرمصرف، متنِ
  دیگران را هم می‌خواند، فقط متن). مالک هر سه گزینهٔ ممکن را انتخاب کرد.
- **CHANGE (دفترچهٔ تماس)** `POST /api/mobile/call` + `CallLogWorker` اندروید (هر ساعت،
  از since آخر): شماره/نوع/مدت/زمان (نه صدا) → لاگ فعالیت، و **اتصال به فردِ «افراد» با
  تطبیق دُمِ شماره** (نه رکورد تکراری)؛ dedup سروری با هشِ (شماره+زمان).
- **CHANGE (خواندن صفحه)** `POST /api/mobile/screen` + سرویس `ScreenReader` (Accessibility):
  throttle ۴ثانیه‌ای + ردِ متنِ تکراری، ردِ فیلدهای رمز روی گوشی، و **پاک‌سازیِ دوم OTP در
  سرور**؛ دسته‌بندی همان `_classify`. اختیاری — با دکمهٔ جدا در اپ و فعال‌سازی از تنظیمات.
- **CHANGE (مصرف دقیق‌تر)** UsageWorker حالا **تعداد باز کردن قفل** (KEYGUARD_HIDDEN) را هم
  می‌شمارد؛ payload مصرف فیلدهای unlocks/sessions گرفت.
- **CHANGE (اندروید)** مجوز READ_CALL_LOG + سرویس Accessibility در manifest، دکمه‌های
  دسترسی در MainActivity، جاب دوره‌ای calllog. همهٔ رویدادها با برچسب `[دسته]` در لاگ.
- **VERIFY** ۵ تست جدید (اتصال تماس به فرد + dedup، نیاز به توکن، دسته‌بندی صفحه + پاک‌سازی
  OTP، unlocks)؛ کل بک‌اند ۱۴۳۶ پاس / همان ۱۳ baseline؛ build سبز. push، CI را وادار به
  ساخت APK تازه (با تماس/صفحه/مصرف) می‌کند.

## 2026-07-31 — دادهٔ موبایل دیگر هرز نمی‌رود: زمانِ واقعی، اتصال به افراد، دیدِ AI، آرشیو درایو

- **FINDING (ممیزی خصمانه)** بررسی شد که آیا دادهٔ استخراج‌شدهٔ موبایل واقعاً استفاده
  می‌شود. نتیجهٔ تلخ: جز مالی (پیامک بانک)، بقیه «جزیره» بودند — نوشته و ایندکس و بکاپ
  می‌شدند ولی **هیچ‌جا خوانده نمی‌شدند**. سه نقص ریشه‌ای: (۱) ActivityLog فقط created_at
  داشت، پس زمانِ واقعیِ تماس/پیامکِ قدیمی نابود می‌شد (همه یک زمان نشان می‌دادند)؛
  (۲) تماس‌ها به Interaction وصل نمی‌شدند، پس امتیاز/کارنامهٔ فرد هرگز تکان نمی‌خورد
  (InteractionType.CALL تولیدکننده نداشت)؛ (۳) AI/دستیار/گزارش هیچ دیدی به این داده نداشتند.
- **CHANGE (زمانِ واقعی)** ستون `activity_logs.occurred_at` (مدل + startup ALTER + مهاجرت
  0054 + record_activity(occurred_at=…)). مسیرهای خواندن بر COALESCE(occurred_at, created_at)
  مرتب و «display_at» را می‌دهند؛ فرانت همان را نشان می‌دهد. اندروید حالا received_at/
  posted_at را هم می‌فرستد؛ call/screen به مجموعهٔ liveness اضافه شدند.
- **CHANGE (اتصال به افراد)** تماسِ وصل‌شده به یک فرد حالا `record_interaction(CALL)` می‌سازد
  (dedup با ref تماس)، پس رابطه واقعاً تغذیه می‌شود؛ پروفایلِ فرد فیلدِ `contact`
  (تعداد تماس، ۳۰ روز اخیر، آخرین تماس) گرفت.
- **CHANGE (دیدِ AI + خوانایی)** سرویس `mobile_insights_service` (کارکرد صفحه/قفل‌گشایی/
  پرتماس‌ترین افراد/دسته‌ها) → به دستیار سراسری، گزارش روزانه، و endpoint
  `GET /api/mobile/insights` وصل شد. برچسب‌های فارسی برای رویدادهای موبایل در
  lib/activityLog.js؛ اعلان گوشی entity_type=`phone_notification` شد تا با اعلانِ
  درون‌برنامه‌ای اشتباه/لینک نشود.
- **CHANGE (آرشیو درایو)** سرویس `activity_archive_service` + جاب روزانه: ماه‌های
  بسته‌شدهٔ لاگ، یک‌بار و append-only روی `LifeManagerData/ActivityArchive/` ذخیره
  می‌شوند **بدون حذف از دیتابیس** (همچنان در برنامه دیده می‌شوند)؛ idempotent با
  فهرست ماه‌های آرشیوشده در global_settings؛ بدون درایو، بی‌صدا به اجرای بعدی موکول.
- **VERIFY** ۹ تست جدید (occurred_at، تماس→Interaction+contact_stats، insights،
  دسته‌بندی، آرشیو idempotent و nondestructive)؛ کل بک‌اند ۱۴۴۰ پاس / همان ۱۳ baseline؛
  vitest همان ۱۵ baseline؛ build سبز.

## 2026-07-31 — مسیریابِ مرکزیِ سیگنال موبایل: پایانِ سیم‌کشیِ دستیِ هر نوع

- **DECISION (معماری، به‌درخواستِ مالک)** مالک درست پرسید: «فقط تماس را سیم‌کشی کرده‌ای؛
  هزار نوع داده می‌آید — هر کدام باید به جای خودش برود و تحلیل شود، چطور؟» به‌جای افزودنِ
  شاخهٔ if برای هر نوع در نقطهٔ ورود، یک **درگاهِ واحد** ساخته شد.
- **CHANGE** سرویس `mobile_dispatch_service`: (۱) `classify_signal` با فهرستِ قاعدهٔ
  مرتب (نویز اول: mirrored/otp/promo، بعد finance/appointment/task/message)؛
  (۲) رجیستریِ `{category: router}` که هر router فقط به سرویسِ دامنهٔ موجود تحویل می‌دهد
  (مالی→apply_bank_message، فردِ شناخته‌شده→record_interaction(MESSAGE)، اکشن‌دار→صندوقِ
  ورودی با تریاژِ AI). افزودنِ نوعِ جدید = یک شاخهٔ classify + یک خط رجیستری.
- **DECISION (ضدهرز + ضدسیل)** «capture» از «route» جدا شد: همه‌چیز در لاگ می‌ماند
  (رکورد کامل + آرشیو + insights)، ولی فقط دستهٔ اکشن‌دار/شناخته‌شده به دامنه/صندوق
  می‌رود؛ نویز و گپِ خنثی مسیریابی نمی‌شوند تا صندوق پر از نویز نشود. کلیدِ ضدتکرار
  `source_ref` در هر router (هشِ مالی، suggestion.source_ref صندوق، dedup_note تعامل).
- **CHANGE** درگاهِ SMS و اعلانِ موبایل از این مسیریاب عبور می‌کنند؛ detailِ لاگ حالا
  `[دسته→مقصد]` را نشان می‌دهد. صفحه‌خوان (screen) پرحجم است و همچنان فقط لاگ/دسته می‌شود.
- **VERIFY** ۵ تست مسیریاب (نوبت→صندوق، پیامِ مخاطبِ شناخته‌شده→پروفایل، گپ→بدونِ سیل،
  مالی→مالی، ترتیبِ classify) + ۲۰ تست قبلی؛ کل بک‌اند ۱۴۴۵ پاس / همان ۱۳ baseline؛
  build سبز. تجربه در experiences/central-signal-dispatcher-over-per-type-wiring.md.

## 2026-07-31 — طبقه‌بندی با مدل + رجیستریِ خودشناسِ مقصدها + وصل‌کردنِ ایمیل‌های بی‌مقصد

- **CHANGE (طبقه‌بندیِ مدلی، با گاردریل)** `classify_signal_smart` در مسیریاب: تصمیم با
  مدل (`inference_gateway.complete`, task=inbox_triage) ولی زیر چهار قید — (۱) نویزِ قطعی
  (اپ‌های آینه‌ای/OTP) هرگز به مدل نمی‌رسد؛ (۲) کشِ درون‌پردازه‌ای روی هشِ متن؛ (۳) سقفِ
  ساعتی `MOBILE_AI_CLASSIFY_PER_HOUR` (پیش‌فرض ۱۲۰)؛ (۴) هر خطا/بی‌کلیدی → قاعدهٔ قطعی.
  مدلِ مردد (confidence<0.5) نمی‌تواند سیگنالِ قطعیِ مالی را نقض کند.
- **CHANGE (رجیستریِ خودشناسِ مقصد)** `inbox_service.FILERS` تک‌نقطهٔ ثبت شد؛
  `INBOX_TARGETS` از آن مشتق می‌شود و `destination_catalog()` تصویرِ زندهٔ مقصدها را
  می‌دهد: فایل‌کننده‌ها + لیست‌های واقعیِ کاربر + صفحه‌های برنامه (از routesMeta).
  پرامپت تریاژ حالا همهٔ مقصدها را می‌بیند (نه فقط ۴ تا) و فیلد `section` هم می‌گیرد.
  دسته‌های مسیریابِ موبایل نیز از همین رجیستری ساخته می‌شوند → **بخش/فایل‌کنندهٔ تازه،
  بدون هیچ ویرایشِ دیگری قابل مسیریابی می‌شود**.
- **FINDING → CHANGE (ایمیل: ۸۳٪ بی‌مقصد)** ممیزی نشان داد `ai_category` ایمیل‌ها هیچ
  مصرف‌کنندهٔ مسیریابی ندارد: فقط سه regex ثابت (بانک/اشتراک/فرد) مسیر می‌دادند و بقیه
  برچسب می‌خوردند و رها می‌شدند. حالا هر ایمیلی که هیچ مسیرِ اختصاصی نگرفت، از همان
  مسیریابِ مرکزی عبور می‌کند (نویزِ خبرنامه/OTP/رسید عمداً مستثنا تا نه سیل شود نه
  دوباره‌کاری)؛ شمارندهٔ `dispatched` به خروجی تریاژ اضافه شد.
- **VERIFY** ۸ تست تازه (برتریِ مدل وقتی مطمئن است، نرسیدنِ نویز به مدل، fallback بی‌کلید،
  ردِ نقضِ سیگنالِ مالی توسط مدلِ مردد، سقف+کش، افزودنِ فایل‌کننده→قابل‌مسیریابی‌شدنِ
  خودکار، زنده‌بودنِ کاتالوگ، مسیریابیِ ایمیلِ بی‌مقصد با نویزگیر) → کل مسیریاب ۳۳ تست؛
  بک‌اند **۱۴۵۳ پاس / همان ۱۳ شکستِ baseline**؛ build سبز.
- **FINDING (باقی‌مانده، مستند برای گام بعد)** تقویم هیچ مسیریابی ندارد (فقط آینه)؛
  تلگرام مغزِ دومقصدیِ خودش را دارد و رسانه‌اش از استخراج‌گر فایل رد نمی‌شود؛ پیوست‌ها
  فقط برای «مالی» خودکار مسیر می‌گیرند و بقیه منتظر تأیید دستی می‌مانند (که برای مدارک
  عمداً درست است، ولی تریاژِ کاملشان اجرا نمی‌شود).

## 2026-07-31 — سه سیم‌کشیِ باقی‌مانده (تقویم/تلگرام/پیوست) + تشخیصِ کانالِ خاموش

- **FINDING (گزارشِ مالک: «رفتم لاگ فعالیت، اعلانِ پیام‌رسان اصلاً ثبت نشده — چرا؟»)**
  مسیرِ سرور سالم است (تست: اعلان → لاگ + مسیریابی). چیزی که ثبت نمی‌شود، اصلاً به سرور
  **نرسیده**: اندروید دسترسیِ NotificationListener را در هر نصبِ دوباره/به‌روزرسانی با
  امضای متفاوت **باطل می‌کند** و سرویس بی‌صدا می‌میرد. مسئلهٔ واقعی این بود که این
  خاموشی **هیچ نشانه‌ای** نداشت — نه در گوشی، نه در برنامه؛ کاربر فقط «هیچ» می‌دید.
- **DECISION** به‌جای حدس‌زدن، رصدپذیری ساخته شد (اصلِ «کانالِ خاموش باید داد بزند»).
- **CHANGE (سرور)** `GET /api/mobile/diagnostics`: برای هر کانال (پیامک/اعلان/تماس/
  صفحه‌خوان/کارکرد/ضربان) تعداد ۲۴ساعت و ۷روز، آخرین زمان، وضعیت (ok/silent/never) و
  راهنمای فارسیِ رفعِ اشکال.
- **CHANGE (برنامه)** `MobileChannelsStrip` روی همان صفحه‌ای که مالک سراغش رفت
  (لاگ فعالیت‌ها): نوارِ جمع‌شونده با ✅/❌ هر کانال + آخرین زمان + راهنما.
- **CHANGE (گوشی)** صفحهٔ اصلیِ همراه حالا وضعیتِ زندهٔ هر مجوز را با ✅/❌ نشان می‌دهد
  (اعلان از `enabled_notification_listeners` خوانده می‌شود، نه از فرضِ نصب)، در
  `onResume` تازه می‌شود، و دکمهٔ «رویدادِ آزمایشی» سرتاسرِ زنجیره را تست می‌کند.
- **CHANGE (تقویم — پایانِ حالتِ آینه)** رویدادهای تازه از مسیریابِ مرکزی می‌گذرند با
  `skip_categories=("appointment",)` (رویداد خودش قرار است؛ کپی در صندوق = شلوغی) و
  `interaction_type="meeting"` → دیدار با فردِ شناخته‌شده روی پروفایلِ او ثبت می‌شود.
  سقف ۲۵ رویداد در هر همگام‌سازی تا اولین sync سیل نسازد؛ کلیدِ ضدتکرار `calendar:<id>`.
- **CHANGE (تلگرام)** متنِ ساده پیش از فلوِ compose از مسیریاب عبور می‌کند (پیامِ بانکی
  در تلگرام حالا به مالی می‌رود و تأییدِ فارسی می‌گیرد)؛ و فایل‌های دریافتی از
  `extract_from_file` عبور می‌کنند — تا امروز رسانهٔ تلگرام هرگز به استخراج‌گر نمی‌رسید.
- **CHANGE (پیوست‌ها)** `_propose` حالا آیتم را برمی‌گرداند و پیشنهادهای task/note از
  تریاژِ صندوق هم عبور می‌کنند و نتیجه **ادغام** می‌شود (نه بازنویسی): تاریخِ سررسید،
  اولویت، نامِ لیست، بخش و نامِ فرد پر می‌شوند؛ ارتقاء فقط note→task/todo/person مجاز است
  تا حدسِ AI نتواند تشخیصِ قطعیِ استخراج‌گر را خراب کند. (تأییدِ دستیِ مدارک عمداً می‌ماند.)
- **CHANGE (مسیریاب، عمومی‌سازی)** `_match_person` علاوه بر دنبالهٔ شماره، با **نام** هم
  تطبیق می‌دهد (طولانی‌ترین تطبیق برنده) — کلیدی است، چون عنوانِ اعلانِ پیام‌رسان نامِ
  مخاطب است نه شماره. پارامترهای `skip_categories`/`interaction_type` اضافه شد، و
  مسیریابیِ فرد **پیش از** بررسیِ skip اجرا می‌شود تا «جلسه با علی» حتی وقتی دستهٔ
  appointment رد می‌شود، پروفایلِ او را تغذیه کند.
- **VERIFY** تست‌های موبایل/مسیریاب به **۳۶** رسید (diagnostics، تطبیقِ نام، تقویم→
  MEETING بدونِ کپیِ صندوق، تلگرام→استخراجِ فایل، ادغامِ تریاژِ پیوست)؛ کل بک‌اند
  **۱۴۵۶ پاس / همان ۱۳ شکستِ baseline**؛ `npm run build` سبز.

## 2026-07-31 — هویتِ اعلانِ گوشی: «کدام اپ / از طرفِ چه کسی»

- **FINDING (گزارشِ مالک)** «بعضی اعلان‌های ثبت‌شده مشخص نیست مالِ کدام اپ یا فرستنده
  بوده‌اند — تبلیغاتی هم شاملش می‌شود؛ فقط محتوا استخراج شده.» بررسی، دو علتِ جدا نشان داد:
  1. **اپ** با نامِ بستهٔ نرم‌افزاری ذخیره می‌شد (`org.telegram.messenger`) — برای کاربر
     یعنی «نامعلوم»، و برای بسته‌های سیستمی/OEM کاملاً بی‌معنا.
  2. **فرستنده** فقط از `android.title` خوانده می‌شد. اعلان‌های تبلیغاتی و سیستمی آن را
     خالی می‌گذارند، و خودِ پیام‌رسان‌ها نامِ مخاطب را جای دیگری می‌گذارند:
     `android.messages` (MessagingStyle) یا `android.conversationTitle`. متنِ کامل هم
     در `android.bigText`/`android.textLines` است نه در `android.text` (که اغلب فقط
     «۳ پیام جدید» است) — یعنی هم فرستنده گم می‌شد هم بخشی از محتوا.
- **CHANGE (گوشی)** شنودگرِ اعلان حالا کاملِ اعلان را می‌خواند: `app_label` (نامِ خواندنیِ
  اپ از خودِ اندروید)، `sender_name` از MessagingStyle، `conversation`، `sub_text`/
  `summary`/`info`، `bigText`، `textLines`، `tickerText`، `category` و `ongoing`.
  ضدِ اکو: اعلانِ «جاری» (پخش موسیقی/دانلود/مسیریاب) هر ثانیه دوباره پست می‌شود؛ تکرارِ
  عینی در ۶۰ ثانیه یک‌بار شمرده می‌شود (چیزی حذف نمی‌شود، فقط اکو).
- **CHANGE (سرور)** سرویس `mobile_identity_service`: `pretty_app` (فهرستِ اپ‌های شناخته‌شده
  + قاعدهٔ عمومی روی نامِ بسته → هرگز خروجیِ خالی)، `resolve_sender` با ترتیبِ اولویتِ صریح
  (MessagingStyle → گفتگو → عنوان → زیرنویس → نامِ اپ) و ردِ عنوان‌های شمارشی («۳ پیام
  جدید» فرستنده نیست)، `notification_label` = «فرستنده · اپ»، و `compose_text` که متن را
  از کامل‌ترین منبع می‌سازد.
- **CHANGE (رکوردهای قدیمی، بدونِ مهاجرت)** `display_entity_label` در `_serialize` لاگ:
  برچسبی که نامِ بسته است، **در لحظهٔ نمایش** خوانا می‌شود. دادهٔ ذخیره‌شده دست‌نخورده
  می‌ماند و در `entity_label_raw` هم برگردانده می‌شود — پس هزاران اعلانِ قبلی هم بدونِ
  هیچ migration و بدون هیچ ریسکِ برگشت‌ناپذیر، درست دیده می‌شوند.
- **CHANGE (مسیریاب)** `dispatch_signal` دو پارامترِ تازه گرفت: `app` (نامِ بسته — چون
  `sender` دیگر نامِ آدم است، تشخیصِ اپ‌های آینه‌ای باید صریح باشد) و `category_hint`.
  دستهٔ اعلامیِ خودِ اندروید (`Notification.CATEGORY_*`) تا امروز دور ریخته می‌شد در حالی
  که از مدل می‌پرسیدیم «این تبلیغ است؟» — حالا حرفِ خودِ اپ مقدم است، ولی **نه** بر OTP و
  **نه** بر سیگنالِ قطعیِ مالی. دستهٔ تازهٔ `system` (اعلانِ جاری/سرویسی) به `_NOISE`
  اضافه شد: ثبت می‌شود، هرگز مسیریابی نمی‌شود.
- **CHANGE (پیامک)** اگر فرستنده مخاطبِ شناخته‌شده باشد، برچسبِ لاگ «نام · شماره» می‌شود
  (مسیریاب حالا `person_name` هم برمی‌گرداند).
- **VERIFY** ۱۱ تستِ تازه (نام‌گذاریِ اپِ شناخته/ناشناخته، ترتیبِ اولویتِ فرستنده، اعلانِ
  تبلیغاتیِ بی‌عنوان، سازگاریِ کلاینتِ قدیمی، ترجیحِ متنِ کامل بر خلاصه، اعلانِ جاری =
  ثبت بدونِ مسیریابی، تشخیصِ آینه از package، عدمِ نقضِ سیگنالِ مالی توسط دستهٔ اندروید،
  خوانا شدنِ رکوردهای قدیمی، نامِ مخاطب در لاگِ پیامک) → ۴۷ تستِ موبایل؛ کل بک‌اند
  **۱۴۶۷ پاس / همان ۱۳ شکستِ baseline**؛ build سبز.

## 2026-07-31 — بازبینیِ کاملِ درخواست‌های پیشین + اصلاحِ یک نقصِ واقعی در تشخیص

- **VERIFY (به‌درخواستِ مالک: «همه‌چیز را یکی‌یکی دقیق‌تر بررسی کن»)** همهٔ درخواست‌های
  قبلی دوباره و با شواهدِ اجرایی بررسی شد، نه با ادعا:
  دیاگرامِ زنده (۲۸۴ گره/۷۸۷ یال؛ `mobile_identity_service` و
  `/api/mobile/diagnostics` **خودکار** روی نقشه ظاهر شدند → قاعدهٔ «هیچ تغییری از
  نقشه جا نمی‌ماند» برقرار است)؛ کشیدن کارت و اتصال دستی (`POST /api/system-map/wires`)؛
  کیتِ ترمیمِ مالی (rebuild-auto-cards / tombstones / owner-accounts / PUT موجودی)؛
  قطعیِ موبایل در **موضوعِ اولِ** گزارشِ روزانه و تکرارِ هشدار تا وصل‌شدن + اعلانِ
  «وصل شد»؛ صفِ آفلاینِ WorkManager؛ بازگشت پس از ریبوت (BootReceiver + کارِ دوره‌ایِ
  ماندگار)؛ تاریخِ تماس از `CallLog.Calls.DATE`؛ مصرفِ واقعیِ دادهٔ موبایل در گزارش
  روزانه/دستیار/`/api/mobile/insights`؛ آرشیوِ درایو؛ مسیریابِ مرکزی و سه سیم‌کشیِ
  تقویم/تلگرام/پیوست. تست‌های هدفمندِ همین حوزه‌ها: ۲۱۰ پاس.
- **FINDING (نقصِ واقعی، در کارِ خودم)** `/api/mobile/diagnostics` نسخهٔ اول وضعیت را
  دوحالته می‌سنجید: `ok if count else silent` روی **تعدادِ کل**. یعنی مجرایی که مدتی
  کار کرده و بعد مرده بود، تا ابد ✅ می‌ماند — **دقیقاً همان خرابی‌ای که این قابلیت
  برای گرفتنش ساخته شده بود را نمی‌گرفت**. ضمناً در گزارشِ قبلی «تعداد ۲۴ساعت/۷روز»
  ادعا شده بود که در کد وجود نداشت.
- **CHANGE (تشخیصِ قطعی به‌جای حدس)** گوشی حالا وضعیتِ زندهٔ پنج دسترسی را همراهِ
  **نبض** می‌فرستد (`Perms.kt` — یک منبعِ واحد برای صفحهٔ اپ و گزارشِ سرور؛ اعلان و
  Accessibility از `Settings.Secure` خوانده می‌شوند چون مجوزِ عادی نیستند). سرور آن را
  در `global_settings` نگه می‌دارد (بدون جدول تازه، با سقفِ ۵۰ دستگاه).
- **CHANGE (وضعیتِ چهارحالته + پنجرهٔ زمانی)** هر مجرا حالا:
  `off` (گوشی گفته دسترسی باطل است — قطعی، بی‌حدس) · `never` (هرگز چیزی نفرستاده) ·
  `silent` (قبلاً می‌فرستاد، در پنجرهٔ خودش ساکت است) · `ok`. اگر خودِ گوشی ساکت باشد،
  مجراها `unknown` می‌شوند تا یک گوشیِ خاموش پنج هشدارِ الکی نسازد. پنجره‌ها با ریتمِ
  هر مجرا تنظیم شد (نبض ۳ساعت، اعلان ۲۴ساعت، کارکرد ۳۶ساعت، پیامک/تماس ۷روز).
  شمارشِ ۲۴ساعت/۷روز/کل اضافه شد.
- **DECISION (کدام ساعت؟)** زنده‌بودنِ مجرا با `created_at` (زمانِ شنیدنِ سرور) سنجیده
  می‌شود نه `occurred_at`؛ وگرنه یک مجرای سالم که دادهٔ قدیمی می‌فرستد (دفترچهٔ تماس،
  صفِ آفلاین) «خاموش» دیده می‌شد. زمانِ خودِ رویداد همچنان به کاربر نشان داده می‌شود.
- **VERIFY** ۵ تستِ تازه — از جمله «دسترسیِ باطل‌شده با وجودِ دادهٔ قدیمی باید `off`
  باشد» و «مجرای کهنه با گوشیِ زنده باید `silent` باشد» (هر دو روی منطقِ قبلی رد
  می‌شدند) → ۵۲ تستِ موبایل؛ کل بک‌اند **۱۴۷۲ پاس / همان ۱۳ baseline**؛ build سبز؛
  vitest همان ۱۵ شکستِ baseline (Header/Footer/Layout/api/Notifications).
- **CHANGE (تجربه)** بندِ اشتباهِ `silent-capture-channel-needs-a-liveness-surface`
  دربارهٔ «کدام timestamp» اصلاح شد و درسِ «وضعیتِ سه‌حالته خودِ قابلیت است، نه جزئیات»
  و «گزارشِ مجوز از حدس بهتر است» به آن اضافه شد.

## 2026-07-31 — حلقهٔ رفعِ ابهام: وقتی موتور شک دارد، **می‌پرسد**

- **DECISION (خواستهٔ مالک)** «ارتباط دوطرفه‌تر شود؛ ابهامِ هوش مصنوعی نه مغفول بماند نه
  اشتباه ثبت شود.» تا امروز خطِ لوله فقط دو انتخاب داشت: حدس بزن، یا رها کن. حالا
  انتخابِ سوم هست: **بپرس**.
- **CHANGE (جدول + سرویس)** `Clarification` + `clarification_service`:
  `ask() → فرمِ تلگرام → پاسخ → تحلیل → ثبت → فیدبک → دوباره‌پرسی`.
  - **هیچ فیلدی هاردکد نیست**: فیلدها هر بار برحسب موضوع با مدل ساخته می‌شوند
    (سقف ۶ فیلد، انواع short/long/choice/date/number/yesno). اگر مدل نبود، یک پرسشِ
    باز جایگزین می‌شود — چون «نپرسیدن» همان مغفول‌ماندنی است که قرار بود حل شود.
    اگر مدل بگوید «ابهامِ واقعی‌ای نیست»، هیچ فرمی ساخته نمی‌شود (سؤالِ الکی ممنوع).
  - **جوابِ نصفه طبیعی است**: فیلدِ خالی «رد» نمی‌شود، «باز» می‌ماند. «نمی‌دانم/بعداً/-»
    هم بی‌جواب حساب می‌شود، نه جواب.
  - **ادغام، نه تکثیر**: سؤالِ تازه دربارهٔ موضوعِ باز به همان فرم اضافه می‌شود؛ تکرارِ
    هم‌معنا (کلیدِ متفاوت، متنِ یکسان) دوباره پرسیده نمی‌شود.
  - **دوباره‌پرسی با backoff فزاینده** (۰/۶/۲۴/۷۲/۱۶۸ ساعت) برای وقتی که پیام دیده نشد
    یا بالا رفت؛ بعد از ۵ تلاش فرم `parked` می‌شود — **حذف نمی‌شود** و در برنامه می‌ماند.
    سؤالِ تازه یک فرمِ parked را دوباره زنده می‌کند.
- **CHANGE (تلگرام)** فرمِ پرشدنی با `force_reply` (کادرِ جواب خودش باز می‌شود). جواب با
  `reply_to_message_id` به فرم گره می‌خورد — پس چند فرمِ باز قاطی نمی‌شوند و فرمی که
  بالا رفته هم همچنان قابلِ جواب است. مسیرِ جواب **قبل از** compose اجرا می‌شود تا
  جوابِ فرم اشتباهاً «کار جدید» نشود. دکمه‌های «⏰ بعداً / 🚫 مربوط نیست» فقط از
  یادآوریِ دوم به بعد می‌آیند (تلگرام force_reply و دکمهٔ شیشه‌ای را در یک پیام نمی‌پذیرد).
  دستور `/questions` + دکمهٔ «❓ پرسش‌های باز» همه را دوباره می‌فرستد بدونِ اینکه
  شمارندهٔ تلاش را بسوزاند — درخواستِ کاربر تلاشِ سیستم نیست.
- **CHANGE (تحلیلِ جواب)** جوابِ آزاد با مدل به فیلدها نگاشت می‌شود (کوتاه/بلند/بی‌قالب/
  خارج از ترتیب)، و یک **مسیرِ قاعده‌ایِ قطعی** زیرش هست (خطوطِ «۱) …» و «برچسب: مقدار»)
  تا بدونِ کلیدِ مدل هم حلقه کار کند.
- **CHANGE (ثبت)** رجیستریِ `_APPLIERS` مثل مسیریابِ سیگنال: `inbox_item` (جواب به محتوا
  اضافه، تریاژِ دوباره، فایل)، `person`، `finance_account`، و catch-all صندوق. جوابِ
  «کدام کارت؟» موجودی را روی همان کارت می‌نشاند و مثل **ورودِ دستیِ مالک** مهر
  `owner_balance_at` می‌خورد — یعنی هیچ سیگنالِ خودکارِ قدیمی‌تری بعداً خرابش نمی‌کند.
- **CHANGE (تولیدکننده‌های پرسش)** دو نقطهٔ واقعی وصل شد: (۱) **مالی** — جایی که چند
  کارت در یک بانک هست و پیام شمارهٔ کارت ندارد، تا امروز سیگنال بی‌صدا **دور ریخته
  می‌شد**؛ حالا با گزینه‌های واقعیِ همان بانک پرسیده می‌شود (و تا جواب نیاید چیزی
  نوشته نمی‌شود — قاعدهٔ قبلی دست‌نخورده). (۲) **مسیریابِ سیگنال** — وقتی مدل زیر ۵۵٪
  مطمئن است، مقصدِ فعلی سرِ جایش می‌ماند (چیزی گم نمی‌شود) و هم‌زمان پرسیده می‌شود.
  افزودنِ نقطهٔ پرسشِ تازه = یک فراخوانِ `ask()`.
- **CHANGE (برنامه)** `GET/POST /api/clarifications/*` + پنل «❓ پرسش‌های من از تو» بالای
  صندوق ورودی در داشبورد (صفحهٔ تازه ساخته نشد). فیلدها از سرور می‌آیند و بر اساس
  `type` رندر می‌شوند، پس هر فرمِ تازه‌ای بدونِ تغییرِ فرانت نمایش داده می‌شود.
- **CHANGE (زمان‌بند)** جابِ `clarifications` هر ۲۰ دقیقه در `jobs_engine.JOBS`.
- **VERIFY** ۲۳ تستِ تازه (ساخت، ضدتکرار، ادغامِ سؤالِ تازه، تکرارِ هم‌معنا، جوابِ نصفه،
  جوابِ بلند/کوتاه، «نمی‌دانم»، جواب با برچسب، جوابِ بی‌قالب، backoff، snooze، park،
  زنده‌شدنِ parked، ثبت در صندوق، نشستنِ موجودی روی کارتِ درست با owner_balance_at،
  صداقتِ فیدبک، رندرِ فرم، و API). کل بک‌اند **۱۴۹۵ پاس / همان ۱۳ baseline**؛ build سبز.
- **VERIFY (نقشهٔ زنده)** هر چهار جزء — router، service، model و job — **خودکار** روی
  دیاگرام ظاهر شدند (۲۸۸ گره / ۸۰۲ یال). هیچ ویرایشِ دستی روی نقشه انجام نشد.

## 2026-07-31 — بازبینیِ حلقهٔ ابهام + پایانِ غرزدنِ «موارد باقی‌مانده» + ویرایشِ جواب

- **FINDING (شکایتِ مالک: «هی می‌زند ۸۰-۱۰۰ فایل منتظر رمز است — آزاردهنده و آشفته»)**
  `notify_locked_digest` هر ۶ ساعت **همان انبارِ ثابت** را دوباره push می‌کرد، برای همیشه.
  بدتر: در شمارش، فایل‌هایی هم می‌آمدند که خودِ سیستم قبلاً «نمی‌ارزد رمزش را بپرسیم»
  تشخیص داده بود (`_is_worthless_locked`) — پس عددِ «۱۰۰» اصلاً واقعی نبود.
- **CHANGE (انضباطِ یادآوری)** سه قاعده: (۱) boilerplateِ بی‌ارزش شمرده نمی‌شود (ولی
  عددِ کلِ صادقانه در پاسخ می‌آید)؛ (۲) اگر **مجموعه عوض نشده باشد** پیام تکرار نمی‌شود
  — انبارِ ثابت خبر نیست، جایش داشبورد است؛ (۳) حتی وقتی عوض شود فاصله فزاینده است
  (۰/۲۴/۷۲ ساعت) و بعد از سه بار ساکت می‌شود تا خودِ مجموعه تغییر کند. فایلِ تازه که
  برسد، **نامِ همان تازه‌ها** گفته می‌شود، نه دوباره کلِ فهرست.
- **FINDING → CHANGE (باگِ واقعی در کارِ خودم: ستونِ JSON ذخیره نمی‌شد)** `record_answers`
  و `edit_answers` روی کپیِ **کم‌عمقِ** `questions` کار می‌کردند و دیکشنری‌ها را درجا
  عوض می‌کردند. SQLAlchemy لیستِ قدیم و جدید را برابر می‌دید و **UPDATE صادر نمی‌کرد**:
  جواب در حافظه بود و در دیتابیس نبود. تست‌های هم‌نشست سبز بودند و واقعیت خالی.
  اصلاح: کپیِ عمیق + `flag_modified` (کمکی `_write_json`)، و یک تستِ تازه که عمداً با
  **نشستِ جدید** می‌خواند تا این طبقه از باگ دوباره پنهان نشود.
- **CHANGE (ویرایشِ جواب — پرسشِ مالک)** حالا از هر دو راه ممکن است: در **تلگرام** همان
  فرم را دوباره ریپلای کن و مقدار را عوض کن؛ در **برنامه** بخشِ «قبلاً جواب گرفته» را
  باز کن و ویرایش کن. مقدارِ خالی در برنامه یعنی «جواب را پس گرفتم» → پرسش دوباره باز و
  پرسیدنی می‌شود. هر ویرایش **دوباره در سیستم ثبت می‌شود** (مثلاً موجودی از کارتِ اشتباه
  به کارتِ درست منتقل می‌شود)، وگرنه مقدارِ غلطِ قبلی جا می‌ماند. تاریخچهٔ ویرایش می‌ماند.
- **CHANGE (فرمِ قابلِ ویرایش + شمارهٔ ثابت)** `render_form` حالا **همهٔ** فیلدها را
  شماره می‌زند و جوابِ قبلی را جلوی خطش می‌نویسد. قبلاً فقط بی‌جواب‌ها شماره می‌خوردند،
  یعنی «۲)» در هر ارسال سؤالِ دیگری بود و جوابِ مالک می‌توانست روی فیلدِ اشتباه بنشیند.
- **CHANGE (میزِ فرمان)** پنلِ پرسش‌ها هر ۶۰ ثانیه خودش تازه می‌شود — پرسش‌ها همان لحظه‌ای
  ساخته می‌شوند که موتور به ابهام می‌خورد، نه با کلیکِ کاربر.
- **FINDING (ایزولاسیونِ تست)** `render_sync_service._owner_id_cache` سراسری است و با هشِ
  توکن کلید می‌خورد؛ دو سوئیت با توکنِ ساختگیِ یکسان ولی مالکِ متفاوت به هم نشت می‌کنند.
  `reset_for_tests()` اضافه شد و یک fixtureِ autouse در conftest کش‌های درون‌پردازه‌ای را
  قبل و بعد از **هر** تست پاک می‌کند.
- **VERIFY** ۱۲ تستِ تازه (۵ برای انضباطِ یادآوری، ۵ برای ویرایش، ۱ برای ماندگاریِ واقعی
  در دیتابیس، ۱ برای فرمِ قابلِ ویرایش) → مجموع ۳۶ تستِ حلقهٔ ابهام؛ build سبز.

## 2026-07-31 — ممیزیِ خصمانهٔ کلِ کارِ این نشست: ۲۸ ایرادِ تأییدشده، همه اصلاح شد

- **DECISION** به‌درخواستِ مالک («یک بار دیگر همه‌چیز را کامل چک کن») یک ممیزیِ
  چندعاملی اجرا شد: ۶ بازبینِ مستقل روی ۶ حوزهٔ جدا، و برای هر حوزه یک بازبینِ
  **خصم** که فرضش «این ادعا غلط است» بود و باید با خواندن/اجرای خودِ کد ردش می‌کرد.
  نتیجه: **۲۸ تأیید، ۳ رد**. آنچه ماند، همه با بازتولیدِ عملی اثبات شده بود.
- **FINDING → CHANGE (مالی، شدید)** `_pick_account` فقط `user_id == uid` را می‌دید،
  در حالی که کارت‌هایی که خوراکِ خودکارِ ایمیل می‌سازد `user_id=None` دارند. یعنی
  **هر پیامکِ بانکی بی‌صدا رد می‌شد** («no confident account match»). با
  `scope_filter` (همان کمکیِ NULL-آگاهی که بقیهٔ خواننده‌های این جدول دارند) درست شد.
- **FINDING → CHANGE (مالی، شدید)** `apply_bank_message` هیچ گاردِ `owner_balance_at`
  و هیچ مقایسهٔ تاریخ نداشت: یک پیامکِ **قدیمی** عددی را که خودِ مالک وارد کرده بود
  پاک می‌کرد و برچسبِ «تنظیم دستی مالک» هم سرِ جایش می‌ماند — برچسب دروغ می‌گفت.
  حالا `occurred_iso` از مسیریاب جلو می‌رود، عددِ دستیِ مالک فقط با سیگنالِ
  **جدیدتر** حرکت می‌کند، عددِ نامثبت رد می‌شود، و برچسبِ منشأ با واقعیت می‌خواند.
- **FINDING → CHANGE (امنیت، شدید)** کلِ `/api/clarifications` `user_id` را می‌گرفت و
  **نادیده می‌گذاشت**: فهرست بدونِ فیلترِ کاربر، و answer/edit/skip/snooze فقط با
  کلیدِ اصلی. یعنی هر کاربرِ واردشده متنِ خامِ پیامکِ بانکیِ دیگری را می‌دید و
  می‌توانست جواب بدهد — و جوابش روی حسابِ مالیِ **او** می‌نشست. `owns()` +
  `_own_or_404` + فیلترِ دامنه اضافه شد (۴۰۴ نه ۴۰۳، تا وجودِ رکورد هم لو نرود).
- **FINDING → CHANGE (تلگرام، شدید)** `self._send` اصلاً وجود نداشت: تأییدِ مسیریابی
  هر بار AttributeError می‌داد، بی‌صدا بلعیده می‌شد و پیام به فلوِ compose می‌افتاد
  → نه تأییدی می‌آمد نه مسیریابی دیده می‌شد، و همان متن دوباره ثبت می‌شد.
- **FINDING → CHANGE (تلگرام، شدید)** مسیرهای retry در `send` (خطای Markdown و ۴۲۹)
  `message_id` برنمی‌گرداندند. یعنی فرمی که پارسِ Markdownش می‌شکست **تحویل** می‌شد
  ولی به رکوردش گره نمی‌خورد → برای همیشه بی‌جواب می‌ماند و جوابِ مالک «کار جدید»
  می‌شد. حالا هر سه مسیر `message_id` می‌دهند.
- **FINDING → CHANGE (تلگرام)** `send_form` بی‌قید `attempts` را بالا می‌برد، حتی وقتی
  ارسال **شکست خورده** بود → فرمی که هرگز تحویل نشده بود بعد از ۵ بار «رهاشده»
  می‌شد. حالا ارسالِ ناموفق تلاش نمی‌سوزاند.
- **FINDING → CHANGE (تلگرام)** هر یادآوری `message_id` را بازنویسی می‌کرد، پس ریپلای
  به نسخهٔ **قبلیِ** فرم شناخته نمی‌شد. حالا علاوه بر تطبیقِ شناسه، از متنِ نقل‌شدهٔ
  ریپلای هم فرم پیدا می‌شود (با نشانهٔ فرم + عنوانِ موضوع، تا ریپلایِ بی‌ربط اشتباه
  گرفته نشود).
- **FINDING → CHANGE (حلقهٔ ابهام، شدید)** اصلاحِ جوابِ «کدام کارت؟» مبلغ را روی کارتِ
  تازه می‌نوشت ولی کارتِ قبلی یک موجودیِ **ساختگی** با مهرِ `owner_balance_at` نگه
  می‌داشت که هیچ سیگنالِ خودکاری هم نمی‌توانست تصحیحش کند. حالا ثبتِ قبلی عقب زده
  می‌شود (موجودی و مهر برمی‌گردند). ضمناً همان گاردهای ارز و عددِ مثبتِ مسیرِ قطعی
  اینجا هم اعمال شد.
- **FINDING → CHANGE (حلقهٔ ابهام، شدید)** `file_answers` idempotent نبود: چون جوابِ
  نصفه طبیعی است، هر جواب یک رکوردِ مقصدِ **تازه** می‌ساخت — دو آیتمِ ناقصِ صندوق،
  یا دو موجودیت از یک آیتم که اولی یتیم می‌ماند. حالا مقصدِ قبلی **به‌روز** می‌شود.
- **FINDING → CHANGE (نویز — شکایتِ مستقیمِ مالک)** جابِ ۳۰دقیقه‌ایِ پیشنهادهای
  زمینه‌ای بدونِ هیچ dedup یا کول‌داونی اعلان می‌داد و ردیف می‌ساخت؛ چون
  تولیدکننده‌هایش قطعی‌اند، یک زمینهٔ ثابت روزی **~۴۸ اعلانِ یکسان** می‌ساخت.
  امضا + کول‌داونِ ۱۲ساعته در global_settings اضافه شد.
- **FINDING → CHANGE (نویز)** `mobile_offline`/`mobile_online` هرگز در
  `EVENT_REGISTRY` ثبت نشده بودند، پس هشدارِ «گوشی قطع شد» **هیچ‌وقت به تلگرام
  نمی‌رسید** — فقط یک ردیفِ زنگوله بود که مالک باید برنامه را باز می‌کرد تا ببیند،
  یعنی دقیقاً ضدِ هدفِ نگهبان. ثبت شد + به کاتالوگِ تنظیماتِ اعلان اضافه شد.
- **FINDING → CHANGE (نویز)** انبارِ **کوچک‌شونده** «تغییر» حساب می‌شد، پس هر فایلی که
  مالک باز می‌کرد شمارنده را صفر می‌کرد و سه یادآوریِ دیگر می‌آورد. حالا فقط
  **رسیدنِ تازه** خبر است.
- **FINDING → CHANGE (موبایل)** گزارشِ کارکرد **تجمعی** است و هر ۱۲ ساعت همان روز
  دوباره می‌آید، ولی هر POST یک ردیف می‌ساخت → زمانِ صفحه و قفل‌گشایی دوبار شمرده
  می‌شد. حالا یک ردیف به‌ازای (روز، دستگاه) که به‌روز می‌شود.
- **FINDING → CHANGE (موبایل)** `device_last_seen` فقط ۵۰۰ ردیفِ آخر را **سراسری**
  می‌خواند، پس گوشیِ مرده پشتِ یک گوشیِ پرحرف از پنجره می‌افتاد، «✅ اتصال برگشت»
  دروغ می‌گرفت و دیگر هرگز «قطع» گزارش نمی‌شد. با `GROUP BY` per-device درست شد.
- **FINDING → CHANGE (مالی/ابهام)** `_ask_which_account` با `institution ==` دقیق
  می‌گشت در حالی که ابهام با تطبیقِ حروف‌کوچکِ زیررشته‌ای اعلام شده بود → گاهی هیچ
  فرمی ساخته نمی‌شد ولی خروجی `asked: True` می‌گفت و سیگنال بی‌صدا گم می‌شد. ضمناً
  `source_ref` شاملِ کلیدِ هر ایمیل بود، پس ادغام هرگز فعال نمی‌شد و یک جاروی صندوق
  ده‌ها فرم می‌ساخت. هر دو اصلاح شد + `asked` حالا راست می‌گوید.
- **FINDING → CHANGE (متفرقه)** فرمتِ مبلغ صفرهای معنادار را می‌خورد (۱٬۲۰۰٬۰۰۰ →
  «1,200,»)؛ `_match_person` بدونِ فیلترِ کاربر و با زیررشتهٔ بی‌مرز تطبیق می‌داد
  («علی» داخلِ «تعالی»)؛ `_strip_prefix` جوابِ «ساعت ۹: قرار با دکتر» را از اولین
  «:» می‌برید؛ `PostWorker` هر ۴xx را برای همیشه retry می‌کرد؛ `CallLogWorker`
  نشانهٔ پیشرفت را حتی وقتی ارسال رد شده بود جلو می‌برد (تماس‌ها برای همیشه گم).
- **VERIFY** ۱۳ تستِ تازه (برگشتِ کارتِ اشتباه، idempotent بودنِ ثبت، نسوختنِ تلاشِ
  ارسالِ ناموفق، جوابِ دارای «:»، مالکیتِ فرم، دسترسیِ پیامک به کارتِ ماشینی، تقدمِ
  عددِ دستیِ مالک، ردِ عددِ نامثبت، به‌روزرسانیِ کارکردِ روز، گوشیِ مرده پشتِ گوشیِ
  پرحرف، مرزِ واژه در نام). کل بک‌اند **۱۵۱۶ پاس / همان ۱۳ baseline**؛ build سبز؛
  نقشهٔ زنده ۲۸۸ گره / ۸۰۳ یال.

## 2026-07-31 — بازسازیِ فرمِ پرسش پس از بازخوردِ مالک (تصویرِ فرمِ واقعی)

- **FINDING (مالک تصویرِ فرمِ واقعی را فرستاد: «مبهم، بد، آماتور — اصلاً نمی‌فهمم چه
  می‌پرسد»)** سه ایرادِ مستقل، همه در تصویر پیدا:
  1. **موضوع بی‌معنا بود**: «Project_manager: Project_manager 📎
     scan_bundle_c9e90b2b-4141-4012-…​.pdf» — نامِ فرستنده دوبار، یک UUID، پسوندِ فایل.
     بدتر: مدل هم با همین متنِ بی‌معنا سؤال می‌ساخت، پس **سؤال‌ها هم بی‌معنا** می‌شدند.
  2. **Markdown شکسته بود**: نامِ فایل پر از «_» است و تلگرام آن را ایتالیک تفسیر کرد،
     پس جمله به‌هم ریخت. (ممیزیِ قبلی این ریسک را دیده بود ولی من فقط نیمی‌اش را —
     برگرداندنِ message_id — درست کرده بودم، نه خودِ escape را.)
  3. **پرسش‌ها کارِ خودِ موتور را به مالک واگذار می‌کردند**: «این ورودی را کجا ثبت کنم؟
     سند/کار/یادداشت/تراکنش» — دسته‌بندی کارِ سامانه است، نه مالک.
- **CHANGE (موضوعِ خوانا)** `humanize_topic` توکن‌محور: UUID/هش/عددِ بلند، واژه‌های
  نویزی (scan/bundle/img/untitled) و تکرارِ پشت‌سرهم حذف می‌شوند و نوعِ فایل به فارسی
  می‌آید («PDF: Project manager»). سرِ **ساخت** اعمال می‌شود نه نمایش، تا سؤال‌سازِ مدل
  هم متنِ تمیز ببیند.
- **CHANGE (HTML به‌جای Markdown)** `send()` پارامترِ `parse_mode` گرفت و فرم با HTML و
  `html.escape` می‌رود — هیچ نامِ فایل یا متنِ کاربری دیگر نمی‌تواند قالب را خراب کند.
  فرم حالا تیترِ درشت، نقلِ‌قولِ متنِ اصلی (`<blockquote>`)، خطِ پیشرفت («۲ از ۳ پاسخ
  داده شده») و زیرنویسِ «چرا می‌پرسم» زیرِ هر پرسش دارد.
- **CHANGE (ابزارهای واقعیِ تلگرام)** کیبوردِ شیشه‌ای: پرسشِ گزینه‌ای/بله‌خیر به **دکمهٔ
  یک‌ضربه‌ای** تبدیل شد (`clar:pick:<id>:<field>:<option>` → `answer_field` ثبت می‌کند،
  فیدبک می‌دهد و فرمِ به‌روز را دوباره می‌فرستد)، به‌علاوهٔ «❓ سؤال دارم»، «📄 نمایش
  دوباره»، «⏰ بعداً»، «🚫 مربوط نیست». اگر هیچ فیلدِ دکمه‌ای نبود، `force_reply` می‌آید
  (تلگرام هر دو را در یک پیام نمی‌پذیرد).
- **CHANGE (گفتگوی دوطرفه — خواستهٔ صریحِ مالک)** ستونِ `discussion` (+ alembic 0056 +
  ALTER راه‌اندازیِ idempotent) و تابعِ `discuss()`: مالک «❓ سؤال دارم» را می‌زند،
  سؤالش با **کلِ زمینه** (موضوع، متنِ اصلی، پرسش‌ها، جواب‌های داده‌شده و نخِ گفتگوی
  تا اینجا) به مدل می‌رود، جوابِ کوتاهِ فارسی می‌گیرد، و **بلافاصله خودِ فرم دوباره
  فرستاده می‌شود** — یعنی هرچند بار هم که رد و بدل شود، موضوع و پرسش‌های اصلی گم
  نمی‌شوند. حالتِ گفتگو تا جواب‌دادن به فرم (یا /cancel) باز می‌ماند و هر دور،
  شمارندهٔ یادآوری را صفر می‌کند تا وسطِ گفتگو «هنوز جواب نگرفتم» نیاید.
- **CHANGE (پرامپتِ سؤال‌سازی، سخت‌گیرانه)** صریحاً ممنوع شد: پرسیدنِ مقصد/دسته از
  مالک، «عنوان کوتاه چیست؟»، ارجاعِ مبهم به «این مورد». هر پرسش باید مستقل و
  قابل‌فهم باشد، `why` اجباری شد (مالک باید بداند چرا وقت می‌گذارد)، و اگر متن آن‌قدر
  بی‌محتواست که پرسشِ معنادار نمی‌شود ساخت، **هیچ فرمی ساخته نمی‌شود**.
- **CHANGE (برنامه)** همان گفتگو در پنلِ داشبورد: نخِ پرسش‌وپاسخ + کادرِ «❓ سؤال دارم»
  و endpointِ `POST /api/clarifications/{id}/discuss`.
- **VERIFY** ۷ تستِ تازه (خوانا شدنِ موضوعِ نامِ‌فایلی، دست‌نخوردنِ موضوعِ فارسیِ واقعی،
  escapeِ HTML در برابر محتوای خصمانه، تبدیلِ گزینه‌ها به دکمه، ثبتِ یک‌ضربه‌ای، چند دورِ
  پرسشِ متقابل بدونِ گم‌شدنِ پرسش‌های اصلی، endpointِ گفتگو) → ۴۱ تستِ حلقهٔ ابهام؛
  کل بک‌اند **۱۵۲۳ پاس / همان ۱۳ baseline**؛ build سبز.

## 2026-07-31 — ممیزیِ دومِ فرم: ۱۴ ایرادِ تأییدشده (۲ بحرانی)، همه اصلاح شد

- **FINDING (بحرانی، ساختهٔ خودم یک ساعت قبل)** `edit_message_text` به `parse_mode`
  ارجاع می‌داد که **پارامترش نبود** — چون هنگام افزودنِ parse_mode به `send`، رشتهٔ
  `"parse_mode": "Markdown"` را سراسری جایگزین کردم و این تابع هم گرفتار شد. هر
  فراخوانیِ واقعی `NameError` می‌داد: پیامِ وضعیتِ compose هرگز به‌روز نمی‌شد و همان
  متن **دو بار** ثبت می‌شد. تست‌ها نمی‌گرفتند چون بدونِ توکن، تابع زودتر برمی‌گشت.
- **FINDING (بحرانی)** حالتِ «سؤال دارم» قبل از همه‌چیز اجرا می‌شد و `/cancel`،
  `/help`، دکمه‌های کیبورد و **حتی جوابِ واقعیِ فرم** را می‌بلعید. یعنی مالک بدونِ
  راهِ خروج در گفتگو گیر می‌کرد و جوابش هم ثبت نمی‌شد. حالا بعد از مسیرِ ریپلای و
  نگاشتِ دکمه‌ها اجرا می‌شود، دستور و ریپلای خودشان حالت را می‌بندند، و مهلتِ حالت
  در هر دور تازه می‌شود تا وسطِ فکرکردن نیفتد.
- **FINDING (شدید) `humanize_topic` ارقام را می‌خورد.** روی متنِ آدمیزاد هم اجرا
  می‌شد: «موجودیِ 1,000,000 IRR» → «موجودیِ 1 000 IRR» (هزار برابر کمتر!)،
  «فاکتور شماره 1002345» شماره‌اش را از دست می‌داد، شمارهٔ تلفن حذف می‌شد. حالا اول
  تشخیص می‌دهد رشته «نامِ فایل/شناسه» است یا جمله؛ تمیزکاریِ تهاجمی فقط برای حالتِ
  اول. متنِ فارسیِ واقعی **عیناً** می‌ماند (۶ تستِ پارامتری).
- **FINDING (شدید) بازگردانیِ کارتِ اشتباه شکست می‌خورد** وقتی `c.result` دو ثبت برای
  همان کارت داشت: حلقه به‌ترتیب بازپخش می‌کرد و آخرین `prev_balance` (که خودش عددِ
  ساختگی بود) برنده می‌شد. حالا فقط **قدیمی‌ترین** عکسِ هر کارت، یک‌بار، بازپخش
  می‌شود و ردیفِ تکراری برای یک کارت ساخته نمی‌شود.
- **FINDING (شدید) `clar:pick` با هر ضربه یک فرمِ تازه می‌فرستاد** (۲N-۱ پیام برای N
  پرسش) و نسخه‌های قدیمی با دکمه‌های زنده می‌ماندند. حالا فرم **در جای خودش**
  ویرایش می‌شود و فیدبک فقط در پایان می‌آید.
- **FINDING (شدید) گیتِ امنیتیِ callback** شرطِ `and chat_id` داشت، پس یک callbackِ
  بدونِ پیام (chat_id خالی) از گیت رد می‌شد و می‌توانست جوابِ فرم بنویسد. اصلاح شد.
- **FINDING (شدید) `/api/clarifications/resend` بدونِ scope** بود: فرم‌های **همهٔ**
  کاربران را دوباره می‌فرستاد و تغییر می‌داد. `resend_all` حالا `user_id` می‌گیرد.
- **FINDING (متوسط)** انتخابِ حساب از **اولین فیلدِ جواب‌داده** انجام می‌شد، پس هر
  فیلدِ دیگری که زودتر جواب می‌گرفت آن را می‌ربود و موجودی بی‌صدا نوشته نمی‌شد؛ حالا
  فیلد با کلیدِ صریحِ `target["field"]` پیدا می‌شود.
- **FINDING (متوسط)** `render_form` بودجهٔ طول نداشت؛ لایهٔ ارسال وسطِ یک تگِ HTML
  می‌بُرد، تلگرام پیام را رد می‌کرد و نسخهٔ بی‌قالب می‌رفت (تگ‌های خام + گم‌شدنِ
  راهنما). حالا خودِ فرم بودجه دارد: اول «چرا»ها، بعد نقلِ‌قول حذف می‌شوند و راهنما
  هرگز قربانی نمی‌شود.
- **FINDING (متوسط)** مهاجرت ۰۰۵۶ گارد نداشت، پس روی دیتابیسی که ALTERِ راه‌اندازی
  ستون را ساخته بود، `alembic upgrade head` با DuplicateColumn می‌افتاد. مثلِ
  ۰۰۵۲/۰۰۵۳ گارد شد.
- **FINDING (کم)** دو assertion در تست‌های تازه **تاتولوژیک** بودند (هرگز نمی‌توانستند
  رد شوند) و `GET /api/clarifications` با توکنِ جعلی ۲۰۰ می‌داد به‌جای ۴۰۱ (برخلافِ
  شش endpointِ همسایه). هر دو اصلاح شد.
- **VERIFY** ۱۰ تستِ تازه — از جمله «`edit_message_text` payload معتبر می‌سازد»،
  «دستور و دکمه همیشه از گفتگو خارج می‌شوند»، «۶ موضوعِ واقعی دست‌نخورده می‌مانند»،
  «اصلاحِ دوباره هم موجودیِ اصلی را برمی‌گرداند»، «فرمِ غول‌پیکر در سقفِ تلگرام جا
  می‌شود و راهنمایش را نگه می‌دارد». کل بک‌اند **۱۵۳۳ پاس / همان ۱۳ baseline**؛
  build سبز.

## 2026-07-31 — پروفایلِ هویتِ مالک + ردیابیِ موقعیت و الگوهای رفت‌وآمد

- **FINDING → CHANGE (نشتِ داده)** مسیرِ ثبتِ Emirates ID سه فیلد
  `date_of_birth`/`sex`/`nationality` را می‌گرفت و **دور می‌ریخت** — یعنی تاریخِ تولدی
  که مالک وارد کرده بود هرگز ذخیره نمی‌شد و هیچ‌جای برنامه سن را نمی‌دانست. ستون‌ها
  اضافه و ذخیره شدند (+ ALTER راه‌اندازی + alembic 0057). ضمناً `issue_place` دیگر
  دوپهلو نیست (قبلاً هم «امارتِ صدور» و هم «ملیت» در همان ستون می‌نشست).
- **CHANGE (پروفایلِ هویت)** جدولِ `owner_identity_fields` — **یک ردیف به‌ازای هر فیلد**،
  نه یک بلابِ JSON، چون هر فیلد باید منبع، درجهٔ اطمینان و «حرفِ آخرِ مالک» جدا داشته
  باشد. سرویس `owner_identity_service` با رجیستریِ استخراج‌کننده‌ها (نام از گواهینامه/
  کارت، تاریخ تولد از گواهینامه یا واقعیتِ رمزنگاری‌شده، سن مشتق، ملیت، محلِ کار از
  کفیلِ ویزا، شغل، محلِ زندگی از مکانِ کشف‌شده، قوت از شاخصِ پشتکار، **ضعف از لیست‌های
  خودِ مالک** «عادت‌های بد»/«دزدان انرژی و زمان»، علایق و شخصیت از خودمدلی).
  `owner_locked`: هر فیلدی که مالک ویرایش کند، استخراجِ خودکار دیگر رویش نمی‌نویسد.
  **محلِ تولد عمداً استخراج‌کننده ندارد** — هیچ منبعِ قابل‌اعتمادی نیست و حدس بدتر از
  پرسیدن است، پس فقط پرسیده می‌شود.
- **CHANGE (اتصال به حلقهٔ ابهام)** فیلدِ خالیِ مهم → فرمِ تلگرام با `source_ref` یکتا
  (پس هرگز دو بار پرسیده نمی‌شود)؛ جواب مستقیم در همان فیلد می‌نشیند و قفل می‌شود.
- **CHANGE (صفحه)** `/identity-profile` — هر فیلد با مقدار، **منبعش** («این را از کجا
  آوردی؟»)، درجهٔ اطمینان، و ویرایشِ درجا. ثبت در `routesMeta` (یک رجیستری، یک حقیقت).
- **CHANGE (موقعیت مکانی)** چهار جدول: `places` (مکانِ تکرارشونده)، `place_visits`
  (یک بارِ حضور)، `place_trips` (جابه‌جایی)، `route_patterns` (الگو). `user_locations`
  دو ستونِ `device`/`speed_kmh` گرفت — چون مالک چند گوشی دارد و باید معلوم باشد **با
  کدام** جابه‌جا شده. endpointِ `POST /api/mobile/location` بسته‌ای می‌گیرد (نه تک‌نقطه)
  با ضدتکرارِ (دستگاه، زمان)، چون صفِ آفلاین ممکن است بسته را دوباره بفرستد.
- **CHANGE (خوشه‌بندی، بدونِ وابستگیِ تازه)** `place_service`: نقاطِ خام → «توقف» (شعاع
  + حداقل زمان) → مکان (نزدیک‌ترین با هاورساین) → سفر. خانه/محلِ کار از **هیستوگرامِ
  ساعت** حدس زده می‌شود و فقط وقتی برنده **آشکارا** جلوتر است؛ تساوی یعنی مبهم و
  مبهم پرسیده می‌شود، نه اینکه برچسبِ غلط بخورد.
- **CHANGE (قیدِ اصلیِ مالک: «مسیرِ کشف‌شده دیگر پرسیده نشود»)** هر سفر یک
  `pattern_key` = مبدأ:مقصد:روزِ هفته:بازهٔ سه‌ساعته می‌سازد (بازهٔ سه‌ساعته عمدی است تا
  ۷:۴۰ و ۸:۲۰ یک الگو باشند). با ۳ تکرار الگو **آموخته** می‌شود؛ از آن به بعد سفرِ
  منطبق سکوت است و **فقط** سفرِ نامنطبق «خلافِ الگو» علامت می‌خورد و تنها همان پرسیده
  می‌شود. جوابِ مالک هم علامتِ غیرعادی را برمی‌دارد.
- **CHANGE (گوشی)** `LocationWorker`: هر ۱۵ دقیقه، با **بافرِ محلی** — نقطه‌ها قبل از
  ارسال ذخیره می‌شوند، پس قطعیِ طولانیِ اینترنت داده را از بین نمی‌برد و با برگشتنِ نت
  همه یک‌جا سینک می‌شوند. مجوزهای موقعیت (+ پس‌زمینه) و POST_NOTIFICATIONS اضافه شد.
- **CHANGE (هشدارِ خاموشیِ موقعیت — با اینترنتِ قطع هم کار می‌کند)** اگر اجازه نباشد یا
  GPS خاموش باشد، **خودِ گوشی** اعلانِ محلی می‌دهد (کول‌داونِ ۶ ساعته تا آزار نشود) —
  چون هشدارِ سرور در قطعیِ نت اصلاً نمی‌رسد. سمتِ سرور هم رویدادِ `location_off` ثبت و
  به تلگرام وصل شد، و «موقعیت مکانی» به تشخیصِ کانال‌ها و نگهبانِ اتصال اضافه شد.
- **CHANGE (اتصال به تحلیل‌ها)** مکان‌ها و سفرها در گزارشِ روزانه و زمینهٔ دستیار
  می‌آیند، و پروفایلِ هویت هم — تا این داده‌ها هم مثل بقیه جزیره نمانند. دو جابِ تازه
  در `jobs_engine`: `places` (ساعتی) و `owner_identity` (۱۲ساعته).
- **VERIFY** ۲۰ تستِ تازه — از جمله «تکرارِ مسیر آموخته می‌شود و دیگر غیرعادی نیست»،
  «فقط سفرِ غیرعادی پرسیده می‌شود»، «تساوی شب/کار یعنی نمی‌دانم»، «ورودِ دوباره تکرار
  نمی‌سازد»، «کدام گوشی جابه‌جا شد»، «ویرایشِ مالک از استخراجِ دوباره جان سالم به‌در
  می‌برد»، «محلِ تولد حدس زده نمی‌شود بلکه پرسیده می‌شود»، «خانهٔ کشف‌شده می‌شود محلِ
  زندگی در پروفایل». کل بک‌اند **۱۵۵۳ پاس / همان ۱۳ baseline** (دو اجرای پیاپی)؛
  build سبز.

## 2026-07-31 — ردیابیِ دقیقِ مسیر (سرویسِ پیش‌زمینه)

- **DECISION (به‌درخواستِ مالک)** کارِ دوره‌ای اندروید کمتر از ۱۵ دقیقه اجرا نمی‌شود، پس
  مسیرِ خیابان‌به‌خیابان با WorkManager درنمی‌آید. تنها راهِ مجاز و پایدار یک
  **Foreground Service** با اعلانِ دائمی است. اعلان اجبارِ اندروید است و صادقانه هم
  هست: مالک همیشه می‌بیند ردیابی روشن است و با یک ضربه خاموشش می‌کند.
- **CHANGE** `LocationTrackingService`: هر ۱۵ ثانیه/۲۰ متر یک نقطه، با فیلترِ نویز
  (جابه‌جاییِ کمتر از ۱۲ متر «حرکت» نیست — وگرنه یک شبِ خواب هزاران نقطهٔ بی‌فایده از
  لرزشِ GPS می‌ساخت). **اختیاری و پیش‌فرض خاموش** چون باتری می‌برد؛ دکمه‌اش در اپ
  وضعیت را نشان می‌دهد و `START_STICKY` + `startIfEnabled` بعد از ریبوت برش می‌گرداند.
- **FINDING → CHANGE (هم‌روندی)** حالا **دو** نویسنده روی بافرِ نقاط دارند (سرویسِ دقیق
  و کارگرِ دوره‌ای). الگوی «بخوان-عوض‌کن-بنویس» روی SharedPreferences بدونِ قفل یعنی
  نقطه‌ها بی‌سروصدا گم می‌شوند. بافر به `LocationBuffer` منتقل شد: همه‌چیز پشتِ یک قفل،
  و `drain` یک عملِ **اتمی** است تا نقطهٔ تازه‌رسیده بینِ خواندن و پاک‌کردن قربانی نشود.
  وقتی سرویسِ دقیق روشن است، کارگر نقطهٔ خودش را اضافه نمی‌کند و فقط بافر را خالی می‌کند.
- **CHANGE** پرچمِ `precise` در payload تا در لاگ معلوم باشد نقطه از کدام حالت آمده؛
  مجوزهای `FOREGROUND_SERVICE`/`FOREGROUND_SERVICE_LOCATION` و
  `foregroundServiceType="location"` (اجباریِ اندروید ۱۴) اضافه شدند.
- **VERIFY** ۵ تستِ تازه: ذخیرهٔ دستهٔ متراکم با برچسبِ حالت، **عدم تکرار وقتی صفِ
  آفلاین بسته را دوباره می‌فرستد**، گیتِ توکن، خبردادنِ خاموشی با بستهٔ خالی، و اینکه
  ۴۰ نقطهٔ متراکم باید **یک** مکان بسازد نه چهل تا → ۲۵ تستِ مکان/هویت؛ کل بک‌اند
  **۱۵۵۷ پاس**؛ build سبز.
- **FINDING (باز، در حالِ بررسی)** `test_dev_sync::test_error_issues_flow` گاهی با
  ترتیبِ تصادفیِ اجرا رد می‌شود. اصلاحِ قبلی (پاک‌کردنِ کشِ سراسری) کافی نبوده. این
  «baseline» نیست — تستی که گاهی رد شود یعنی سبزبودنِ سوئیت قابل اعتماد نیست؛ در حالِ
  بازتولید و ریشه‌یابی است.

## 2026-07-31 — ریشه‌یابیِ تستِ ناپایدار (و اینکه فرضیهٔ اولم غلط بود)

- **FINDING** `test_dev_sync::test_error_issues_flow` گاهی رد می‌شد. بارِ اول حدس زدم
  وابسته به **ترتیب** است و یک نشتِ کشِ سراسری را بستم؛ آن یک نشتِ واقعی بود ولی
  **علتِ این تست نبود** — چون بعدش هم برگشت.
- **VERIFY (اندازه‌گیری به‌جای حدس)** اجرای همان تست **به‌تنهایی و با ترتیبِ ثابت**،
  ۲۵ بار: **۴ شکست**. پس اصلاً ترتیبی نبود؛ **مسابقهٔ زمانی** بود.
- **ROOT CAUSE** `_now_iso()` در fake تستی `datetime.now()` را به ثانیه گرد می‌کرد.
  شناسهٔ هر ردیفِ لاگ `md5(service|timestamp|message)` است. مرحلهٔ آخرِ تست «همان
  لاگ‌ها» را دوباره می‌گیرد تا ثابت کند تکراری‌اند و مسئلهٔ حل‌شده را باز نمی‌کنند —
  ولی اگر آن فراخوانی به **ثانیهٔ بعد** می‌افتاد، fake یک خطِ لاگِ واقعاً **تازه**
  می‌ساخت، کدِ محصول درست عمل می‌کرد و مسئله را دوباره باز می‌کرد، و تستِ بی‌گناه
  رد می‌شد. احتمالِ شکست = احتمالِ عبور از مرزِ ثانیه ≈ ۱۶٪ — دقیقاً ۴ از ۲۵.
- **CHANGE** زمانِ fake در هر تست **یک بار** ثابت می‌شود (در fixture ریست می‌شود):
  fakeی که ادعا می‌کند «همان لاگ‌ها» را می‌دهد، باید واقعاً همان‌ها را بدهد.
  کدِ محصول دست‌نخورده ماند — رفتارش (بازکردنِ دوبارهٔ مسئله وقتی خطا **دوباره**
  رخ می‌دهد) درست است.
- **VERIFY** بعد از اصلاح: **۰ شکست در ۳۰ اجرا**؛ کل بک‌اند **۱۵۶۰ پاس / ۱۳ baseline**
  و `dev_sync` دیگر بینشان نیست.

## 2026-07-31 — پیش‌نیازهای واقعیِ «همیشه روشن» روی اندروید

- **FINDING** پرسشِ مالک («بعد از خاموش/روشنِ گوشی خودش در حالتِ همیشه می‌مانَد؟»)
  دو شکافِ واقعی را آشکار کرد: `ACCESS_BACKGROUND_LOCATION` در مانیفست **اعلام** شده
  بود ولی هرگز در زمانِ اجرا **خواسته نمی‌شد** (از اندروید ۱۰ باید جدا خواسته شود و
  از ۱۱ اصلاً دیالوگ ندارد)، و هیچ درخواستی برای معافیت از بهینه‌سازیِ باتری نبود —
  یعنی سازنده (مخصوصاً سامسونگ) سرویس را می‌کشت و «همیشه روشن» عملاً دروغ بود.
- **CHANGE (گوشی)** دکمهٔ «اجازهٔ همیشه + آزادسازی از باتری» که مرحله‌به‌مرحله راه را
  باز می‌کند (اندروید ۱۰: درخواستِ جدا؛ ۱۱+: صفحهٔ تنظیماتِ اپ)، و دو خطِ وضعیت که
  صریح می‌گویند «فقط موقعِ استفاده» یعنی با بسته‌شدنِ اپ ثبت قطع می‌شود.
- **CHANGE (سرور)** وضعیتِ تازهٔ `partial` در تشخیصِ کانال‌ها: مجرا روشن است ولی
  پیش‌نیازهای پایداری نیستند. بدونِ این، مالک «✅ فعال» می‌دید و بعد ناگهان سکوت —
  همان ابهامی که کلِ صفحهٔ تشخیص برای حذفش ساخته شد.
- **VERIFY** ۲ تستِ تازه (partial وقتی پیش‌نیاز نیست، ok وقتی همه هست) → ۵۷ تستِ موبایل.

## 2026-08-01 — «۱۳ خرابیِ پایه» تمام شد: هیچ‌کدام باگِ محصول نبود، دو تایشان پوششِ قلابی بودند

- **FINDING (چارچوب)** سوئیت مدت‌ها با «۱۳ failed, ۱۵۶۰ passed» زندگی می‌کرد و آن ۱۳
  تا «baseline» صدا زده می‌شد. این خودش یک نقص است: دروازهٔ ادغام عملاً از «هیچ خرابی»
  به «خرابیِ **جدید** نباشد» تنزل کرده بود. بدتر اینکه تستی که در baseline نشسته
  فقط «اجرا نشده» نیست — می‌تواند فعالانه دربارهٔ پوشش **دروغ** بگوید.
- **VERIFY** هر ۱۳ مورد یکی‌یکی ریشه‌یابی شد. نتیجهٔ صادقانه: **هیچ‌کدام باگِ محصول
  نبود** — کلِ diff فقط `tests/` است و یک خط از `app/` عوض نشد. ولی «فقط تست بود»
  یعنی «بی‌اهمیت» نیست؛ دو موردش جدی‌تر از باگِ محصول بود.
- **FINDING (جدی — پوششِ قلابی روی گیتِ ادمین)**
  `test_role_change_endpoint_rejects_non_admin` ادعا می‌کرد گیتِ ادمینِ
  `POST /admin/approve-user/{id}` را تضمین می‌کند، ولی handler را **مستقیم** صدا
  می‌زد. گیت در **امضای** تابع است (`Depends(get_current_admin_user)`) نه در بدنه‌اش،
  پس با فراخوانیِ مستقیم اصلاً اجرا نمی‌شد و تست همیشه روی `db=None` می‌مرد. جفتش
  (`..._allows_admin`) هم سبز بود ولی چیزی از **اجازه** ثابت نمی‌کرد. **اثبات:** گیت
  را کاملاً حذف کردم — سوئیت همچنان سبز ماند. یعنی این مسیرِ تغییرِ نقش/دسترسی عملاً
  **بدونِ هیچ پوششِ مؤثری** بود.
- **CHANGE** هر سه تست از راهِ HTTP روی روترِ mount‌شده اجرا می‌شوند (به‌علاوهٔ حالتِ
  ناشناس ⇒ ۴۰۱). **تستِ جهش:** با حذفِ گیت، حالا قرمز می‌شود.
- **FINDING (پوششِ قلابی روی وابستگیِ سختِ هویت)** پنج تستِ
  `test_auth_required_user_id_9a5a3b4d` به `auth_deps.AuthService` monkeypatch می‌زدند
  و `get_required_user_id(credentials=…)` صدا می‌زدند — هر دو مدت‌ها پیش با refactor
  به `(request, db)` + `validate_token` عوض شده بودند. هر پنج تست در setup می‌مردند:
  **API را pin کرده بودند، نه رفتار را**.
- **CHANGE** با `starlette.Request` واقعی (هدر **و** کوکیِ `access_token`) و یک
  DBِ بدلیِ کوچک بازنویسی شدند؛ سه حالتِ تازه هم اضافه شد (توکن در کوکی، امضای درست
  ولی کاربرِ حذف‌شده، انفجارِ DB روی مسیرِ نرم). **تستِ جهش:** با برگرداندنِ آسیب‌پذیریِ
  اصلی (تنزلِ بی‌صدای توکنِ نامعتبر به scope ناشناس) ۹ تست قرمز می‌شود.
- **FINDING (۴۰۱ در برابر ۴۰۳)** چهار تست برای درخواستِ **بدونِ توکن** انتظارِ ۴۰۳
  داشتند و محصول ۴۰۱ می‌داد. اینجا محصول درست است و تست کهنه: ۴۰۱ یعنی «هویتی ندادی»
  (+ هدرِ `WWW-Authenticate`)، ۴۰۳ یعنی «هویت دادی ولی اجازه نداری». شاهدِ قاطع:
  در همان فایل‌ها تستِ «کاربرِ لاگین‌کرده ولی غیرادمین ⇒ ۴۰۳» **سبز** بود، و
  interceptor فرانت‌اند (`lib/api.js`) پاک‌کردنِ توکنِ کهنه را فقط روی **۴۰۱** انجام
  می‌دهد — با ۴۰۳ توکنِ مرده تا ابد در localStorage می‌ماند. تست‌ها به ۴۰۱ اصلاح شدند
  و حالا هدرِ چالش را هم می‌سنجند.
- **FINDING (تغییرِ عمدیِ محصول)** `/auth/google` دیگر ۳۰۲ به صفحهٔ رضایتِ کلاسیک
  نمی‌دهد؛ عمداً به صفحهٔ Google Identity Services تبدیل شده تا بدونِ
  `GOOGLE_REDIRECT_URI` کار کند (در docstring خودش نوشته است). ظرفیت حذف نشده —
  `/auth/google/callback` سرِ جایش است. تست به قراردادِ زنده هم‌تراز شد.
- **FINDING (بمبِ ساعتی)** `test_google_sync::test_google_routes_flow` نه به‌خاطرِ کد،
  که به‌خاطرِ **تاریخ** قرمز بود: fixture پیام‌ها را روی `NOW = 2026-07-19` ثابت
  می‌کرد و روتِ `/api/google/emails` روی پنجرهٔ **غلتانِ ۷ روزه با ساعتِ واقعی** فیلتر
  می‌کند. یعنی این تست از ۲۶ تیر خودبه‌خود قرمز شده بود بدونِ آنکه کسی چیزی عوض کند.
  تاریخِ داده‌ها نسبت به «الان» شد. (هم‌خانوادهٔ همان ریشه‌ای که تستِ ناپایدارِ
  `dev_sync` را قرمز می‌کرد: ناهماهنگیِ ساعتِ تست و ساعتِ محصول.)
- **VERIFY** برای پیداکردنِ بمب‌های بعدی، کلِ سوئیت یک بار با ساعتِ **۱۲۰ روز جلوتر**
  اجرا شد (افزونهٔ یک‌بارمصرفِ pytest، در repo نیست).
- **DECISION** از این پس «baseline قرمز» پذیرفته نیست: دروازهٔ ادغام یعنی **صفر خرابی**.
- **EXPERIENCE** `experiences/tests-that-cannot-fail-the-red-baseline.md`.

## 2026-08-01 — جاروی «بمبِ ساعتی»: نتیجهٔ اجرای سوئیت با ساعتِ آینده

- **VERIFY** کلِ سوئیت با ساعتِ **۱۲۰ روز جلوتر** اجرا شد. اجرای اولِ کاوشگر
  **۶۰ خرابی** داد که بیشترشان **تقصیرِ خودِ کاوشگر** بود: با جانشین‌کردنِ کلاسِ
  `datetime`، بررسی‌های `isinstance(x, datetime)` در کتابخانه‌ها False می‌شدند
  (python-jose دیگر `exp` را تبدیل نمی‌کرد و json می‌ترکید). درسِ خودِ ابزار:
  مقدارِ برگشتی باید نمونهٔ کلاسِ **جانشین** باشد نه کلاسِ اصلی. بعد از اصلاح:
  **۱۳ خرابی**.
- **VERIFY (تفکیکِ صادقانه)** آن ۱۳ تا یکی‌یکی بررسی شد؛ **۱۲ تا آرتیفکتِ کاوشگر
  بودند نه بمبِ واقعی**، به دو دلیلِ مشخص:
  - تست از `date.today()` می‌خواند (که عمداً جابه‌جا نشد، چون `datetime` زیرکلاسِ
    `date` اصلی است و جانشین‌کردنش همان تلهٔ isinstance را برعکس می‌سازد) در حالی
    که محصول از `datetime.now()` می‌خواند → دو ساعتِ ناهمگام. در واقعیت یکی‌اند.
    (`test_attention`, `test_command_center`, `test_finance_analysis`,
    `test_weekly_review`)
  - ردیف‌های تشخیصِ موبایل زمانشان از `server_default=func.now()` یعنی ساعتِ
    **دیتابیس** می‌آید که کاوشگر آن را جابه‌جا نمی‌کند → ۱۲۰ روز اختلاف. در
    واقعیت هر دو یک ساعت‌اند. (۷ تستِ `test_mobile_companion`)
- **FINDING (بمبِ واقعی — فتیلهٔ یک‌ماهه)**
  `test_finance_precision_overhaul::test_balance_delta_is_not_summed_in_the_monthly_report`
  داده را روی «۲۰۲۶-۰۷» ثابت می‌کرد و `build_report(months=2)` پنجره‌اش نسبت به
  «الان» است. امروز (اول مرداد) هنوز سبز بود و **اول شهریور** با یک
  `StopIteration` لخت قرمز می‌شد، بدون آنکه کسی چیزی عوض کرده باشد.
- **CHANGE** ماهِ داده از **همان ساعتی** گرفته می‌شود که خودِ سرویس می‌خواند
  (`datetime.now(timezone.utc)`، نه `date.today()`). **VERIFY** همان فایل با
  ساعتِ +۰ / +۱۲۰ / +۴۰۰ / +۱۰۰۰ روز: هر چهار بار سبز.
- **DECISION** ابزارِ جابه‌جاییِ ساعت در repo نگه داشته نمی‌شود (وابستگیِ تازه
  نمی‌خواهیم و خودش دو تلهٔ isinstance دارد)؛ روشش در experience ثبت شد تا هر بار
  لازم شد در چند خط بازساخته شود.

## 2026-08-01 — ممیزیِ کاملِ کارِ اخیر: ۳۱ ادعا، ۱۵ تأییدشده، ۱ ردشده

- **روش** ۸ ممیزِ موازی روی ۸ برشِ کار (هویت/مکان، حلقهٔ ابهام و تلگرام، بک‌اندِ
  موبایل، اپِ اندروید، مهاجرت‌ها، امنیت و دامنهٔ داده، فرانت‌اند، کیفیتِ تست)، بعد
  **یک شکاکِ مستقل برای هر ادعا** که وظیفه‌اش **ردکردن** آن بود. نتیجه:
  **۱۵ CONFIRMED / ۱۵ OVERSTATED / ۱ REFUTED**. یعنی نیمی از یافته‌های خامِ
  ممیزی در برخورد با کدِ در حالِ اجرا آب رفت — بدونِ مرحلهٔ ابطال، نیمی از این
  گزارش نویز بود. (خودِ من هم یکی از ادعاها را «تأییدشده» خواندم و شکاک نشان داد
  اشتباه می‌کنم: کدِ نردبانِ وضعیت را درست خوانده بودم ولی **مصرف‌کننده‌اش** را نه.)

### اصلاح‌شده — امنیت
- **CHANGE (بحرانی)** `POST /api/clarifications/ask` مقصد (`target`) را از بدنهٔ
  درخواست می‌گرفت و **بدونِ اعتبارسنجی** ذخیره می‌کرد؛ ۵ اپلایر از ۷ تا شناسهٔ
  داخلش را بدونِ دامنه resolve می‌کردند و `_apply_to_owner_identity` حتی
  `user_id` قربانی را از همان بدنه می‌خواند. حالا دو لایه: `sanitize_target()`
  در مرز (هر شناسه در دامنهٔ همان کاربر تأیید می‌شود، وگرنه به «none» تنزل
  می‌کند و جواب در صندوقِ خودِ کاربر می‌نشیند — چیزی هدر نمی‌رود)، و
  `file_answers` هویتِ مالکِ فرم را روی مقصد **مهر** می‌زند تا هیچ‌وقت از
  کلاینت نیاید. اپلایرهای inbox/person/place/trip هم مالکیت را جدا بررسی می‌کنند.
- **CHANGE (بالا)** `ask()` روی `source_ref` ادغام می‌کرد **بدونِ دامنهٔ کاربر**:
  دو کاربر با شناسهٔ منبعِ یکسان به یک فرم می‌رسیدند و دومی موضوع و متنِ خامِ
  اولی (پیامکِ بانکی!) را پس می‌گرفت. دامنه اضافه شد.
- **CHANGE (بالا)** `dispatch_pending()` / `find_by_message` / `find_by_quoted_text`
  / `newest_open` بی‌دامنه بودند: فرمِ هر حسابِ دیگری در چتِ **مالک** بالا می‌آمد
  و جوابِ مالک روی دادهٔ آن حساب می‌نشست. مجرای تلگرام حالا فقط دامنهٔ مالک را
  می‌بیند؛ فرمِ بقیه در «سؤال‌های باز»ِ خودشان می‌ماند.
- **VERIFY** ۴ تستِ تازه + **تستِ جهش**: با برداشتنِ هر چهار نگهبان، هر چهار تست
  قرمز می‌شوند.

### اصلاح‌شده — درستیِ داده
- **CHANGE (بالا)** ضدتکرارِ بازدید بر «برابریِ دقیقِ لحظهٔ ورود» بود، ولی پنجرهٔ
  ورودی **غلتان** است و کار **ساعتی**. وقتی لبهٔ پنجره از وسطِ یک اقامتِ طولانی
  رد می‌شد، هر ساعت یک بازدیدِ تازه ساخته می‌شد: یک شبِ خوابِ ۹ ساعته ۱۰ ردیف و
  `total_minutes` تقریباً ۶ برابر. و در جهتِ عکس، اقامتِ **درجریان** هرگز
  به‌روز نمی‌شد. حالا ضدتکرار بر **هم‌پوشانی** است و بازدید سرِ جایش کِش می‌آید.
- **CHANGE (بالا)** `learn_patterns` هر ساعت `is_anomaly` را از نو حساب می‌کرد و
  سفری را که مالک **توضیح داده بود** دوباره غیرعادی می‌کرد → همان سؤال برمی‌گشت.
  خلافِ قیدِ صریحِ مالک. **و یافتهٔ خودمان حینِ اصلاح:** جوابِ مالک به «آنجا چه
  کردی؟» اصلاً **ذخیره نمی‌شد** — فقط در پیامِ تأیید تکرار می‌شد. ستون‌های
  `place_trips.note/explained_at` اضافه شد (ALTER در startup + مهاجرتِ ۰۰۶۱).
- **CHANGE (متوسط)** هیستوگرامِ ساعت با UTC پر می‌شد ولی `infer_kind` ساعت‌های
  **محلی** را می‌سنجید؛ با +۴ ساعت، خواب در سطلِ اداری می‌افتاد و خانه «محل کار»
  برچسب می‌خورد. حالا ساعتِ محلی (همان پیش‌فرضِ موتورِ توجه).
- **CHANGE (متوسط)** `_x_age` سن را از استخراجِ دوبارهٔ سند می‌گرفت، پس تاریخِ
  تولدِ اصلاح‌شدهٔ مالک کنارِ سنِ قدیمی می‌نشست. حالا از مقدارِ قفل‌شدهٔ خودِ پروفایل.
- **CHANGE (متوسط)** `from app.services import identity_facts` مسیرش غلط بود
  (`app.services.ingest.identity_facts`)، پس واقعیتِ هویتیِ رمزنگاری‌شدهٔ خودِ
  مالک — که بالاترین اعتبار (۰٫۹۹) را دارد — **هرگز خوانده نمی‌شد** و
  `except Exception: pass` صدایش را می‌خورد.

### اصلاح‌شده — هشدارها و رابط
- **CHANGE (متوسط)** رویدادِ `location_off` ثبت شده بود ولی **هیچ فرستنده‌ای
  نداشت**: `/api/mobile/location` فیلدِ `location_enabled` را می‌خواند و دور
  می‌ریخت. حالا با پنجرهٔ خنک‌شدنِ ۶ ساعته هشدار می‌دهد و با روشن‌شدنِ دوباره
  یادداشتش پاک می‌شود. (قیدِ صریحِ مالک: «اگر لوکیشن خاموش بود هشدار جدی بده».)
- **CHANGE** صفحهٔ «من که هستم» **هیچ دری نداشت** — فقط با تایپِ آدرس باز می‌شد.
  به منو اضافه شد.
- **CHANGE** `IdentityProfile` خطای خواندن را به «۰ از ۰» ترجمه می‌کرد (نبودِ داده
  از نرسیدنِ داده قابلِ تشخیص نبود) و هیچ دکمهٔ ذخیره‌ای شکستش را نشان نمی‌داد.
- **CHANGE** «نمایش دوباره»ی تلگرام سهمیهٔ تلاش را می‌سوزاند؛ پنج بار زدنش فرم را
  park می‌کرد — یعنی «دوباره نشانم بده» عملاً «دیگر نپرس» می‌شد.
- **CHANGE** با یک جلسهٔ compose باز، سؤالِ متقابلِ مالک به بافرِ «کارِ جدید»
  می‌رفت و به `discuss()` نمی‌رسید. حالا حالتِ «سؤال دارم» مقدم است.
- **CHANGE** فهرستِ `silent` در پاسخِ تشخیص، `partial` را جا می‌گذاشت؛ و خلاصهٔ
  نوار «داده نمی‌فرستد» می‌گفت در حالی که سطرش «فعال، ولی ناپایدار» بود.
- **CHANGE (اندروید)** ترجیحِ `precise_tracking` وقتی سرویس **خودش** متوقف می‌شد
  (اجازهٔ «تقریبی»، کشته‌شدن توسط سازنده) پاک نمی‌شد؛ کارگرِ دوره‌ای با دیدنِ
  همان ترجیح نمونه‌برداری نمی‌کرد و **هر دو** مجرا با هم ساکت می‌شدند در حالی که
  رابط «روشن» نشان می‌داد. حالا نبضِ `isAlive()` ملاک است، نه ترجیح.
- **CHANGE (اندروید)** `NotifListener` بعد از قطعِ اتصال (به‌روزرسانیِ اپ،
  کم‌حافظگی) دیگر برنمی‌گشت. `onListenerDisconnected` + `requestRebind` اضافه شد.
- **CHANGE** مهاجرتِ ۰۰۵۵ تنها مهاجرتِ تازه‌ای بود که `create_table` را
  **بی‌نگهبان** صدا می‌زد؛ مثل خواهرهایش گارد گرفت.
- **CHANGE** نقطهٔ موقعیت با زمانِ بی‌منطقه، مقایسهٔ ضدتکرار را با `TypeError`
  می‌شکست و کلِ بسته ۵۰۰ می‌گرفت — یعنی یک نقطهٔ بدشکل تمامِ صفِ آفلاین را
  زمین می‌زد. حالا همیشه به UTC نرمال می‌شود.

### کیفیتِ تست (سه موردِ تأییدشده)
- **CHANGE** `test_person_matching_...` اصلاً محصول را صدا نمی‌زد — الگویی را که
  خودش تعریف کرده بود می‌سنجید. حالا `_match_person` را با دادهٔ واقعی و با
  مرزِ مالک می‌آزماید.
- **CHANGE** `test_a_dead_phone_stays_silent_...` با ۴ ردیف در برابرِ سقفِ ۵۰۰
  اجرا می‌شد، پس نسخهٔ معیوب هم سبز می‌ماند. حالا از سقف عبور می‌کند.
- **CHANGE** `test_an_empty_batch_still_reports_the_location_switch` فقط ۲۰۰ را
  می‌سنجید. حالا خودِ هشدار و پنجرهٔ خنک‌شدنش را می‌سنجد.
- **CHANGE** ۱۲ نقطهٔ هر دو روترِ تازه هیچ تستِ «توکنِ جعلی ⇒ ۴۰۱» نداشتند؛
  تستِ پارامتری اضافه شد.

## 2026-08-01 — «من که هستم» از جزیره به گردآورنده

- **FINDING (نقدِ مالک، و درست بود)** صفحهٔ «من که هستم» دو خرابیِ جدا داشت:
  (۱) **کم‌عمق** — «نقاط قوت» یک f-string بی‌آستانه بود و نمرهٔ **۱۰ از ۱۰۰**
  را به‌عنوان نقطهٔ قوت نشان می‌داد؛ (۲) **موازی‌کاری** — به‌جای وصل‌کردنِ
  «خودنگاره»، «پروندهٔ زندگی»، «نوشته‌های من» و ستون‌های `users`، جزیرهٔ
  تازه‌ای شد. نقشه‌برداری با ۷ ممیز تأیید کرد: از ۱۴ فیلد، ۱۳ تا رونوشتِ خامِ
  یک ستون یا کدِ مرده بودند.
- **FINDING (سه فیلدِ ساختاراً مرده)** `weaknesses` → `select(TodoItem.title)`
  در حالی که ستون `content` است (`AttributeError`، خفه‌شده در `except`);
  `personality` → دنبالِ `assessment_type='big_five'` که هیچ‌جا نوشته نمی‌شود;
  `interests` → `i.get('name')` در حالی که کلیدها `category/score/terms`اند.
  هر سه از روز اول مرده بودند و هیچ تستی نمی‌دیدشان.
- **FINDING (فروختنِ حدس به‌جای واقعیت)** `residence` وقتی GPS نبود،
  `identity_documents.issue_place` — **محلِ صدورِ کارت** — را به‌عنوان محلِ
  زندگی می‌نوشت.
- **DECISION (تأییدِ مالک)** صفحه **گردآورنده** می‌شود و هیچ داده‌ای از خودش
  ذخیره نمی‌کند؛ تنها استثنا حرفِ خودِ مالک است که قفل می‌شود.
- **CHANGE** پکیجِ `app/services/owner_insight/` با سه قیدِ ساختاری:
  هر ادعا `tone` دارد (نبودِ همین بود که عددِ پایین را «قوت» کرد)؛ `owns_page`
  اجباری است (تضمینِ ضدِ جزیره)؛ و «نمی‌دانم» جوابِ مجاز است.
  کشفِ منابع خودکار است (`pkgutil`) — منبعِ تازه = یک فایلِ تازه، بدونِ
  ویرایشِ هیچ فهرستی. خواستهٔ صریحِ مالک دربارهٔ منابعی که بعداً اضافه می‌شوند.
- **CHANGE** هفت منبع: `documents`، `self_model` (با احترام به `has_signal` و
  شاخه‌بندی روی مقدار)، `writings`، `behaviour`، `habits`، `world`، و
  `unlinked` (که محتویاتِ واقعیِ ستون‌های مردهٔ `users` را نشان می‌دهد — چون
  مالک خواست اول ببیند).
- **FINDING (باگِ خودم، پیداشده توسطِ ممیز)** در اسکلت، تابعِ `providers()` و
  زیرپکیجِ `providers/` هم‌نام بودند؛ پایتون ماژول را روی همان نام می‌نشانَد،
  پس تابع بعد از اولین بارگذاری با ماژول جایگزین می‌شد و فراخوانیِ دوم
  `TypeError` می‌داد. یعنی روت در **درخواستِ دوم** ۵۰۰ می‌گرفت — باگی که یک
  بار کار می‌کند و بعد می‌شکند. نام به `registered_providers()` تغییر کرد و
  تستِ رگرسیون، روت را **دو بار پشتِ سرِ هم** صدا می‌زند.
- **VERIFY** ۱۲۳ تستِ تازه؛ هر منبع تستی روی **دیتابیسِ خالی** دارد که ثابت
  می‌کند چیزی از خودش درنمی‌آورد، و منابعِ دو-حالته برای هر دو شاخه تست دارند.
- **NOTE** هیچ ظرفیتی حذف نشد: `owner_identity_service`، مسیرِ ویرایش/قفل، و
  همهٔ صفحه‌های موجود دست‌نخورده‌اند.

## 2026-08-01 — موقعیت: از «۱۱ نقطهٔ موقعیت» به نشانی، خطِ حرکت و سؤالِ معنادار

- **FINDING (نقدِ مالک، و هر چهار موردش درست بود)** نقاطِ موقعیت جمع می‌شدند،
  خوشه می‌شدند و الگو می‌شدند — و بعد **هیچ اتفاقی نمی‌افتاد**. تأییدشده در کد:
  - `places.address` از ابتدا وجود داشت و **هیچ‌جای برنامه در آن نمی‌نوشت**؛
    فقط در serialization خوانده می‌شد.
  - `google_maps_service` تنها `geocode_address` داشت (نشانی ← مختصات).
    **جغرافیای معکوس — همان جهتی که اینجا لازم بود — اصلاً نوشته نشده بود.**
  - در کلِ `frontend/src` هیچ نقشه‌ای نبود (نه leaflet، نه polyline) و
    **هیچ روتی هم نقاط یا سفرها را برنمی‌گرداند**. یعنی تنها راهِ دیدنِ این
    داده یک سطرِ لاگ بود.
  - سؤالِ تلگرام: «سفری که با الگوهای همیشگی‌ات نمی‌خواند (۲۵ دقیقه، ۸.۴
    کیلومتر)» — در حالی که `from_place_id`، `to_place_id` و `started_at`
    همه در دست بود و **هیچ‌کدام استفاده نمی‌شد**. مالک درست گفت که این سؤال
    بی‌معناست: خودش هم نمی‌توانست جوابش را به یاد بیاورد.
- **CHANGE** `reverse_geocode(lat, lon)` اضافه شد و **یک بار برای هر مکانِ
  تازه** صدا زده می‌شود، نه برای هر نقطه — یک شبِ خواب صدها نقطه دارد ولی یک
  مکان است. نشانی در `places.address` می‌نشیند و نامِ محله برچسبِ اولیه می‌شود
  (بدونِ قفل، تا حرفِ مالک بعداً جایگزینش کند). بدونِ کلیدِ Maps، None
  می‌ماند و مختصات نمایش داده می‌شود — افتِ کیفیت، نه خطا.
- **CHANGE** روترِ تازهٔ `app/routes/places.py`: `/api/places` (مکان‌ها و
  سفرها با **نام و ساعتِ محلی**) و `/api/places/track` (نقاطِ خام به‌ترتیبِ
  زمان، تفکیک‌شده به دستگاه).
- **CHANGE** صفحهٔ «کجاها بوده‌ام» (`/places`): مسیر با SVG رسم می‌شود — یک
  خط برای هر گوشی، مکان‌های نام‌دار، و **خطِ مقیاس** (بدونِ آن شکل بی‌معناست).
  عرضِ جغرافیایی تصحیح می‌شود وگرنه مسیر در جهتِ شرق-غرب کشیده می‌شود و شکلش
  دروغ می‌گوید. عمداً بدونِ Leaflet/Google Maps: کاشیِ نقشه یعنی درخواست به
  میزبانِ بیرونی و یک وابستگیِ تازه.
- **CHANGE** سؤالِ سفر حالا **جا و ساعت** را می‌گوید:
  «ساعت ۱۴:۲۰ از «خانه» رفتی به «Baniyas Road, Deira» — آنجا چه بود؟»
- **FINDING (دروازه کار کرد)** صفحه را در سه رجیستری ثبت کردم ولی
  `docs/ARCHITECTURE_INVENTORY.json` را جا انداختم؛
  `test_inventory_lists_all_frontend_pages` گرفتش. همان قاعده‌ای که برای
  «هیچ تغییری از نقشه جا نماند» گذاشته شده بود، این بار خودم را گرفت.
- **NOTE (محدودیتِ صادقانه)** نشانی فقط برای مکان‌هایی می‌آید که از این پس
  کشف شوند. مکان‌های موجود تا وقتی دوباره دیده نشوند `address` خالی دارند —
  یک کارِ یک‌باره برای پرکردنِ گذشته لازم است و هنوز نوشته نشده.

## 2026-08-02 — «چی کار کنم کلا؟»: از تشخیصِ جزیره‌ای‌بودن تا اولین حرکت

- **FINDING (نشتِ داده — رفع شد)** «نوبت دکتر فردا ساعت ۱۰» هم تاریخش را
  گم می‌کرد هم ساعتش را، به دو دلیلِ جدا: (۱) پرامپتِ دسته‌بندیِ صندوق ورودی
  **هرگز به مدل نمی‌گفت امروز چندم است**، پس «فردا» اصلاً قابلِ محاسبه نبود —
  مدل یا `null` می‌داد یا از خودش تاریخ می‌ساخت؛ (۲) حتی با تاریخِ درست،
  ساعت جایی برای نشستن نداشت چون `due_date` از نوع `Date` است.
- **CHANGE** «امروز» با تاریخ و نامِ روزِ هفته **به وقتِ محلیِ مالک** وارد
  پرامپت شد (نه UTC — ساعت ۲ بامدادِ دبی هنوز دیروزِ UTC است و «فردا» یک روز
  زودتر می‌نشست)، فیلدِ `due_time` اضافه شد، و ساعت در `Task.deadline` که
  ستونِ timestamp است ذخیره می‌شود. `_norm_date` ارقامِ فارسی/عربی را هم
  می‌پذیرد. `_norm_time` ساعتِ تنها («۱۰») را **عمداً رد می‌کند** چون به همان
  اندازه می‌تواند «مقدار» باشد؛ حدس‌زدنش یعنی ساعتِ غلط روی تقویمِ مالک.
- **DECISION (پاسخ به سؤالِ راهبردیِ مالک)** چهار طرحِ مستقل برای مسئلهٔ
  «همه‌چیز جزیره است» ساخته و توسطِ دو داورِ مستقل (یکی از زاویهٔ مالک، یکی
  مهندسِ بدبین) امتیازدهی شد. هر دو داور مستقلاً به یک چیز رسیدند که در
  خلاصهٔ اولیه نبود: **پُل‌ها وجود دارند، ولی نمی‌گویند کجا فرود می‌آیند.**
- **FINDING (اندازه‌گیری‌شده)** `grep` برای `useSearchParams` و
  `scrollIntoView` در کلِ `frontend/src/` → **صفر نتیجه**. یعنی برنامه
  هیچ راهی برای آدرس‌دادن به **یک ردیف** نداشت. `global_search.py` دقیقاً
  `task.id` را در دست داشت و لینکِ `"/tasks"` می‌داد؛ کارت‌های ساحت هم یک
  کارِ مشخص را نام می‌بردند و به `/tasks` لینک می‌دادند.
- **CHANGE (اولین حرکت)** `app/services/focus_service.py` +
  `frontend/src/lib/focus.js` — قراردادِ `?focus=kind:id`. در **گلوگاه**
  نصب شد: تابعِ `add()` در جستجو (۹ دامنه یکجا) و تابعِ `att()` در
  `sahat_service` (کارت‌های کار/آیتم/فرد/مسیر). صفحه‌های Tasks، Lists،
  ListDetail و Writings با دو خط عضو شدند.
- **DECISION (چرا این، نه لایهٔ افقی)** طرحِ «ستون‌فقرات» (`/api/facets`)
  امتیازِ برابر گرفت ولی هر دو داور یک ایراد را به آن گرفتند: درِ خروجیِ
  هر facet یک **صفحه** است نه یک ردیف — یعنی همان حسِ نیمه‌وصل را بازتولید
  می‌کرد. پس ترتیب برعکس شد: اول primitive، بعد لایه. لایه بعداً از همین
  `focus_url` استفاده می‌کند.
- **NOTE (رفتار حفظ شد)** قرارداد عمداً **قابلِ نادیده‌گرفتن** است: صفحه‌ای
  که عضو نشده یک query param ناشناس می‌بیند و مثلِ دیروز رندر می‌شود. پس
  تولیدکننده‌ها همین حالا منتشر شدند و بقیهٔ صفحه‌ها یکی‌یکی عضو می‌شوند؛
  رول‌اوتِ نیمه‌کاره هم از دیروز بهتر است.
- **VERIFY** `1765 passed` + بیلدِ فرانت سبز. جهش‌ها آزموده شد: برگرداندنِ
  `focus_url(url, ...)` به `url` در جستجو، حذفِ `deadline`، حذفِ تبدیلِ
  تایم‌زون، و حذفِ «امروز» از پرامپت — **هر چهار جهش تست را قرمز می‌کند**.
- **FINDING (از قبل موجود — کارِ من نیست، ولی ثبت می‌شود)** `npx vitest run`
  پانزده تستِ قرمز دارد در `Footer/Header/Layout/api/Notifications.settings`.
  با `git stash` روی HEADِ تمیز هم **دقیقاً همان ۱۵ تا** قرمزند. دروازهٔ
  merge (`pytest` + `npm run build`) سبز است، ولی این‌ها بدهیِ جداگانه‌ای
  است که باید جدا رسیدگی شود.

## 2026-08-02 (۲) — لایهٔ facets: ستون‌فقرات دهانِ دوم پیدا کرد

- **FINDING (اندازه‌گیری‌شده، تعیین‌کننده)** `owner_insight.collect()` — هفت منبع و
  حدود ۳٬۶۰۰ خطِ تست‌شده — **یک** صداکننده داشت: `GET /api/identity-profile`. و
  کارتِ نمایشش یک تابعِ محلی داخلِ `pages/IdentityProfile.jsx` بود، نه کامپوننت،
  پس هیچ صفحهٔ دیگری نمی‌توانست از آن استفاده کند. اندام سالم بود، دهان یکی بود.
- **FINDING (اجرا شد، نه استدلال — این نقشه را عوض کرد)** خروجیِ خامِ `collect()`
  روی یک پایگاه‌دادهٔ ۸ ردیفی (۳ کار + ۴ قلمِ فهرست) **دقیقاً یک جمله** است:
  «پشتکارت این دوره پایین بوده؛ … از هر ۱۰ تا حدود ۰ تا را نگه داشته‌ای.»
  یعنی تنها چیزی که برنامه دربارهٔ مالک می‌گوید یک نمرهٔ ارادهٔ پایین است — همان
  «شاخص پشتکار ۱۰/۱۰۰» که مالک گفت «احمقانه»، این بار در قالبِ جمله. و کلِ
  ۱٬۷۷۸ تست سبز می‌ماند. **یک مجموعه‌تستِ سبز می‌تواند یک قابلیتِ مرده — یا
  توهین‌آمیز — را تأیید کند.**
- **DECISION** نقشهٔ اولیه («یک روتِ عمومی که همه‌چیز را می‌دهد») رد شد.
  `/api/facets` **گزینش‌شده** منتشر می‌شود و مصرف‌کنندهٔ اولش خودِ
  `/identity-profile` است؛ نوارِ میز فرمان کامیتِ بعدی است، بعد از این‌که مالک
  مجموعهٔ گزینش‌شده را روی صفحه‌ای که خودش باز می‌کند دیده باشد.
- **CHANGE (قرنطینه، نه حذف — قاعدهٔ ۲)** دو کارت **معیوب** از سطحِ تازه بیرون
  گذاشته شد و علتِ هرکدام با شواهدِ اجراشده در `REMOVAL_CANDIDATES.md` ثبت شد:
  `self_model_diligence` (جملهٔ «این دوره» دروغ است — `compute_diligence` هیچ
  پنجرهٔ زمانی ندارد، پس نمره هرگز نمی‌تواند بهتر شود) و `self_model_interests`
  (دسته‌بند با زیررشته کار می‌کند: «برنامه‌ریزی»→فناوری، «خدا»/«خانواده»/«سلامتی»
  →general که دور ریخته می‌شود؛ ساختاراً نمی‌تواند دربارهٔ ایمان یا خانواده چیزی
  بگوید). هر دو روی `/api/identity-profile` سرِ جای‌شان‌اند و با `?include=`
  برمی‌گردند.
- **CHANGE** دو گروه پیش‌فرض ساکت‌اند (خراب نیستند، جای دیگری دارند): `unlinked`
  (گزارشِ ستون‌های بی‌مصرفِ خودِ برنامه — صاحبش `/system-map`) و `facts`
  (رونوشتِ مدارکِ هویتی؛ ضمناً این استقرار `REQUIRE_AUTH=False` دارد). با
  `?groups=` کاملاً برمی‌گردند.
- **CHANGE (اتصالِ دیروز به امروز)** `Facet` سه فیلدِ **افزودنی** گرفت:
  `focus_kind`/`focus_id` و `surfaces`. `owns_page` **دست نخورد** — حدود بیست
  assertion در شش فایلِ تست آن را «یک مسیرِ خالی» می‌خواهند — و به‌جایش
  `link` محاسبه می‌شود که `?focus=` دیروز را روی مسیر می‌نشاند. نتیجهٔ اجراشده:
  کارتِ «بلندترین نوشته‌ات» حالا لینکش `/writings?focus=writing%3A1` است، یعنی
  روی **خودِ آن نوشته** فرود می‌آید نه سرِ فهرست. همان ایرادی که هر دو داور به
  طرحِ ستون‌فقرات گرفته بودند.
- **DECISION (`surfaces` به‌جای عددِ salience)** یک کارت فقط وقتی به نوارِ یک
  صفحه می‌رسد که نویسنده‌اش صراحتاً آن سطح را نوشته باشد. عمداً عدد نیست: عدد را
  آن‌قدر تنظیم می‌کنند تا بی‌معنا شود، ولی افزودنِ یک سطح یک خطِ خوانا در diff است.
- **CHANGE** `components/FacetStrip.jsx` — کارت از صفحه بیرون آمد. سه چیزی که یک
  «کپیِ عیناً» خراب می‌کرد و رعایت شد: `<ul>`ِ خودش (کارت `<li>` است)،
  `dir="rtl"`ِ خودش (جمله‌ها فارسی و لاتین را قاطی می‌کنند و build این را
  نمی‌گیرد)، و `.catch` که خطای شبکه صفحهٔ میزبان را نخواباند. رندرِ
  `IdentityProfile` عوض نشد.
- **FINDING (تستِ خودم پوچ بود — گرفتم و درست کردم)** اولین نسخهٔ
  `test_every_facet_door_is_a_real_page` دنبالِ `owns_page="..."` می‌گشت، ولی
  **همهٔ** مقادیر در این پکیج ثابتِ ماژول‌اند (`PAGE`، `PAGE_LISTS`…)، پس regex
  هیچ‌چیز نمی‌گرفت و تست بی‌صدا سبز می‌شد. حالا ثابت‌ها resolve می‌شوند و یک
  `checked >= 8` جلوی پوچ‌شدنِ دوباره را می‌گیرد.
- **VERIFY** `1778 passed` + بیلدِ فرانت سبز + `vitest` بدونِ رگرسیونِ تازه
  (همان ۱۵ قرمزِ از قبل موجود، ۱۴۴ سبز مثل قبل). سه جهش آزموده شد —
  برداشتنِ `QUARANTINED_KEYS`، برداشتنِ `QUIET_GROUPS`، و نادیده‌گرفتنِ
  `surface` — **هر سه تست را قرمز می‌کند**.

## 2026-08-02 (۳) — بازبینیِ متخاصمِ لایهٔ facets: یک خرابیِ تولیدی که مجموعه‌تست نمی‌دید

چهار بازبینِ مستقل ۱۸ ایراد آوردند؛ هرکدام به یک شکاکِ جدا داده شد که وظیفه‌اش
**رد کردن** بود. ۱۳ تا رد شد (بدخوانی یا دست‌نیافتنی)، ۵ تا ماند، یکی merge را بست.

- **FINDING (بحرانی — بازتولید شد)** `collect()` هفت provider را با
  `asyncio.gather` روی **یک** AsyncSession پخش می‌کند. موتورِ تولید
  `pool_pre_ping=True` دارد (`app/database.py:25`)، یعنی هر checkout یک
  رفت‌وبرگشتِ واقعی است و **اولین** statementِ هر session به حلقهٔ رویداد کنترل
  می‌دهد. اگر آن اولین statement داخلِ gather صادر شود، providerهای ۲ تا ۷ وسطِ
  provisioning وارد می‌شوند و SQLAlchemy می‌گوید
  «This session is provisioning a new connection; concurrent operations are
  not permitted». هر هفت‌تا استثنای خودشان را می‌بلعند، همه در `unavailable`
  می‌نشینند، و روت با `ok=True, degraded=False` و **صفر کارت** جواب می‌دهد.
  خرابیِ کامل، کاملاً بی‌صدا.
  - *اندازه‌گیریِ خودم روی sqlite* (۷ statementِ همزمان × ۱۰ session):
    `pool_pre_ping=False` → ۶/۷۰ شکست؛ `pool_pre_ping=True` → **۶۰/۷۰ شکست**.
  - *تصحیحِ شکاک که وضع را برای من بدتر کرد:* `/api/identity-profile` این ایراد
    را **ندارد**، چون قبل از `collect()` یک `get_identity` صدا می‌زند که اتصال
    را می‌گیرد. یعنی روتِ تازهٔ من **اولین** صداکننده‌ای است که این باگِ نهفته را
    واقعاً شلیک می‌کند.
  - *چرا دروازه کور بود:* موتورِ `tests/conftest.py` بدونِ `pool_pre_ping` ساخته
    می‌شود. ۱٬۷۷۸ تست سبز بود روی یک endpointِ خراب.
- **CHANGE** قبل از fan-out یک `SELECT 1` سریالی اضافه شد تا اتصال گرفته شود.
  `tests/test_owner_insight_concurrency.py` موتورِ **خودش** را با
  `pool_pre_ping=True` می‌سازد وگرنه گارد تشریفاتی است. جهش (برداشتنِ warm-up)
  همان خطای تولیدی را عیناً بازتولید می‌کند و تست قرمز می‌شود.
- **FINDING (تستِ خودم پوچ بود — دوباره)** پنج تستِ سطحِ endpoint در
  `test_facets_route.py` روی پایگاه‌دادهٔ **خالیِ** fixture اجرا می‌شدند، پس
  `facets: []` بود و همهٔ assertionها روی فهرستِ خالی بدیهی. کلِ `curate()`
  را می‌شد از روت برداشت و **هیچ تستی قرمز نمی‌شد**. حالا `collect()` با
  مجموعه‌ای معلوم stub می‌شود؛ همان جهش الان **۴ تست** را قرمز می‌کند.
- **REVERT (تصحیحِ سندِ خودم)** در `REMOVAL_CANDIDATES.md` نوشته بودم نمرهٔ
  پشتکار «هرگز نمی‌تواند بهتر شود». **غلط بود.** اندازه‌گیری: با ۳ کارِ باز نمره
  ۰ است و با تمام‌کردنشان ۱۰۰ می‌شود. ادعای درست: نسبت **مادام‌العمر** است، پس با
  انباشتِ تاریخی هر تلاش فقط کسری تکانش می‌دهد (۲۰۳ کار، تمام‌کردنِ ۲۰ تا: ۱ → ۱۱)
  و «این دوره» همچنان به هیچ پنجره‌ای اشاره نمی‌کند. سند تصحیح شد — یک توجیهِ
  غلطِ ثبت‌شده از نبودِ توجیه بدتر است.
- **NOTE (رد شد، ثبت می‌شود)** سه ادعا دربارهٔ «`?groups=facts` بدونِ توکن
  مدارک را می‌دهد» رد شد: افشا **از قبل** روی `/api/identity-profile` هست و از
  `REQUIRE_AUTH=False` می‌آید، نه از این کامیت. ادعای «`surfaces` هیچ تولیدکننده‌ای
  ندارد پس مرده است» هم رد شد: مکانیزم عمداً قبل از مصرف‌کننده‌اش ساخته شده و
  پیش‌فرضِ خالی یعنی «فقط صفحهٔ گردآورنده».
- **NOTE (از قبل موجود، رفع نشد)** با یک مدلِ «شخصیت» پیکربندی‌شده، `writings`
  یک فراخوانِ LLM بدونِ کش دارد با بودجهٔ ۸ ثانیه. تا وقتی نوار روی صفحهٔ
  پربازدید ننشسته این هزینه پرداخت نمی‌شود؛ قبل از mount کردن روی میز فرمان
  باید کش یا snapshot بیاید.
- **VERIFY** `1783 passed` + بیلدِ فرانت سبز + `vitest` بدونِ رگرسیونِ تازه
  (همان ۱۵ قرمزِ از قبل، ۱۴۴ سبز).
